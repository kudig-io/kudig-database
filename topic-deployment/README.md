...
| 类别 | 文档路径 | 说明 |
|------|---------|------|
| 集群架构模式 | `domain-1-architecture-fundamentals/12-cluster-deployment-patterns.md` | 各种部署架构模式详解 |
| 集群生命周期 | `domain-9-platform-ops/02-cluster-lifecycle-management.md` | 创建→运维→升级→回收全流程 |
| 生产部署实践 | `domain-3-control-plane/24-production-deployment-best-practices.md` | 企业级部署最佳实践 |
| Deployment 模式 | `domain-4-workloads/02-deployment-production-patterns.md` | 蓝绿/金丝雀/滚动更新详解 |
| 生产架构原则 | `domain-18-production-operations/01-production-architecture-design-principles.md` | 高可用/安全/可扩展设计 |
| 故障排查大全 | `domain-12-troubleshooting/` | 各类故障排查手册 |
| 网络深入 | `domain-5-networking/` | CNI、Service、Ingress 详解 |
| 存储深入 | `domain-6-storage/` | CSI、PV/PVC、存储类详解 |
| 安全深入 | `domain-7-security/` | RBAC、NetworkPolicy、安全加固 |
| 监控深入 | `domain-8-observability/` | Prometheus、Grafana、日志 |
