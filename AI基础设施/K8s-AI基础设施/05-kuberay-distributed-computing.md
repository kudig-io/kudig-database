...
KubeRay 的核心价值在于：将 Ray 的弹性伸缩能力与 K8s 的资源管理结合，实现 GPU 资源的按需分配；通过 Ray Serve 提供低延迟在线推理；通过 Ray Train 实现 PyTorch/TensorFlow 分布式训练的弹性扩缩；通过 RayJob 管理批处理任务的生命周期。关于分布式训练框架的整体对比，参见 [[AI基础设施/基础设施/05-distributed-training-frameworks]]；GPU 调度参见 [[概念/gpu-scheduling-ai-workloads]]。
...
7. **与 Gang Scheduling 配合**：分布式训练需要所有 Worker 同时就绪，建议配合 [[概念/gang-scheduling]] 避免资源碎片化。Volcano 或 Kueue 可作为调度器。
