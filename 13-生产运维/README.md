---
title: Production Operations
description: 生产运维知识域 — FinOps 成本治理、集群治理、事件响应、绿色计算、工单案例、回复话术
summary: 生产运维知识域入口，涵盖成本优化与 FinOps、集群治理与合规、事件响应与 On-Call、绿色计算、工单案例库、回复话术模板
category: domain
tags:
- finops
- cost-optimization
- incident-response
- governance
- green-computing
- operations
tier: core
created: '2026-05-23'
last_updated: '2026-07-21'
difficulty: intermediate
audience:
- SRE
- 所有工程师
- 平台团队
estimated_read_time: 5min
---
# 生产运维 Production Ops

> 成本治理、集群治理、事件响应、绿色计算、工单案例与回复话术。

## 二级子目录

| 子目录 | 内容 | 核心话题 |
|--------|------|----------|
| [[13-生产运维/01-成本治理/README.md\|成本治理/]] | FinOps | 资源右调优、Spot/RI、成本分摊、Kubecost |
| [[13-生产运维/02-集群治理/README.md\|集群治理/]] | 治理 | OPA/Gatekeeper、准入控制、合规审计 |
| [[13-生产运维/03-事件响应/README.md\|事件响应/]] | 事件 | On-Call 流程、严重级定义、升级路径、复盘 |
| [[13-生产运维/04-绿色计算/README.md\|绿色计算/]] | 绿色 | 碳排放监控、能效优化、调度策略 |
| [[13-生产运维/05-工单案例/README.md\|工单案例/]] | 工单 | 典型故障工单、处理流程、知识沉淀 |
| [[13-生产运维/06-回复话术/README.md\|回复话术/]] | 话术 | 客户沟通模板、状态通报、SLA 说明 |

## 跨域导航

- [[15-AI基础设施/README.md|AI基础设施]]
- [[16-专项技术/README.md|专项技术]]
- [[18-云厂商/README.md|云厂商]]
- [[11-发布变更/README.md|发布变更]]
- [[09-可观测性/README.md|可观测性]]
- [[12-可靠性/README.md|可靠性]]
- [[06-存储/README.md|存储]]
- [[08-安全/README.md|安全]]
- [[14-容器运行时/README.md|容器运行时]]
- [[02-工作负载/README.md|工作负载]]
- [[10-平台工程/README.md|平台工程]]
- [[04-应用模式/README.md|应用模式]]
- [[19-故障诊断/README.md|故障诊断]]
- [[07-数据库中间件/README.md|数据库中间件]]
- [[03-清单模式/README.md|清单模式]]
- [[21-生态参考/README.md|生态参考]]
- [[17-系统基础/README.md|系统基础]]
- [[05-网络/README.md|网络]]
- [[01-集群基础/README.md|集群基础]]
