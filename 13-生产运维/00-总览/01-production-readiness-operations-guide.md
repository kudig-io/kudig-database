...
本指南为 **生产运维** 提供生产就绪（Production Readiness）门控框架与持续运维动作集，适用于集群上线前评审、重大变更前的状态确认，以及日常 SRE 值班。生产就绪不是一次性验收，而是贯穿容量、成本、安全、可观测性与事件响应的持续状态管理。它不会替代 [[13-生产运维/07-运维手册/01-production-sre-daily-ops.md|日常巡检与值班手册]] 或 [[13-生产运维/07-运维手册/02-change-management-guide.md|变更管理指南]]，而是将这些实践汇总为可执行的生产就绪标准，并在 gap 分析推荐的专题缺失时给出最小可行方案。
...
当监控触发 P0/P1 告警时，立即按 [[13-生产运维/03-事件响应/04-on-call-playbook.md|值班手册]] 启动升级，并按 [[13-生产运维/03-事件响应/05-incident-response-template.md|事故响应模板]] 记录时间线。
...
- **可靠性工程**：SLO/SLI 定义、容灾架构、故障演练与 postmortem 由 [[12-可靠性/README.md|可靠性]] 主导，生产运维域负责落地值班、告警响应与执行回滚。
- **平台工程**：自助服务平台、租户 onboarding、配额模板与 GitOps 基线由 [[10-平台工程/README.md|平台工程]] 提供，生产运维域负责监控这些策略在生产环境中的实际效果。
- **安全合规**：PSP→PSS 迁移、CIS 加固、Secret 轮换与 RBAC 审计由 [[08-安全/README.md|安全]] 主导，生产运维域负责将安全基线纳入变更窗口与巡检。
- **可观测性**：指标、日志、链路追踪与 SLO dashboard 由 [[09-可观测性/README.md|可观测性]] 建设，生产运维域使用这些工具进行告警与事故定位。
- **发布与变更管理**：GitOps 工作流、金丝雀/蓝绿发布策略由 [[11-发布变更/README.md|发布变更]] 定义，生产运维域执行发布窗口管理与回滚操作。
- **故障排查与诊断**：复杂故障的根因定位与 FTA 由 [[19-故障诊断/README.md|故障诊断]] 支持，生产运维域提供一线排查数据与变更上下文。
- **云服务商**：ACK/EKS/GKE 等 provider 特有升级、DR、配额与账单由 [[18-云厂商/README.md|云厂商]] 覆盖，生产运维域将 provider 操作纳入统一 runbook。

## 6. 推荐阅读

### 本域核心资料

- [[13-生产运维/07-运维手册/01-production-sre-daily-ops.md|生产环境日常巡检与值班手册]]
...
- [[13-生产运维/02-集群治理/06-resource-quota-management.md|资源配额管理]]
- [[13-生产运维/01-成本治理/05-kubernetes-cost-governance.md|Kubernetes 成本治理]]
...
- 多集群与舰队运维（待补充）
- 灾备与备份恢复运维（待补充）
- [[13-生产运维/07-运维手册/08-security-operations-runbook.md|生产安全运维手册]]（待补充）
- 集群升级运维手册（待补充）
- [[13-生产运维/07-运维手册/09-observability-operations.md|可观测性运维]]（待补充）
- GitOps 运维（待补充）
- 自动化修复运维（待补充）
- 节点与容器运行时运维（待补充）
...
- [[08-安全/README.md|安全合规]]
- [[09-可观测性/README.md|可观测性]]
- [[11-发布变更/README.md|发布与变更管理]]
- [[19-故障诊断/README.md|故障排查与诊断]]
- [[18-云厂商/README.md|云服务商]]
