...
| 12 | **灾难演练与混沌工程** | 已完成 AZ 故障切换、节点故障、控制平面断网演练并记录 RTO | 参考 [[12-可靠性/04-混沌工程/01-chaos-engineering-overview.md|混沌工程概述]] |
...
- **[[01-集群基础/README.md|集群基础架构]]** — 控制平面高可用、版本偏斜、API 废弃、节点生命周期由本域落地，但架构设计原则由该域定义。
- **[[05-网络/README.md|网络与流量]]** — VPC/CNI、负载均衡、Ingress、Service Mesh、跨云网络互联依赖该域的网络模型与排障方法。
- **[[08-安全/README.md|安全合规]]** — IAM/workload identity、NetworkPolicy、Pod Security Standards、镜像签名与审计在该域与云 IAM 之间形成交叉责任面。
- **[[09-可观测性/README.md|可观测性]]** — Metrics/Logs/Traces 采集、告警体系、SLI/SLO 定义由该域提供方法论，本域负责对接云厂商监控服务（CloudWatch、SLS、Azure Monitor）。
- **[[10-平台工程/README.md|平台工程]]** — GitOps、多集群 fleet 管理、标准化交付模板、FinOps 由该域主导，本域提供各云厂商的具体实现参数。
- **[[12-可靠性/README.md|可靠性工程]]** — 灾难恢复、备份演练、混沌工程、SLO/SLA 由该域统筹，本域补充厂商特定的 RTO/RPO 方案。
- **[[13-生产运维/README.md|生产运维]]** — 值班、变更管理、事件响应流程由该域定义，本域提供云厂商相关的 runbook 与回滚命令。
...
- [[18-云厂商/01-阿里云/公有云-ACK/alicloud-ack-overview.md|阿里云 ACK 概述]] — ACK 托管版/专有版架构、Terway 网络、RRSA 身份与安全加固。
- [[18-云厂商/07-多云混合/00-multi-cloud-hybrid-deployment-strategy.md|多云混合部署策略]] — 主备/主主模式、跨云数据同步、故障切换与统一监控。
- [[18-云厂商/02-AWS-EKS/aws-eks-overview.md|AWS EKS 概述]] — EKS 架构、IRSA、托管节点组与 Fargate 模式。
- [[18-云厂商/03-Google-GKE/google-cloud-gke-overview.md|Google GKE 概述]] — GKE Autopilot/Standard、Workload Identity、Backup for GKE。
- [[18-云厂商/04-Azure-AKS/azure-aks-overview.md|Azure AKS 概述]] — AKS 网络、Azure AD 集成、托管 Prometheus/Grafana。

### 跨域参考

- [[01-集群基础/00-总览/99-kubernetes-production-architecture-blueprint.md|Kubernetes 生产架构蓝图]] — 生产集群整体架构设计原则。
- [[08-安全/README.md|安全合规域]] — RBAC、NetworkPolicy、Pod Security、镜像安全与审计。
- [[12-可靠性/README.md|可靠性工程域]] — 灾备、备份演练、SLO/SLA 与混沌工程。

---

*本指南作为 云厂商 的统一生产就绪入口，后续将逐步补充各云厂商专属的升级 runbook、灾难恢复 runbook 与可观测性配置 runbook。当前若需具体厂商操作细节，请优先参考上述推荐阅读中的专题页面。*
