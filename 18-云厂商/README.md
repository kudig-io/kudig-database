---
title: Cloud Providers
description: 云厂商知识域 — 阿里云/AWS/GCP/Azure/腾讯云/华为云 K8s 服务对比、多云混合部署、选型决策矩阵
summary: 云厂商知识域入口，涵盖各云托管 K8s 服务对比、多云/混合云架构、云厂商特有功能与限制、选型决策矩阵
category: domain
tags:
- cloud
- aws
- gcp
- azure
- alicloud
- multicloud
- hybrid
tier: core
created: '2026-05-23'
last_updated: '2026-07-21'
difficulty: intermediate
audience:
- 所有工程师
- 架构师
estimated_read_time: 5min
---
# 云厂商 Cloud Providers

> 阿里云、AWS-EKS、Google-GKE、Azure-AKS、腾讯云 TKE、华为云 CCE、多云混合与其他云。

## 二级子目录

| 子目录 | 内容 | 核心话题 |
|--------|------|----------|
| [[18-云厂商/01-阿里云/index.md\|阿里云/]] | ACK / Apsara Stack | ACK 集群管理、Terway 网络、专有云（天基/盘古/合规加固/故障手册） |
| [[18-云厂商/02-AWS-EKS/index.md\|AWS-EKS/]] | EKS | EKS 架构、Fargate、IRSA、Add-ons |
| [[18-云厂商/03-Google-GKE/index.md\|Google-GKE/]] | GKE | Autopilot/Standard、Workload Identity |
| [[18-云厂商/04-Azure-AKS/index.md\|Azure-AKS/]] | AKS | AKS 架构、AAD 集成、Virtual Nodes |
| [[18-云厂商/05-腾讯云TKE/index.md\|腾讯云TKE/]] | TKE | TKE 集群、超级节点、弹性容器 |
| [[18-云厂商/06-华为云CCE/index.md\|华为云CCE/]] | CCE | CCE Turbo、Volcano 调度、鲲鹏 |
| [[18-云厂商/07-多云混合/index.md\|多云混合/]] | 多云架构 | 多云管理、混合云连接、一致性 |
| [[18-云厂商/08-其他云/index.md\|其他云/]] | 其他 | 天翼云/移动云/IBM/Oracle/UCloud/火山引擎 |

## 跨域导航

- [[15-AI基础设施/README.md|AI基础设施]]
- [[16-专项技术/README.md|专项技术]]
- [[11-发布变更/README.md|发布变更]]
- [[09-可观测性/README.md|可观测性]]
- [[12-可靠性/README.md|可靠性]]
- [[06-存储/README.md|存储]]
- [[08-安全/README.md|安全]]
- [[14-容器运行时/README.md|容器运行时]]
- [[02-工作负载/README.md|工作负载]]
- [[10-平台工程/README.md|平台工程]]
- [[04-应用模式/README.md|应用模式]]
- [[19-故障诊断/README.md|故障诊断]]
- [[07-数据库中间件/README.md|数据库中间件]]
- [[03-清单模式/README.md|清单模式]]
- [[13-生产运维/README.md|生产运维]]
- [[21-生态参考/README.md|生态参考]]
- [[17-系统基础/README.md|系统基础]]
- [[05-网络/README.md|网络]]
- [[01-集群基础/README.md|集群基础]]
