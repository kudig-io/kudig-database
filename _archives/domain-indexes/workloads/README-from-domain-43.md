...
```mermaid
graph TB
    ROOT[Java on [[系统基础/topic-cheat-sheet/k8s.md|k8s]]<br/>综合实践指南] --> A[容器化]
    ROOT --> B[运行时]
    ROOT --> C[框架]
    ROOT --> D[CI/CD]
    ROOT --> E[可观测性]
...
| 6 | [Spring Cloud K8s + 服务网格](./网络/99-spring-cloud-kubernetes-service-mesh-guide.md) | Spring Cloud → K8s 原生映射、[[entities/istio.md|Istio]] 集成、Resilience4j、迁移路径 | 高级 |
