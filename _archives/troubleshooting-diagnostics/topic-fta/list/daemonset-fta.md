...
```json
{
  "flow_steps": [
    ...
    { "name": "网络分区", "action": "event", "step": "evt_network_partition", "severity": "critical", "probability": "rare", "mttr_minutes": 30, "detection": { "events": [], "metrics": ["apiserver_request_duration_seconds 异常高"], "logs": ["controller-manager: context deadline exceeded"] }, "remediation": { "manual_steps": ["检查网络连通性", "排查网络设备问题"], "auto_actions": ["网络恢复后自动重连"] } },
