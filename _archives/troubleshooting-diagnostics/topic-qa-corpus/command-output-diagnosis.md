...
<!-- chunk: 1. Pod 状态异常诊断 -->## 1. Pod 状态异常诊断

#<!-- chunk: 1.1 CrashLoopBackOff -->## 1.1 CrashLoopBackOff
...
#<!-- chunk: 1.2 ImagePullBackOff -->## 1.2 ImagePullBackOff
...
diagnosis:
  - "镜像拉取失败, [[entities/kubernetes|k8s]] 正在退避重试"
...
#<!-- chunk: 1.3 Pending (调度失败) -->## 1.3 Pending (调度失败)
...
#<!-- chunk: 1.4 OOMKilled -->## 1.4 OOMKilled
...
#<!-- chunk: 1.5 Init:Error / Init:CrashLoopBackOff -->## 1.5 Init:Error / Init:CrashLoopBackOff
...
<!-- chunk: 2. Node 状态异常诊断 -->## 2. Node 状态异常诊断

#<!-- chunk: 2.1 NotReady -->## 2.1 NotReady
...
#<!-- chunk: 2.2 DiskPressure -->## 2.2 DiskPressure
...
#<!-- chunk: 2.3 MemoryPressure -->## 2.3 MemoryPressure
...
#<!-- chunk: 2.4 PIDPressure -->## 2.4 PIDPressure
...
<!-- chunk: 3. etcd 诊断 -->## 3. etcd 诊断

#<!-- chunk: 3.1 etcd 健康检查失败 -->## 3.1 etcd 健康检查失败
...
#<!-- chunk: 3.2 etcd Leader 切换频繁 -->## 3.2 etcd Leader 切换频繁
...
#<!-- chunk: 3.3 etcd DB 大小超限 -->## 3.3 etcd DB 大小超限
...
#<!-- chunk: 4.1 Service 无法访问 -->## 4.1 Service 无法访问
...
#<!-- chunk: 4.2 CoreDNS 解析失败 -->## 4.2 CoreDNS 解析失败
...
#<!-- chunk: 4.3 Ingress 502 Bad Gateway -->## 4.3 Ingress 502 Bad Gateway
...
#<!-- chunk: 5.1 PVC Pending -->## 5.1 PVC Pending
...
#<!-- chunk: 5.2 Pod 挂载 Volume 失败 -->## 5.2 Pod 挂载 Volume 失败
...
#<!-- chunk: 6.1 资源不足 (Insufficient CPU/Memory) -->## 6.1 资源不足 (Insufficient CPU/Memory)
...
#<!-- chunk: 6.2 ResourceQuota 超限 -->## 6.2 ResourceQuota 超限
...
<!-- chunk: 7. 控制平面诊断 -->## 7. 控制平面诊断

#<!-- chunk: 7.1 API Server 响应慢 -->## 7.1 API Server 响应慢
...
#<!-- chunk: 7.2 Controller Manager 选举失败 -->## 7.2 Controller Manager 选举失败
...
#<!-- chunk: 8.1 RBAC 权限不足 -->## 8.1 RBAC 权限不足
...
#<!-- chunk: 8.2 Pod Security Standards 违规 -->## 8.2 Pod Security Standards 违规
