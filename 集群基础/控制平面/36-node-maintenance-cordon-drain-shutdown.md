...
节点维护（Node Maintenance）是生产集群日常运维中最高频、风险最集中的操作之一。无论是给内核打安全补丁、更换故障内存条、迁移云实例，还是配合 [[集群基础/控制平面/35-cluster-upgrade-runbook.md|集群升级 Runbook]] 滚动升级 kubelet，核心动作都收敛到一条标准链路：
...
| PDB | [[系统基础/知识字典/operations/pdb.md\|PodDisruptionBudget]] 字典条目 | drain 中 PDB 的实战行为与排障 |
...
> **关键认知**：PDB 只保护**自愿中断**（drain、升级、CA 缩容）。节点宕机、OOM Kill、kubelet 硬驱逐属于**非自愿中断**，PDB 不拦。详见 [[系统基础/知识字典/operations/pdb.md|PodDisruptionBudget]]。
...
**为什么 Guaranteed Pod 默认禁用 swap**：Guaranteed Pod 的语义是"资源严格隔离、可预测"。一旦允许 swap，内存超卖后的延迟与 OOM 行为不可预测，破坏了 QoS 承诺。详见 [[集群基础/控制平面/15-kubelet-deep-dive.md|kubelet 深度剖析]] 中 QoS 章节。
...
- [[集群基础/控制平面/15-kubelet-deep-dive.md|kubelet 深度剖析]] — kubelet 配置全貌、QoS、eviction、graceful shutdown 原始配置位
- [[集群基础/控制平面/33-kubelet-eviction-thresholds.md|kubelet 驱逐阈值]] — 节点压力下的 Pod 驱逐机制（与 drain 的自愿驱逐互补）
- [[集群基础/控制平面/35-cluster-upgrade-runbook.md|集群升级 Runbook]] — 升级场景下节点 drain 的滚动流程
- [[集群基础/控制平面/32-kubeadm-upgrade-complete-guide.md|kubeadm 升级完整指南]] — kubelet/kubeadm 升级中的节点维护
- [[可靠性/容量规划/03-resource-quota-limitrange.md|资源配额]] — request/limit 与 QoS，影响 drain 时的容量评估
- [[系统基础/知识字典/operations/pdb.md|PodDisruptionBudget]] — PDB 字典定义，drain 中 PDB 的保护逻辑
...
- [[集群基础/控制平面/31-kubectl-complete-reference.md|kubectl 完整参考]] — cordon/drain/uncordon 命令速查
- [[集群基础/控制平面/06-plane-troubleshooting.md|控制平面故障排查手册]] — 节点级故障诊断
