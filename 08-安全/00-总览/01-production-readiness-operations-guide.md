...
本指南聚焦 [[08-安全/README.md|安全]] 的五大子域：身份访问、网络安全、运行时安全、策略治理、供应链与合规审计。对于各子域的详细原理与配置示例，请参阅对应专题文档；本文仅保留生产就绪必须验证的关键项与可执行命令。
...
kubectl get nodes
```
更完整的轮换流程与回滚方案参见 [[08-安全/06-合规审计/07-certificate-management.md|证书管理与 TLS 配置]]。
...
kubectl get clusterroles -o json | jq -r '
  ...
  .metadata.name'
```
推荐采用最小权限角色模板，具体矩阵参考 [[08-安全/01-身份与访问/06-rbac-matrix-configuration.md|RBAC 权限矩阵表]]。
...
```
签名、SBOM 与 SLSA 实施细节参见 [[08-安全/05-供应链/01-supply-chain-security-overview.md|供应链安全概览]]。
...
```
规则调优与事件响应流程参考 [[08-安全/03-运行时安全/08-falco-runtime-security-guide.md|Falco 运行时安全指南]]。
...
```
网络分段与零信任架构设计参考 [[08-安全/02-网络安全/04-zero-trust-architecture.md|零信任架构]]。
...
- **[[05-网络/README.md|网络]]**：NetworkPolicy、服务网格 mTLS、Ingress TLS 由网络域负责配置与 SLO，安全域负责策略审查与审计。网络域提供流量可见性，安全域基于流量特征生成检测规则。
- **[[09-可观测性/README.md|可观测性]]**：审计日志、Falco 事件、策略告警需统一接入日志平台与告警路由。观测域负责采集、存储与告警通道，安全域负责规则语义、分级与响应。
- **[[11-发布变更/README.md|发布变更]]**：镜像签名、SBOM、GitOps Secret 管理（Sealed Secrets / External Secrets / SOPS）在发布域落地，安全域负责准入校验与合规审计。
- **[[13-生产运维/README.md|生产运维]]**：变更管理、on-call、事件响应由生产运维域统筹，安全域提供安全专项 runbook 与取证支持。
- **[[12-可靠性/README.md|可靠性]]**：安全组件（Vault、cert-manager、OPA、Kyverno、Falco）本身的高可用与灾备由可靠性域负责架构，安全域负责组件选型、策略备份与恢复验证。
...
- [[08-安全/01-身份与访问/06-rbac-matrix-configuration.md|RBAC 权限矩阵表]] — 最小权限角色设计参考
- [[08-安全/04-策略治理/03-pod-security-standards.md|Pod 安全标准详解]] — PSA 分级与迁移
- [[08-安全/06-合规审计/07-certificate-management.md|证书管理与 TLS 配置]] — 集群证书与 cert-manager 运维
- [[08-安全/02-网络安全/04-zero-trust-architecture.md|零信任架构]] — 网络分段与身份驱动访问
- [[08-安全/03-运行时安全/08-falco-runtime-security-guide.md|Falco 运行时安全指南]] — 运行时威胁检测
- [[08-安全/05-供应链/01-supply-chain-security-overview.md|供应链安全概览]] — 镜像签名与 SBOM
- [[13-生产运维/03-事件响应/09-incident-response-process.md|安全事件响应流程]] — 事件响应与取证

### 相关域参考

- [[05-网络/README.md|网络]] — 网络安全策略与服务网格
- [[09-可观测性/README.md|可观测性]] — 安全审计日志与监控告警
- [[13-生产运维/README.md|生产运维]] — 生产运维与 on-call 体系
