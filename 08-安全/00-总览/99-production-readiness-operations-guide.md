...
kubectl get nodes
```
更完整的轮换流程与回滚方案参见 [[08-安全/06-合规审计/10-certificate-management.md|证书管理与 TLS 配置]]。
...
kubectl get clusterroles -o json | jq -r '
  ...
  .metadata.name'
```
推荐采用最小权限角色模板，具体矩阵参考 [[08-安全/01-身份与访问/07-rbac-matrix-configuration.md|RBAC 权限矩阵表]]。
...
```
规则调优与事件响应流程参考 [[08-安全/03-运行时安全/99-falco-runtime-security-guide.md|Falco 运行时安全指南]]。
...
```
网络分段与零信任架构设计参考 [[08-安全/02-网络安全/19-zero-trust-architecture.md|零信任架构]]。
...
- [[08-安全/01-身份与访问/07-rbac-matrix-configuration.md|RBAC 权限矩阵表]] — 最小权限角色设计参考
- [[08-安全/04-策略治理/06-pod-security-standards.md|Pod 安全标准详解]] — PSA 分级与迁移
- [[08-安全/06-合规审计/10-certificate-management.md|证书管理与 TLS 配置]] — 集群证书与 cert-manager 运维
