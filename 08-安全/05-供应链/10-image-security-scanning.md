...
summary: '容器镜像是云原生应用交付的基本单元，其安全性直接决定了运行时安全基线。据 Sysdig 2025 容器安全报告统计，生产环境中超过 75% 的镜像包含已知漏洞，其中约 10% 属于严重级别。镜像安全扫描是 DevSecOps 流程中的关键环节，通过在构建、存储和部署阶段持续检测漏洞、恶意代码和配置问题，建立从代码到运行的完整安全屏障。'
...
echo "<!-- chunk: 镜像漏洞摘要" >> "$REPORT_FILE" -->## 镜像漏洞摘要" >> "$REPORT_FILE"
...
echo "" >> "$REPORT_FILE"
echo "<!-- chunk: 未签名镜像" >> "$REPORT_FILE" -->## 未签名镜像" >> "$REPORT_FILE"
...
echo "" >> "$REPORT_FILE"
echo "<!-- chunk: 使用 latest 标签的镜像" >> "$REPORT_FILE" -->## 使用 latest 标签的镜像" >> "$REPORT_FILE"
kubectl get pods --all-namespaces -o json | \
