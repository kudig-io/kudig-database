...
本文覆盖三大核心轮换场景：TLS 证书自动轮换（基于 cert-manager）、API Key 和 Secret 轮换（配合 External Secrets Operator 和应用热加载）、etcd 加密密钥轮换（Kubernetes 控制平面级别），并详解零停机轮换的通用策略设计。Secrets 管理方案见 [[08-安全/01-身份与访问/08-secrets-management-comparison.md|Secrets 管理方案对比]]，Vault 实践见 [[08-安全/01-身份与访问/05-vault-enterprise-secrets-management.md|Vault 企业级 Secrets 管理]]。
...
- [[08-安全/01-身份与访问/08-secrets-management-comparison.md|Secrets 管理方案对比]]
...
- [[05-网络/01-K8s网络核心/23-ingress-tls-certificate.md|Ingress TLS 证书]]
