...
数据库 Schema 变更是发布流程中风险最高的操作之一。与应用代码可以通过 [[发布变更/变更管理/03-change-rollback-playbook.md|回滚]] 快速恢复不同，数据库变更往往不可逆——一旦列被删除或数据被修改，恢复成本极高。在微服务架构中，多个服务共享数据库或存在数据依赖时，Schema 变更的协调复杂度呈指数增长。
...
3. **迁移前必须备份**：通过 [[可靠性/备份恢复]] 流程确保有可用备份。
...
数据库迁移应集成到 [[发布变更/GitOps/08-cicd-pipeline-patterns.md|CI/CD 流水线]] 中：
...
与 [[发布变更/变更管理/01-change-window-and-approval.md|变更窗口与审批]] 流程集成，确保跨团队协调。

## Related

- [[发布变更/变更管理/03-change-rollback-playbook.md|变更回滚手册]]
- [[发布变更/变更管理/01-change-window-and-approval.md|变更窗口与审批]]
- [[发布变更/GitOps/08-cicd-pipeline-patterns.md|CI/CD 流水线模式]]
