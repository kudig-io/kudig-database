...
本文覆盖回滚自动化的完整模式：触发条件设计、Argo Rollouts Analysis 驱动回滚、健康检查回滚、数据库回滚、有状态服务回滚以及回滚后的验证闭环。与 [[发布变更/变更管理/03-change-rollback-playbook.md|变更回滚手册]] 侧重人工操作手册不同，本文聚焦于自动化回滚的系统设计。
...
与 [[发布变更/变更管理/06-database-migration-release-strategy.md|数据库迁移发布策略]] 配合，实现 Schema 回滚的半自动化：
...
回滚自动化应与 [[发布变更/GitOps/09-argo-rollouts-progressive-delivery.md|Argo Rollouts 渐进式交付]] 和 [[发布变更/变更管理/05-feature-flags-progressive-exposure.md|Feature Flag 渐进式暴露]] 深度集成：
- Feature Flag 提供秒级功能回滚（关闭 Flag）
- Argo Rollouts 提供分钟级版本回滚
- 数据库回滚作为最后手段，需要人工审批

与 [[可靠性/SRE实践/03-incident-command-system.md|事件指挥系统]] 集成，回滚触发后自动创建事件记录。

## Related

- [[发布变更/变更管理/03-change-rollback-playbook.md|变更回滚手册]]
- [[发布变更/GitOps/09-argo-rollouts-progressive-delivery.md|Argo Rollouts 渐进式交付]]
- [[发布变更/变更管理/06-database-migration-release-strategy.md|数据库迁移发布策略]]
- [[发布变更/变更管理/05-feature-flags-progressive-exposure.md|Feature Flag 与渐进式暴露]]
- [[发布变更/变更管理/02-canary-release-strategy.md|金丝雀发布策略]]
