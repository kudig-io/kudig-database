...
核心概念详见 [[concepts/progressive-delivery-strategies]]。
...
- [[concepts/gitops-production-operations]] — GitOps（Argo CD、Flux）是 Kubernetes 声明式变更管理与发布自动化的事实标准
- [[concepts/platform-engineering-idp]] — 内部开发者平台（IDP）将发布流程标准化为自助式工作流，提升交付效率
- [[concepts/slo-error-budget-framework]] — Error Budget 决策框架指导发布频率与变更风险容忍度的平衡
- [[concepts/k8s-observability-stack]] — 发布可观测性（金丝雀分析、自动化回滚）依赖实时指标采集与评估引擎
