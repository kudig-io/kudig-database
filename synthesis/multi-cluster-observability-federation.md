...
relationships:
  - target: "[[domain-17-system-foundation/topic-dictionary/networking/cluster-mesh]]"
    type: uses
  ...
  - target: "[[entities/cilium]]"
    type: uses
  - target: "[[synthesis/Cilium eBPF × 可观测性]]"
    type: uses
---
...
现代 [[entities/kubernetes|Kubernetes]] 平台通常由多个集群组成——按环境划分（开发/测试/生产）、按地域划分（Region A/B/C）、按团队划分（平台/业务/AI）。每个集群独立运行 [[entities/prometheus|Prometheus]]，但运维团队需要一个统一的"全局视图"来回答跨集群问题："所有生产集群的 API Server 延迟趋势如何？""跨地域的服务调用链路是否健康？" 本页连接 domain-03-networking-traffic 的多集群网络拓扑与 domain-06-observability 的联邦监控系统，展示 Thanos 和 Cortex 如何实现真正的多集群可观测性联邦。

## 核心连接

| 域 | 核心能力 | 联邦监控的桥接作用 |
|---|---|---|
| **Networking (domain-03)** | 多集群网络（[[entities/cilium|Cilium]] [[domain-17-system-foundation/topic-dictionary/networking/cluster-mesh|Cluster Mesh]]）、流量路由 | 联邦查询需要跨集群网络可达，mTLS 保障安全传输 |
...
| **自治性 vs 集中化** | 集群本地 Prometheus 自治（无外部依赖）是 K8s 设计哲学，但联邦监控需要依赖中心存储和网络。灾备设计需考虑联邦层故障时的降级方案。 |
...
- **联邦链路故障：** 当某集群与联邦中心断联时，该集群的数据是否能在恢复后补录？Thanos 的"延迟上传"机制如何处理？
- **跨集群追踪关联：** 指标联邦成熟，但跨集群的分布式追踪（Trace）联邦仍不成熟。如何关联集群 A 的入口到集群 B 的服务调用？
- **联邦层的单点故障：** Thanos Query 或 Cortex Query Frontend 成为事实上的单点。如何设计联邦层自身的高可用？
