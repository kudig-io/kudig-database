...
description: '[[concepts/controller-pattern]] 描述协调循环的通用框架，[[entities/deployment]] 是最常用的工作负载控制器。wiki 将前者视为抽象模式、后者视为具体资源，但 Deployment
...
[[concepts/controller-pattern]] 描述协调循环的通用框架，[[entities/deployment]] 是最常用的工作负载控制器。wiki 将前者视为抽象模式、后者视为具体资源，但 Deployment 不仅是控制器模式的一个实例——它是控制器模式的**最小完备实现**。Deployment 的每一特性（声明式 replica 管理、滚动更新策略、版本回滚、进度追踪）都直接映射到控制器模式的四个阶段（Observe→Compare→Act→Update）。
