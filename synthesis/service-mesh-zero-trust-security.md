...
# [[domain-17-system-foundation/topic-dictionary/networking/service-mesh|Service Mesh]] 零信任安全架构

## 概述

零信任（Zero Trust）的核心原则是"从不信任、始终验证"——无论流量来自集群外部还是内部，都必须经过认证和授权。Service Mesh（以 [[entities/istio|Istio]] 为代表）是零信任理念在 [[entities/kubernetes|Kubernetes]] 服务间通信层的具体技术实现：通过自动 mTLS、L7 授权策略和统一身份框架，将安全从"网络边界"下沉到"每次服务调用"。本页连接 domain-03-networking-traffic 的服务网格能力与 domain-05-security-compliance 的零信任安全框架，展示如何在 K8s 环境中构建服务网格驱动的零信任架构。
...
| **[[entities/cilium|Cilium]]** | eBPF 网络 | 替代方案：L4 mTLS + NetworkPolicy |
