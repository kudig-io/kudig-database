...
混沌工程通过在生产环境注入故障来验证系统韧性，可观测性通过采集和分析系统信号来理解系统行为。两者的交汇点在于：**混沌实验的可信度依赖于可观测性——你无法验证"系统在故障中是否表现正确"，除非你能够看到系统在故障中的真实行为。** 本页连接 domain-09-reliability-engineering 的混沌工程方法论与 domain-06-observability 的监控体系，展示如何将可观测性注入混沌实验的全生命周期——从实验设计、执行监控到结果验证。

## 核心连接

| 域 | 核心能力 | 融合的桥接作用 |
|---|---|---|
| **Reliability (domain-09)** | 故障注入、韧性验证、游戏日 | 混沌实验定义"注入什么故障"和"期望什么结果" |
...
| 问题 | 可观测性来源 | 关键指标 |
|---|---|---|
| **故障是否生效？** | 事件日志、Chaos Dashboard | 实验状态、目标 Pod/节点状态 |
...
metadata:
  ...
  annotations:
    # 实验元数据，用于可观测性关联
    experiment.purpose: "验证支付服务在 Pod 故障时的恢复能力"
...
| **Litmus Chaos** | 混沌实验平台 | CloudNative 项目，[[entities/argo|Argo Workflows]] 集成 |
| **Gremlin** | SaaS 混沌平台 | 商业方案，丰富的可观测性集成 |
| **Prometheus** | 指标存储 | 实验期间指标采集和 SLO 验证 |
| **Grafana** | 可视化 | 混沌实验 Dashboard，实时观察 |
| **[[entities/jaeger|Jaeger]]** | 分布式追踪 | 追踪实验期间的请求链路 |
...
| 张力 | 详情 |
|---|---|
| **实验真实度 vs 安全风险** | 越接近真实的故障（如生产环境网络分区）越能发现真实问题，但风险也越大。需要在"发现未知问题"和"避免已知风险"之间平衡。 |
| **自动化频率 vs 团队疲劳** | 频繁的自动化混沌实验（如每天）可能导致团队对告警麻木（"又是混沌实验触发的告警"）。需要区分"混沌告警"和"真实告警"的优先级。 |
| **实验覆盖度 vs 执行成本** | 覆盖所有服务的所有故障类型需要巨大的时间和计算成本。优先覆盖核心业务路径（如支付链路）是务实的选择。 |
...
- **多租户环境的混沌隔离：** 在共享集群中，对 A 团队的命名空间注入故障是否会影响 B 团队？如何设计 namespace 级的故障隔离？
...
- [[entities/prometheus|Prometheus (entities)]]
- [[entities/chaos-mesh|Chaos Mesh [entities]]]
- [[entities/litmus|LitmusChaos]]
