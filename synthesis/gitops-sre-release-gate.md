...
| 严重故障（P0） | Git revert + 强制同步 | 5-10min |
...
| **自动回滚 vs 人工判断** | 自动回滚在明确故障时有效，但某些 SLO 波动是"预期内"的（如依赖服务升级导致的临时延迟）。过度自动回滚会阻止正常发布。 |
| **指标延迟 vs 响应速度** | Prometheus 的指标通常有 1-5 分钟的 scrape 延迟。在故障快速扩散的场景（如级联失败），门控可能在检测到问题之前就已经造成广泛影响。 |
