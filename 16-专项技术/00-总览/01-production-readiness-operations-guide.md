...
本指南面向 SRE 与平台工程师，聚焦 专项技术 所覆盖的三大专项技术——边缘计算（KubeEdge/OpenYurt/SuperEdge）、WebAssembly（SpinKube/wasmCloud/WasmEdge）以及 Kubernetes 扩展机制（CRD/Operator/Webhook/Helm）——在生产环境中的就绪检查、风险缓解与日常运维。文档不重复基础概念，重点补充部署后运维、故障排查与跨域协作缺口。
...
- **集群基础**：Kubernetes 扩展机制（API Server、Aggregation Layer、调度器）是本域 CRD/Operator/Webhook 的底层依赖。生产问题若涉及 API Server 负载或 etcd 延迟，需联动控制平面团队。
- **网络**：边缘计算的 CNI、EdgeMesh、NAT 穿透、服务发现以及 WebAssembly 服务的 Ingress/Gateway 策略由网络域主导，本域关注云边通道与应用层暴露。
- **安全**：Webhook 证书生命周期、RBAC、Pod Security Standards、镜像签名与边缘节点安全加固需遵循安全域基线。
- **可观测性**：边缘带宽受限场景下的指标采样、日志缓存、SLO 定义与告警路由由可观测域提供框架，本域负责组件级指标埋点与端侧采集配置。
- **平台工程**：平台扩展、Helm Chart 治理、开发者门户与本域的 Operator 生态高度重叠，建议统一 Chart 仓库与发布流水线。
- **可靠性**：CloudCore HA/DR、边缘节点退役、PodDisruptionBudget、混沌演练与灾备恢复由可靠性域统筹。
- **生产运维**：事件响应、On-Call 流程、变更管理与 FinOps 是跨域通用要求，本域专项组件需接入统一运维体系。
- **容器运行时**：containerd、shim、镜像 GC 与 RuntimeClass 是 Wasm 与边缘容器运行的基础，版本兼容性需与运行时域对齐。
...
- [[16-专项技术/01-边缘计算/04-kubeedge-architecture-deployment.md|KubeEdge 架构与部署]]
- [[16-专项技术/02-WebAssembly/03-spinkube-framework.md|SpinKube 框架实践]]
- [[16-专项技术/03-扩展机制/03-admission-webhook-configuration.md|准入控制器 Webhook 配置与实现]]
- [[16-专项技术/03-扩展机制/02-operator-development-patterns.md|Operator 开发模式]]
- [[16-专项技术/03-扩展机制/06-helm-charts-management.md|Helm Chart 管理]]
...
- [[01-集群基础/02-设计原则/02-production-architecture-design-principles.md|生产架构设计原则]]
- [[05-网络/README.md|网络流量域]]
- [[08-安全/README.md|安全合规域]]
- [[09-可观测性/README.md|可观测性域]]
- [[10-平台工程/README.md|平台工程域]]
- [[12-可靠性/README.md|可靠性工程域]]
- [[13-生产运维/README.md|生产运维域]]
- [[14-容器运行时/03-containerd-CRI-O/01-containerd-deep-guide.md|containerd 深度指南]]
...
*本指南作为 专项技术 的生产就绪入口，建议结合具体组件文档与跨域运行手册共同使用。*
