...
- **资源共享**：Sidecar 与主容器共享网络和存储命名空间；Pod 的有效资源请求/限制为：[[domain-17-system-foundation/topic-dictionary/scheduling/pod-overhead.md|Pod Overhead]] + max(非 init 容器之和, 有效 init 容器值)。
