...
- **日志输出稳定性**：与命令行标志不同，日志输出的格式和内容**不属于 [[domain-17-system-foundation/topic-dictionary/fundamentals/the-kubernetes-api|Kubernetes API]] 稳定性保证范围**，不同版本之间日志条目和格式可能发生变化。
