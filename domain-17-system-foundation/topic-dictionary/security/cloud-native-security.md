...
- **运行时（Runtime）**：分为以下关键领域：
  - **访问（Access）**：保护 [[domain-17-system-foundation/topic-dictionary/fundamentals/the-kubernetes-api|Kubernetes API]]，实施有效的认证与授权，使用 ServiceAccount 管理工作负载身份，启用 TLS 保护 API 流量。
