...
Emissary-Ingress 以 Helm Chart 或 Operator 方式部署。Emissary Controller 作为 Deployment 运行，监听集群中的 Mapping/Host/Listener CRD。Controller 将 CRD 配置翻译为 Envoy 的 xDS 配置，通过 gRPC stream 推送给 Envoy 代理。Envoy 作为 Deployment 运行（多副本高可用），通过 Kubernetes Service 暴露（LoadBalancer 类型）。Emissary 支持 [[22-概念/01-核心架构/kubernetes-architecture-overview.md|Kubernetes 架构]] 中的标准 Ingress API 和 Gateway API，可以作为集群的唯一入口。
