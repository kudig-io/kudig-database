...
工作负载管理是 Kubernetes 原生核心能力，通过内置控制器和调度器实现，无需额外安装。通过 kubectl、Helm 或 GitOps 工具（ArgoCD/Flux）管理各类工作负载 CRD。HPA 可通过 `kubectl autoscale` 或 Prometheus Adapter 集成自定义指标。生产环境推荐配合 [[22-概念/01-核心架构/kubernetes-architecture-overview.md|Kubernetes 架构]] 中的 ResourceQuota、LimitRange 和 PodDisruptionBudget 使用。
...
- [[keda]] — KEDA
- [[10-平台工程/06-代码分析/deployment-create/08-hpa-integration.md|Deployment 与 HPA 集成源码分析]]
- [[23-实体/02-K8s核心组件/kube-scheduler.md|kube-scheduler]] — K8s 调度器
- [[22-概念/01-核心架构/controller-pattern.md|controller-pattern]] — 控制器模式
