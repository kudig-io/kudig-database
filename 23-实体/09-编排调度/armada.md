...
Armada 的 Executor 以 Deployment 运行在每个目标 Kubernetes 集群中。Executor 接收来自 Armada Server 的作业指令，将其转换为 Kubernetes Pod/Job 资源创建到本地集群。Executor 监控 Pod 状态并实时回传到 Server。用户通过 `armadactl` CLI 或 REST API 提交 YAML 格式的作业定义（类似 Kubernetes Pod spec）。Armada 支持 [[22-概念/01-核心架构/kubernetes-architecture-overview.md|Kubernetes 架构]] 中的全部 Pod 特性（resource requests、nodeSelector、tolerations 等），并与 JobSet API 集成支持复杂批处理工作流。
...
- armada
- karmada
- [[23-实体/11-AI与边缘/cohdi.md|Cohdi]]
- [[23-实体/09-编排调度/kubefleet.md|KubeFleet]]
- [[23-实体/09-编排调度/clusternet.md|Clusternet]]
- [[23-实体/09-编排调度/kured.md|Kured (KUbernetes REboot Daemon)]]
- [[23-实体/10-平台与开发工具/kubevela.md|KubeVela]]
- [[23-实体/09-编排调度/kubestellar.md|KubeStellar]]
- [[23-实体/10-平台与开发工具/microcks.md|Microcks]]
- [[23-实体/08-交付与制品/kudo.md|KUDO]]
