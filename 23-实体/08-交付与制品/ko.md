...
ko 与 Kubernetes 深度集成。`ko apply -f config/` 自动构建 YAML 中引用的 Go 应用镜像，替换 `ko://` 引用为实际镜像地址，然后通过标准 Kubernetes API apply 资源。`ko resolve -f config/` 只渲染不部署，输出可用于 ArgoCD/Flux 等 GitOps 工具。ko 还支持 `ko dev` 本地开发模式——本地编译 Go 应用，在远程集群中运行。与 [[22-概念/01-核心架构/kubernetes-architecture-overview.md|Kubernetes 架构]] 中的标准 Deployment/YAML 管理完全兼容。
