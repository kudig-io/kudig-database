...
# [[23-实体/06-安全/open-policy-containers.md|Open Policy Containers]] (OPCR)
...
OPCR 打包的策略镜像可以被 Kubernetes 中的 OPA 或 Gatekeeper 消费。Gatekeeper 支持从 OCI Registry 拉取策略镜像作为 ConstraintTemplate。OPCR 还可以与策略编排工具（如 OPA Operator）集成——Operator 监听 Registry 中的策略镜像更新，自动拉取新版本策略并加载到 OPA。通过 cosign 签名验证确保只有受信任的策略被部署。与 [[22-概念/01-核心架构/kubernetes-architecture-overview.md|Kubernetes 架构]] 中的准入控制（Admission Webhook）和供应链安全机制集成。
