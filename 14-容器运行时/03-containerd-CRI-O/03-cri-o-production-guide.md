...
sudo systemctl restart crio
```
在 Pod 中通过 `runtimeClassName: kata` 即可使用 Kata。详见 [[14-容器运行时/03-containerd-CRI-O/04-oci-runtimes-comparison.md|OCI 运行时对比]]。
...
- [[14-容器运行时/03-containerd-CRI-O/04-oci-runtimes-comparison.md|OCI 运行时对比]]
