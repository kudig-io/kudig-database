...
| 维度 | 示例 | 用途 |
|---|---|---|
| 技术域 | networking, storage, security | 技术领域分类 |
| 难度 | beginner, intermediate, advanced | 学习路径 |
| 场景 | production, troubleshooting | 应用场景 |
| 工具 | helm, gitops, operator | 工具链 |
...
1. **Q：如何设计知识库标签体系？**
   A：明确分类维度、控制标签数量、保持一致性、支持多维度检索。

2. **Q：标签 vs 目录的区别？**
   A：目录是层级结构，标签是扁平多维度。一个文件只能在一个目录，但可有多个标签。

3. **Q：标签在 RAG 检索中的作用？**
   A：提供元数据过滤、支持精确匹配、辅助语义理解、提升检索精度。
