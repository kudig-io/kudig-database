...
|---------|------|-------|--------|---------------|
| 故障排查大全 | `domain-12-troubleshooting/` | 42 | ~1.5M 字 | ★★★★★ 排障 Agent 核心语料 |
| 结构化排障 | `topic-structural-trouble-shooting/` | 48+ | ~600K 字 | ★★★★★ 已有决策树结构 |
| FTA 故障树 | `topic-fta/list/` | 37 | ~1.5M 字 | ★★★★★ Agent 推理链直接输入 |
| YAML 清单手册 | `domain-32-yaml-manifests/` | 36 | ~1.7M 字 | ★★★★☆ 模板生成 Agent 基础 |
| K8s Events 大全 | `domain-33-kubernetes-events/` | 15 | ~800K 字 | ★★★★☆ 事件解读 Agent 关键 |
| 运维词典 | `topic-dictionary/` | 16 | ~1.4M 字 | ★★★★☆ 术语和最佳实践 |
| FEBM 取证循证 | `topic-febm/` | 11 | ~1.0M 字 | ★★★★☆ 诊断方法论 |
| FTA 方法论 | `topic-fta/` (非 list) | 30 | ~500K 字 | ★★★★☆ Agent 编排理论 |
| 控制平面 | `domain-3-control-plane/` | 28 | ~1.1M 字 | ★★★★☆ 核心组件深度知识 |
| 网络 | `domain-5-networking/` | 41 | ~900K 字 | ★★★★☆ 网络排障 Agent |
| 安全 | `domain-7-security/` | 21 | ~440K 字 | ★★★☆☆ 安全审计 Agent |
| 可观测性 | `domain-8-observability/` | 27 | ~600K 字 | ★★★☆☆ 监控 Agent |
| 生产运维 | `domain-18-production-operations/` | 24 | ~500K 字 | ★★★☆☆ SOP 改造基础 |
| 速查卡 | `topic-cheat-sheet/` | 3 | ~130K 字 | ★★★☆☆ 可直接检索 |
| 迁移指南 | `topic-migration/` | 10 | ~150K 字 | ★★★☆☆ 迁移 Agent 蓝本 |
| 云厂商 | `domain-17-cloud-provider/` | 13目录 | ~200K 字 | ★★☆☆☆ ACK 较完整，其余薄弱 |
...
|---------|-------|----------------|----------|
| `domain-12-troubleshooting/` | 42 | 缺 intent_queries、severity、symptom_tags | 致命 - 排障场景检索准确率低 |
| `domain-32-yaml-manifests/` | 36 | 缺 resource_type、api_version、use_case | 严重 - YAML 生成 Agent 无法按资源类型过滤 |
| `domain-33-kubernetes-events/` | 15 | 缺 event_type、source_component、severity | 严重 - 事件解读 Agent 无法精确匹配 |
| `domain-3-control-plane/` | 28 | 缺 component、layer、failure_mode | 高 - 控制平面诊断检索耗时长 |
| `topic-fta/list/` | 37 | 缺 component、top_event、gate_count | 中 - FTA 已自带结构但缺查找入口 |
...
```yaml
# 以 domain-12/05-pod-pending-diagnosis.md 为例
---
id: D12-05
...
fta_ref: topic-fta/list/pod-fta.md  # 关联故障树
structural_ref: topic-structural-trouble-shooting/05-workloads/01-pod-troubleshooting.md
---
```
...
|---------|------|------------------|----------|
| `topic-dictionary/12-incident-management-runbooks.md` | 3245 行，事故管理框架+流程 | ★★★☆☆ 有框架但缺可执行步骤 | 中 - 需提取为结构化 SOP |
| `domain-18/23-incident-response-handling.md` | 应急响应处理 | ★★☆☆☆ 理论偏重 | 大 - 需重写为分钟级步骤 |
| `domain-18/22-change-management-process.md` | 变更管理流程（62K） | ★★★☆☆ 较完整但非 Agent 格式 | 中 |
| `domain-18/17-disaster-recovery-drills.md` | 灾备演练（45K） | ★★★☆☆ 有步骤但非结构化 | 中 |
| `domain-3/10-plane-backup-disaster-recovery.md` | etcd 备份恢复（58K） | ★★★★☆ 有具体命令 | 小 - 需提取整合 |
| `domain-7/10-certificate-management.md` | 证书管理（40K） | ★★★☆☆ 有原理和命令 | 中 - 需拆分为按组件的 SOP |
| `domain-1/07-upgrade-paths-strategy.md` | 升级策略 | ★★☆☆☆ 策略为主 | 大 - 缺分版本操作清单 |
| `topic-migration/` | 10 篇迁移指南 | ★★★☆☆ 流程清晰但非 SOP 格式 | 中 |

**缺失的 30 个核心 SOP 完整清单**：

| # | SOP 名称 | 触发场景 | 严重级 | 现有可复用内容 | 改造工作量 |
|---|---------|---------|--------|----------------|----------|
| 1 | etcd 紧急备份恢复 | etcd 不可用/数据损坏 | P0 | domain-3/10, domain-3/19 | 小 |
| 2 | API Server 不可用应急 | apiserver 无响应 | P0 | domain-12/01, domain-3/12 | 中 |
| 3 | 集群证书过期轮换 | 证书即将/已过期 | P0 | domain-7/10, domain-12/13 | 中 |
| 4 | 生产环境 Pod 大规模 CrashLoop | 核心服务崩溃 | P0 | domain-12/08 | 中 |
| 5 | Node NotReady 应急处置 | 多节点同时 NotReady | P0 | domain-12/06, domain-12/09 | 中 |
| 6 | 集群网络分区处理 | 跨节点网络不通 | P0 | domain-12/03, domain-12/25 | 大 |
| 7 | 日常巡检 SOP | 每日/每周巡检 | P1 | domain-8/13 | 大 |
| 8 | 节点上下线（drain/cordon） | 节点维护 | P1 | 零散 | 大 |
| 9 | 集群版本升级 | 计划升级 | P1 | domain-1/07, domain-1/18 | 大 |
| 10 | 应用滚动更新/回滚 | Deployment 更新失败 | P1 | domain-12/11, domain-4/02 | 中 |
| 11 | HPA/VPA 调整 | 扩缩容异常 | P1 | domain-12/17, domain-4/21 | 中 |
| 12 | DNS 故障应急 | 服务发现失败 | P1 | domain-12/26, domain-5/28 | 中 |
| 13 | PVC 存储故障处理 | PVC Pending/挂载失败 | P1 | domain-12/14, domain-6/09 | 中 |
| 14 | Ingress/Gateway 故障处理 | 外部访问异常 | P1 | domain-12/15, domain-5/19-26 | 中 |
| 15 | RBAC 权限问题处理 | 权限不足/过大 | P1 | domain-12/12, domain-7/07 | 中 |
| 16 | Secret/ConfigMap 热更新 | 配置变更不生效 | P2 | domain-12/19 | 小 |
| 17 | CronJob 故障处理 | 定时任务不触发 | P2 | domain-12/18 | 小 |
| 18 | DaemonSet 故障处理 | 系统组件异常 | P2 | domain-12/20 | 小 |
| 19 | StatefulSet 故障处理 | 有状态服务异常 | P2 | domain-12/21 | 小 |
| 20 | 监控告警系统故障 | Prometheus/Alertmanager 异常 | P2 | domain-12/30 | 中 |
| 21 | Helm Release 故障处理 | Chart 部署/升级失败 | P2 | domain-12/36 | 中 |
| 22 | ArgoCD 同步故障处理 | GitOps 同步失败 | P2 | domain-12/38 | 中 |
| 23 | 镜像仓库故障处理 | 镜像拉取失败 | P2 | domain-12/27 | 中 |
| 24 | 集群自动扩缩容故障 | Cluster Autoscaler 异常 | P2 | domain-12/28 | 中 |
| 25 | NetworkPolicy 故障处理 | 网络策略异常 | P2 | domain-12/16 | 小 |
| 26 | GPU 设备故障处理 | GPU 不可见/分配失败 | P2 | domain-11/03-04 | 大 |
| 27 | Velero 备份恢复 SOP | 集群级备份恢复 | P2 | domain-12/31 | 中 |
| 28 | 多集群故障转移 | 跨集群切换 | P3 | domain-12/37 | 大 |
| 29 | 混沌工程演练 SOP | 故障注入测试 | P3 | domain-12/42 | 大 |
| 30 | 安全事件应急处置 | 安全漏洞/入侵 | P3 | domain-7/20 | 大 |
...
sop:
  ...
  knowledge_refs:
    - domain-3/10-plane-backup-disaster-recovery.md
    - domain-3/19-etcd-operations.md
    - domain-12/02-control-plane-etcd-troubleshooting.md
```
...
|---------|-----------|-----------|---------------|
| `topic-fta/list/` 37 个故障树 | ★★★★★ 树状结构 | ~200+ | ✗ 需拕平为查找表 |
| `topic-structural-trouble-shooting/` 48+ 篇 | ★★★★☆ 决策树 | ~150+ | ✗ 需提取为映射表 |
| `domain-12/` 42 篇排障文档 | ★★★☆☆ 长文叙述 | ~300+ | ✗ 需结构化提取 |
| `domain-33/` 15 篇事件文档 | ★★★☆☆ 按事件分类 | ~100+ | ✗ 需映射到症状 |
| `domain-12/08` Pod 状态速查表 | ★★★★☆ 有表格 | ~10 | △ 接近但缺诊断命令 |
  ...
  - symptom: "Pod 状态 CrashLoopBackOff"
    ...
    possible_causes:
      - cause: "应用启动失败"
        ...
        knowledge_ref: "domain-12/08-pod-comprehensive-troubleshooting.md#crashloopbackoff"
      - cause: "OOMKilled"
        probability: medium
        diagnosis:
          commands:
        ...
        knowledge_ref: "domain-12/07-oom-memory-diagnosis.md"
        fta_ref: "topic-fta/list/pod-fta.md#oomkilled"
      - cause: "Liveness Probe 失败"
        ...
        knowledge_ref: "domain-33/04-probe-health-check-events.md"
      - cause: "配置错误（ConfigMap/Secret 缺失）"
        ...
        knowledge_ref: "domain-12/19-configmap-secret-troubleshooting.md"
  ...
  - symptom: "Pod 状态 Pending"
    ...
    possible_causes:
      - cause: "集群资源不足（CPU/内存）"
        ...
        knowledge_ref: "domain-12/05-pod-pending-diagnosis.md#3"
      - cause: "节点污点/亲和性不匹配"
        probability: high
        diagnosis:
          commands:
        ...
        knowledge_ref: "domain-12/05-pod-pending-diagnosis.md#4"
      - cause: "PVC 未绑定"
        probability: medium
        diagnosis:
          commands:
        ...
        knowledge_ref: "domain-12/14-pvc-storage-troubleshooting.md"
  ...
  - symptom: "Service 无法访问"
    ...
    possible_causes:
      - cause: "Endpoint 为空"
        ...
        knowledge_ref: "domain-12/10-service-comprehensive-troubleshooting.md"
      - cause: "Selector 不匹配"
        probability: high
        diagnosis:
          commands:
      ...
      - cause: "NetworkPolicy 阻断"
        ...
        knowledge_ref: "domain-12/16-networkpolicy-troubleshooting.md"
```
...
| 症状类别 | 具体症状 | 数量 | 来源文档 |
|---------|---------|------|----------|
| **Pod 异常状态** | CrashLoopBackOff, Pending, ImagePullBackOff, OOMKilled, Evicted, ContainerCreating, Init:Error, Terminating, Unknown, RunContainerError, CreateContainerConfigError, PreemptionByScheduler, BackOff, ErrImageNeverPull, InvalidImageName | 15 | domain-12/05,07,08 |
| **Node 异常** | NotReady, MemoryPressure, DiskPressure, PIDPressure, NetworkUnavailable, CordonedUnexpectedly, KubeletDown, ContainerRuntimeDown, ClockSkew, KernelDeadlock | 10 | domain-12/06,09,35 |
| **Service/网络** | ServiceUnavailable, EmptyEndpoints, DNSResolutionFailed, IngressRouteError, TLSHandshakeFailed, ConnectionRefused, ConnectionTimeout, LoadBalancerPending, ExternalIPNotAssigned, GatewayRouteNotMatched, CrossNodeNetworkFailure, MTLSCertificateError | 12 | domain-12/10,15,16,25,26 |
| **存储异常** | PVCPending, VolumeAttachFailed, VolumeMountFailed, CSIDriverError, StorageClassNotFound, VolumeResizeFailed, SnapshotFailed, DataCorruption | 8 | domain-12/04,14 |
| **控制平面** | APIServerUnresponsive, APIServer5xx, etcdLeaderLost, etcdHighLatency, SchedulerBacklog, ControllerManagerStuck, WebhookTimeout, APFThrottling, etcdDiskSpaceLow, CertificateExpired | 10 | domain-12/01,02 |
| **调度异常** | Unschedulable, PreemptionFailure, PriorityClassMissing, TopologySpreadViolation, ResourceQuotaExceeded, LimitRangeViolation | 6 | domain-12/05,17,24 |
| **安全/权限** | Forbidden, Unauthorized, CertificateExpired, PodSecurityViolation, NetworkPolicyDrop, AuditPolicyError, SecretNotFound, ServiceAccountTokenExpired | 8 | domain-12/12,13,32 |
| **运维工具** | HelmReleaseFailed, ArgoCDOutOfSync, VeleroBackupFailed, ClusterAutoscalerNoScale | 4 | domain-12/28,36,38 |
| **合计** | | **73** | |

**工作量估算**：
- 从 FTA 故障树自动提取映射（50%）：~3 天
- 从 domain-12 文档人工提取补充：~5 天
- 从 domain-33 事件文档关联映射：~2 天
...
| `kubectl get events` | domain-33 有事件分类 | 异常模式识别、时间序列分析 | **中** | 🔴 最高 |
| `kubectl top node/pod` | 无 | 性能判断阈值（CPU >80%、Memory >85%） | **大** | 🟡 高 |
| `etcdctl endpoint status` | domain-3/19 有部分 | 健康判断标准、Raft 状态解读 | **中** | 🟡 高 |
| `etcdctl endpoint health` | domain-3/19 有 | 异常模式识别 | **小** | 🟡 高 |
| `crictl ps/inspect` | domain-3/21 有部分 | 运行时状态解读 | **中** | 🟢 中 |
| `helm status/list` | domain-12/36 有部分 | Release 状态解读、异常模式 | **中** | 🟢 中 |
| `argocd app get` | domain-12/38 有部分 | 同步状态解读、Health/Sync 判断 | **中** | 🟢 中 |
| Prometheus 告警规则 | domain-8 有规则但无解读 | 告警含义映射、严重级判断 | **大** | 🟢 中 |
...
|---------|-------|---------|----------|
| `topic-migration/10-real-world-case-study.md` | 1 个完整迁移案例 | ★★★★☆ | 中 - 仅覆盖迁移场景 |
| `domain-12/` 各篇文档中散落的案例 | ~20 个片段 | ★★☆☆☆ | 低 - 非结构化 |
| `topic-dictionary/02-failure-patterns-analysis.md` | ~15 个模式 | ★★★☆☆ | 中 - 模式而非工单 |
| `domain-8/22-best-practices-case-studies.md` | ~5 个案例 | ★★★☆☆ | 中 - 偏监控场景 |
...
incident:
  ...
  knowledge_refs:
    - domain-12/07-oom-memory-diagnosis.md
    - domain-12/08-pod-comprehensive-troubleshooting.md#oomkilled
...
| 场景类别 | 需要的工单数 | 典型场景 | 来源 |
|---------|-----------|---------|------|
| Pod 异常 | 15 | CrashLoop/OOM/Pending/Evicted/ImagePull | domain-12/05,07,08 |
| 网络故障 | 10 | DNS失败/Service不通/Ingress异常/跨节点不通 | domain-12/03,10,15,25,26 |
| 存储故障 | 8 | PVC Pending/挂载失败/性能下降 | domain-12/04,14 |
| 控制平面 | 8 | apiserver 慢/etcd 不健康/证书过期 | domain-12/01,02,13 |
| 安全权限 | 5 | RBAC 拒绝/ServiceAccount 失效 | domain-12/12,32 |
| 扩缩容 | 5 | HPA 不触发/CA 不扩容 | domain-12/17,28 |
| 升级迁移 | 5 | 版本不兼容/滚动升级卡住 | domain-12/34 |
| 工具链 | 5 | Helm 失败/ArgoCD 不同步 | domain-12/36,38 |
| 性能问题 | 5 | CPU 瓶颈/IO 延迟/网络延迟 | domain-12/33 |
| **合计** | **66** | | |

**工作量估算**：~20 人天（66 个工单 × 每个 0.3 天，可从 domain-12 现有内容提取结构化）
...
| 文档 | 版本相关内容 | Agent 可用度 |
|------|-----------|----------|
| `domain-1/03-api-versions-features.md` | API 版本演进 | ★★★☆☆ 有但不够精确 |
| `domain-1/07-upgrade-paths-strategy.md` | 升级策略 | ★★☆☆☆ 偏策略 |
| `domain-12/05` 第 13 节 | Pod Pending 版本特定变更 | ★★★☆☆ 局部覆盖 |
| `domain-32/` | YAML 示例中的 apiVersion | ★★☆☆☆ 隐含但未整理 |
  ...
  - feature: "Gateway API"
    changes:
      ...
      - version: "1.29"
        ...
        breaking: true
        note: "v1beta1 部分字段废弃"
        ref: "domain-5/35-gateway-api-overview.md"
  ...
  - feature: "Sidecar Containers"
    changes:
      ...
      - version: "1.29"
        ...
        ref: "domain-4/14-sidecar-containers-patterns.md"
```
...
| 类别 | 估计条目数 | 优先级 | 现有可复用 |
|------|-----------|--------|----------|
| API 废弃/移除时间线 | ~15 | 🔴 高 | domain-1/03 可提取 |
| 默认行为变更 | ~20 | 🔴 高 | 零散 |
| Feature Gate 状态 | ~15 | 🟡 中 | K8s 官方文档 |
| 组件参数变更 | ~10 | 🟡 中 | domain-3 可提取 |
| **合计** | **~60** | | |
...
| 现有文档 | 决策内容 | 缺失 |
|---------|---------|------|
| `domain-5/03-cni-plugins-comparison.md` | CNI 插件对比 | 缺量化选择条件 |
| `domain-6/04-storageclass-dynamic-provisioning.md` | 存储类选择 | 缺性能/成本判断指标 |
| `domain-4/22-cluster-capacity-planning.md` | 容量规划 | 缺自动化计算公式 |
| `domain-1/12-cluster-deployment-patterns.md` | 部署模式 | 缺场景→模式映射 |
  ...
  - id: DR-001
    ...
    conditions:
      - if: "节点数 > 500 AND 需要 NetworkPolicy"
        ...
        ref: "domain-5/03-cni-plugins-comparison.md"
      - if: "阿里云 ACK 环境"
        then: "Terway"
        reason: "原生 ENI 集成，性能最优，支持 VPC 原生网络"
        ref: "domain-5/05-terway-advanced-guide.md"
  ...
  - id: DR-002
    ...
    conditions:
      - if: "Java 应用"
        ...
        ref: "domain-12/07-oom-memory-diagnosis.md"
      - if: "Go 应用"
  ...
  - id: DR-003
    ...
    conditions:
      ...
      - if: "有状态服务数据"
        ...
        ref: "domain-9/12-backup-recovery-strategy.md"
```
...
| 决策场景 | 估计规则数 | 来源 |
|---------|-----------|------|
| CNI/网络插件选择 | 5 | domain-5/03 |
| 存储类/CSI 驱动选择 | 5 | domain-6/04-05 |
| 资源 requests/limits 设置 | 5 | domain-4/23, domain-12/07 |
| 集群规模/架构选型 | 5 | domain-1/12 |
| 监控方案选择 | 3 | domain-8/01 |
| Ingress/Gateway 选型 | 5 | domain-5/19,35 |
| 备份策略选择 | 3 | domain-9/12 |
| 安全策略选型 | 5 | domain-7 |
| 扩缩容策略选择 | 3 | domain-4/21 |
| **合计** | **~39** | |
...
**现状审计**：`domain-7-security/` 和 `domain-25-cloud-native-security/` 有安全知识，但缺少 **Agent 执行前的安全检查规则集**。这是 Agent 进入生产环境的**必要条件**。
...
**现状**：`domain-17-cloud-provider/` 中阿里云 ACK 有 8 个文件覆盖较好，但 AWS EKS、GKE、AKS 各只有 1 个文件。
...
└── 提取症状→原因映射表 (73 条)                    ~15 人天
    ├── 从 FTA 故障树自动提取                         ~3 天
    ├── 从 domain-12 人工提取补充                      ~5 天
    ├── 从 domain-33 事件文档关联                       ~2 天
    └── 编写诊断命令和 indicators                      ~5 天
...
| 现有资产 | 文件数 | 可复用于 | 改造工作量 |
|---------|-------|---------|-----------|
| `domain-12-troubleshooting/` | 42 | 症状→原因映射表、SOP | 中 - 需结构化提取 |
| `topic-structural-trouble-shooting/` | 48+ | 决策树、症状映射 | 低 - 已半结构化 |
| `topic-fta/list/` | 37 | 故障树推理链 | 低 - 已结构化 |
| `domain-32-yaml-manifests/` | 36 | YAML 生成模板 | 低 - 已标准化 |
| `domain-33-kubernetes-events/` | 15 | 事件解读语料 | 中 - 需提取映射 |
| `topic-dictionary/` | 16 | 术语表、最佳实践 | 中 - 需分块 |
| `topic-cheat-sheet/` | 3 | 命令速查 | 低 - 可直接用 |
| `topic-migration/` | 10 | 迁移 SOP | 中 - 需 SOP 化 |
| `topic-febm/` | 11 | 诊断方法论 | 中 - 需模板化 |
...
| 文档 | 说明 |
|------|------|
| [Agent 设计思路与落地路径](./agent-design.md) | Agent 赋能的整体设计思路 |
...
*本文档为 kudig-database 项目 topic-agent 专题的差距分析报告。*
