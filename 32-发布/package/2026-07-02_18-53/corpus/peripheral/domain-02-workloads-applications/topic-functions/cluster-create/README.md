---
title: Cluster Create — Kubernetes 集群新建源码分析
description: '| API 类型 | `cmd/kubeadm/app/apis/kubeadm/` | 配置类型定义 |'
summary: '| API 类型 | `cmd/kubeadm/app/apis/kubeadm/` | 配置类型定义 |'
category: functions
tags:
- k8s
- operations
...
# Cluster Create — [[Kubernetes|Kubernetes]] 集群新建源码分析
...
| [[etcd|etcd]] 阶段 | `cmd/kubeadm/app/phases/etcd/` | etcd 初始化 |
| 附加组件 | `cmd/kubeadm/app/phases/addons/` | [[CoreDNS|CoreDNS]]/kube-proxy |
...
| 4 | `kubelet-start` | 启动 [[kubelet|kubelet]] 服务 |
...
| CNI 未安装 | CoreDNS Pending | 缺少 CNI 插件 | 安装 Calico/Cilium |
| etcd 启动失败 | API Server 无法连接 etcd | etcd 数据目录权限 | 检查 `/var/lib/etcd` 权限 |

## 相关函数

- [`CreatePKIAssets`](32-发布/package/2026-07-02_18-53/corpus/peripheral/domain-07-platform-engineering/topic-code-analysis/cluster-cert/01-ca-generation.md) — PKI 证书生成
