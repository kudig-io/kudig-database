...
`kubectl` 是与 [[Kubernetes|Kubernetes]] 集群的控制平面进行通信的主要命令行工具。它通过 [[domain-17-system-foundation/知识字典/fundamentals/the-kubernetes-api.md|Kubernetes API]] 发送请求，是用户管理集群资源、检查集群状态和调试应用的主要接口。
...
执行命令时，`kubectl` 将用户意图转换为一个或多个发往 Kubernetes API 的 HTTP 请求。API 服务器验证每个请求，将其应用到存储在 [[domain-17-system-foundation/知识字典/fundamentals/etcd.md|etcd]] 中的集群状态，并返回结果。
