...
> 本指南与 [[32-发布/package/2026-07-02_18-53/corpus/peripheral/domain-17-system-foundation/01-linux/08-linux-operations-basics|Linux 运维基础与应急响应]]、[[32-发布/package/2026-07-02_18-53/corpus/peripheral/domain-17-system-foundation/02-hardware/15-kubernetes-hardware-troubleshooting|K8s 硬件故障排查]] 互补，避免重复罗列通用命令，重点补充生产就绪视角的落地动作。
...
1. **OS 镜像与加固基线已固化**  
   生产环境不应依赖人工逐台配置节点。使用 CIS Benchmark、OpenSCAP 或 Lynis 生成合规报告，将加固动作写入镜像构建脚本或 Ansible Playbook；禁用 cups、bluetooth、firewalld 等非必要服务，SSH 仅启用密钥认证并关闭 Root 直接登录。参考 [[32-发布/package/2026-07-02_18-53/corpus/peripheral/domain-17-system-foundation/01-linux/06-linux-security-hardening|Linux 安全加固]]。
...
6. **节点资源压力阈值已合理设置**  
   根据实际磁盘与内存规格配置 `evictionHard`、`evictionSoft`、`imageGCHighThresholdPercent`、`imageGCLowThresholdPercent`，并在低峰期人为触发验证。阈值设置过晚会导致服务降级，设置过早则造成资源浪费。参考 [[32-发布/package/2026-07-02_18-53/corpus/peripheral/domain-17-system-foundation/03-kubernetes-events/05-node-lifecycle-condition-events|节点生命周期与状态事件]]。
...
8. **硬件健康监控与告警已接入**  
   部署 Node Problem Detector（NPD）并配置 Prometheus 规则覆盖 SMART/NVMe 寿命、ECC 内存错误、CPU MCE、温度、IPMI SEL。对可纠正错误设置增长阈值告警，对不可纠正错误立即触发 P0 隔离。参考 [[32-发布/package/2026-07-02_18-53/corpus/peripheral/domain-17-system-foundation/02-hardware/15-kubernetes-hardware-troubleshooting|K8s 硬件故障排查]]。
...
- **domain-01-cluster-fundamentals**：控制平面组件（API Server、etcd、kubelet）的安装与升级依赖本域的 OS/内核/时间同步基线；当问题上升到组件级崩溃或证书异常时，移交给集群基础域。
- **domain-13-container-runtime**：containerd/CRI-O 的安装、镜像仓库配置、运行时版本升级由运行时域主导；本域负责 systemd 服务依赖、内核参数、节点资源清理等“承载层”配合。
- **domain-03-networking-traffic**：CNI 插件依赖 `br_netfilter`、`vxlan` 等内核模块以及网络 sysctl；网络策略、Service 拓扑、Ingress 问题在网络域闭环，本域提供节点网络环境排查。
- **domain-05-security-compliance**：节点加固、SSH/sudo 审计、Pod Security Standards / OPA Gatekeeper 由安全域定义策略；本域负责落地到 OS 镜像与 kubelet 配置。
- **domain-06-observability**：Node Exporter、NPD、journald、事件监控由可观测域统一采集；本域输出需要被监控的节点级关键指标与事件。
- **domain-10-troubleshooting-diagnostics**：当出现复杂 Node NotReady、硬件故障树或跨域根因时，使用 [[domain-10-troubleshooting-diagnostics/README.md|故障排查诊断]] 域的 FTA 与决策树。
- **domain-11-production-operations**：值班响应、变更管理、SLO 定义由生产运维域统一调度；本域提供变更窗口内的节点维护 SOP。
...
- [[32-发布/package/2026-07-02_18-53/corpus/peripheral/domain-17-system-foundation/01-linux/08-linux-operations-basics|Linux 运维基础与应急响应]]
- [[32-发布/package/2026-07-02_18-53/corpus/peripheral/domain-17-system-foundation/01-linux/05-linux-performance-tuning|Linux 性能调优]]
- [[32-发布/package/2026-07-02_18-53/corpus/peripheral/domain-17-system-foundation/01-linux/06-linux-security-hardening|Linux 安全加固]]
- [[32-发布/package/2026-07-02_18-53/corpus/peripheral/domain-17-system-foundation/02-hardware/15-kubernetes-hardware-troubleshooting|K8s 硬件故障排查]]
- [[32-发布/package/2026-07-02_18-53/corpus/peripheral/domain-17-system-foundation/03-kubernetes-events/05-node-lifecycle-condition-events|节点生命周期与状态事件]]
...
- [[domain-01-cluster-fundamentals/README.md|集群基础组件]]
- [[domain-13-container-runtime/README.md|容器运行时]]
- [[domain-05-security-compliance/README.md|安全合规]]
- [[domain-10-troubleshooting-diagnostics/README.md|故障排查诊断]]
- [[domain-11-production-operations/README.md|生产运维]]

---

*本文件基于 domain-17-system-foundation 当前内容缺口分析编写，重点补齐时间同步、systemd 服务管理、内核实时补丁、OS 加固基线与硬件 NUMA/CPU 拓扑感知等生产就绪动作。*
