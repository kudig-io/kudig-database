#!/usr/bin/env python3
"""
导出 llm-wiki 语料到 release/package/<DATE_TIME>/，供 AgentScope Agent 挂载 NAS 使用。
语料筛选严格由 _meta/corpus-config/profiles/<profile>.yaml 的 include/exclude 决定。

Release 目录布局（NAS 挂载根）：
  release/
  ├── scripts/                       (输出脚本与评估脚本，与 package 平级)
  │   ├── export_corpus_for_nas.py   (本脚本)
  │   └── evaluate_corpus_completeness.py
  └── package/                       (按时间戳归档的历史导出)
      └── 2026-07-02_18-29/          (YYYY-MM-DD_HH-MM)
          ...
          │   └── taxonomy/          (分类体系)
          └── profiles/              (可选：同次导出的其他 profile 子集)
              └── sre/               (rag-sre-profile.yaml 产出)

默认行为（不指定 -o）：写入 release/package/<DATE_TIME>/，时间戳由脚本生成。
指定 -o：直接写入指定目录（跳过自动时间戳）。

用法:
  python scripts/export_corpus_for_nas.py                             # 默认 profile → release/package/<TS>/
  python scripts/export_corpus_for_nas.py -p rag-sre-profile.yaml     # 切换 profile
  python scripts/export_corpus_for_nas.py -o /tmp/export              # 自定义输出目录（跳过时间戳）
"""
...
def export_corpus(vault: Path, output_dir: Path, profile: dict, profile_name: str):
    # Safety: refuse to clean paths that would wipe sibling content.
    # -o release would nuke release/scripts + release/package; vault root is catastrophic.
    protected = {
        str(vault.resolve()),
        str((vault / 'release').resolve()),
        str((vault / 'release' / 'scripts').resolve()),
        str((vault / 'release' / 'package').resolve()),
    }
    resolved = str(output_dir.resolve())
    if resolved in protected:
        raise ValueError(
            f"Refusing to clean protected path: {output_dir}\n"
            f"Use a subdirectory (e.g. release/package/<DATE_TIME>/) or omit -o to use the default."
        )
...
def main():
    ...
    parser.add_argument('--output', '-o', type=str, default=None,
                        help='Output directory for exported corpus. '
                             'Default: release/package/<YYYY-MM-DD_HH-MM>/; '
                             'explicit -o writes to the given path directly.')
    ...
    if args.output is None:
        ...
        output_dir = vault / 'release' / 'package' / ts
        print(f"Default output → {output_dir.relative_to(vault) if str(output_dir).startswith(str(vault)) else output_dir}")
    else:
