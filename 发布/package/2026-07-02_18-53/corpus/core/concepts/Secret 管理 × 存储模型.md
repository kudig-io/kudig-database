...
[[domain-17-system-foundation/知识字典/configuration/secrets.md|secrets]]-management]] 覆盖密钥的安全存储，[[concepts/storage-model.md|storage model]] 覆盖 PV/PVC/StorageClass 的三层抽象。两者的交叉点是 **Secret 的物理存储路径**：[[domain-17-system-foundation/速查卡/k8s.md|K8s]] Secret 从 API Server 写入 [[entities/etcd.md|etcd]]，从 etcd 同步到 [[entities/kubelet.md|kubelet]]，从 kubelet 挂载为 tmpfs 到容器。这条路径上的每个存储层都有不同的安全特性和失效模式。
...
- **跨节点 Secret 同步的一致性**：当 Secret 更新后，各节点 kubelet 的同步时间是异步的。在滚动更新期间，同一 [[entities/deployment.md|Deployment]] 的不同 Pod 可能使用不同版本的 Secret。如何确保 Secret 更新的一致性？
