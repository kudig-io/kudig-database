...
- **默认存储状态**：默认情况下，Secret 以未加密形式存储在 API Server 的后端数据存储（[[domain-17-system-foundation/知识字典/fundamentals/etcd.md|etcd]]）中。任何拥有 API 访问权限或 etcd 访问权限的人都可以读取或修改 Secret。
