...
## 概述
Pod Group Policies 是 [[domain-17-system-foundation/知识字典/workloads/workload-api.md|Workload API]] 的组成部分（Alpha，v1.35 默认禁用）。Workload 中定义的每个 Pod 组都必须声明一个调度策略，该策略决定调度器如何处理该组 Pod 的集合。
...
2. **gang（集体调度策略）**
   - 强制“全有或全无”调度（[[domain-17-system-foundation/知识字典/scheduling/gang-scheduling.md|gang scheduling]]）。
