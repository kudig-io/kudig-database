...
|:---|:---|:---|:---|:---|
| 节点 NotReady / 节点异常 | domain-10-troubleshooting-diagnostics | k8s-node-notready | node-fta | 优先检查 kubelet/containerd |
| Pod 调度失败 / Pending | domain-10-troubleshooting-diagnostics | k8s-pod-pending | pod-fta | 资源、污点、亲和性 |
| 节点资源不足 / CPU 高 / 内存满 | domain-09-reliability-engineering | k8s-capacity-planning | resource-fta | 扩容或优化 |
| 证书过期 / apiserver 证书 | domain-05-security-compliance | k8s-certificate-rotation | cert-fta | 需区分 kubelet/apiserver |

### 3.2 工作负载与应用

| 意图关键词 | Domain | Skill | FTA | 备注 |
|:---|:---|:---|:---|:---|
| Pod CrashLoopBackOff / 重启 | domain-10-troubleshooting-diagnostics | k8s-pod-crashloop | pod-fta | 退出码 + 日志 |
| OOMKilled / 内存溢出 | domain-07-platform-engineering | k8s-memory-optimization | oom-fta | limits/request 调优 |
| Java 应用异常 / GC 问题 | domain-02-workloads-applications | java-jvm-tuning | jvm-fta | JVM 参数、堆dump |
| Spring Boot 启动失败 | domain-02-workloads-applications | java-springboot-k8s | app-fta | 配置、探针 |

### 3.3 网络与流量

| 意图关键词 | Domain | Skill | FTA | 备注 |
|:---|:---|:---|:---|:---|
| Service 不通 / DNS 解析失败 | domain-03-networking-traffic | k8s-service-unreachable | network-fta | CoreDNS、SLB、安全组 |
| Ingress 不通 / 证书错误 | domain-03-networking-traffic | k8s-ingress-troubleshoot | ingress-fta | ALB/NLB、Secret |
| Pod 网络不通 / 跨节点 | domain-03-networking-traffic | k8s-cni-troubleshoot | cni-fta | Terway/Calico/Cilium |
| 服务延迟高 / 超时 | domain-03-networking-traffic | k8s-network-latency | latency-fta | 链路与延迟 |

### 3.4 存储

| 意图关键词 | Domain | Skill | FTA | 备注 |
|:---|:---|:---|:---|:---|
| PVC 绑定失败 / Pending | domain-04-storage-data | k8s-pvc-pending | pvc-fta | StorageClass、配额 |
| PV 挂载失败 / MountVolume 错误 | domain-04-storage-data | k8s-pv-mount-fail | mount-fta | CSI、节点路径 |
| 存储扩容失败 | domain-04-storage-data | k8s-volume-expansion | expansion-fta | SC allowVolumeExpansion |
| 备份失败 / Velero 错误 | domain-04-storage-data | velero-backup-recovery | backup-fta | OSS、快照 |
| Rook-Ceph / Longhorn 异常 | domain-04-storage-data | rook-ceph-production / longhorn-production | storage-fta | 分布式存储 |

### 3.5 安全与合规

| 意图关键词 | Domain | Skill | FTA | 备注 |
|:---|:---|:---|:---|:---|
| RBAC 权限不足 / 拒绝访问 | domain-05-security-compliance | k8s-rbac-troubleshoot | rbac-fta | Role/ClusterRole |
| 镜像安全 / 漏洞 | domain-05-security-compliance | k8s-image-security | image-fta | ACR 扫描 |
| 网络策略拦截 | domain-05-security-compliance | k8s-network-policy | netpol-fta | NetworkPolicy |
| Pod 安全策略拒绝 | domain-05-security-compliance | k8s-pod-security | pss-fta | PSS/PSP |

### 3.6 发布与变更

| 意图关键词 | Domain | Skill | FTA | 备注 |
|:---|:---|:---|:---|:---|
| Helm 发布失败 / 回滚 | domain-08-release-change-management | helm-production-guide | helm-fta | values、依赖 |
| ArgoCD 同步失败 | domain-08-release-change-management | argocd-troubleshoot | gitops-fta | Git、权限 |
| GitLab CI / 镜像构建失败 | domain-08-release-change-management | cicd-troubleshoot | cicd-fta | Runner、Dockerfile |

### 3.7 可观测性

| 意图关键词 | Domain | Skill | FTA | 备注 |
|:---|:---|:---|:---|:---|
| 监控缺失 / Prometheus 异常 | domain-06-observability | prometheus-troubleshoot | prom-fta | Target、规则 |
| 日志采集失败 / SLS | domain-06-observability | logging-troubleshoot | logging-fta | Fluent Bit/Logtail |
| 告警风暴 | domain-06-observability | alert-management | alert-fta | 降噪、分组 |
...
- 优先级：P1
- Domain：domain-03-networking-traffic
