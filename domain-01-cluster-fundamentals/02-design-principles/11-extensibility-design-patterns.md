...
<!-- chunk: 资深视点：CEL (Common Expression Language) 的革命 -->
...
<!-- chunk: 二、CRD (Custom Resource Definition) 设计模式 -->
...
> **eBPF (Extended Berkeley Packet Filter)** 是 Kubernetes 扩展性中增长最快的内核级技术，允许在不修改内核源码或加载内核模块的情况下，安全地在内核态运行沙箱程序。

#### eBPF 在 K8s 中的扩展点

| 扩展点 | 说明 | 典型实现 |
|--------|------|----------|
| 网络数据面 | 替代 iptables/kube-proxy，实现 O(1) 服务路由 | Cilium CNI |
| 网络安全策略 | L3-L7 精细化网络策略执行 | CiliumNetworkPolicy |
| 可观测性 | 内核级进程/网络/文件 I/O 追踪，极低开销 | Pixie, Parca |
| 运行时安全 | 系统调用审计，容器逃逸检测 | Falco (eBPF driver), Tetragon |
| 混沌工程 | 内核态注入 I/O 延迟、DNS 错误、网络丢包 | Chaos Mesh (eBPF mode) |

#### eBPF 扩展架构

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                        eBPF 扩展架构在 Kubernetes 中的位置                    │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  用户态 (User Space)                                                         │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
...
│  │  • 将 CRD 配置编译为 eBPF 程序                                     │    │
│  │  • 通过 bpf() 系统调用加载到内核                                   │    │
│  │  • 通过 BPF Maps 与内核态交换数据                                  │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                              │ bpf() syscall + BPF Maps                      │
│  内核态 (Kernel Space)                                                       │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  eBPF 虚拟机 (Verifier 保证安全)                                   │    │
│  │  ├── XDP (eXpress Data Path) → 网卡入口极早处理                    │    │
│  │  ├── tc (Traffic Control) → 网络流量控制                            │    │
│  │  ├── cgroup/connect → 连接级别策略                                  │    │
│  │  ├── kprobes/kretprobes → 内核函数追踪                             │    │
│  │  ├── tracepoints → 静态追踪点                                      │    │
│  │  └── perf_events → 性能事件采样                                    │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
...
| 程序类型 | 挂载点 | K8s 扩展用途 |
|----------|--------|-------------|
| XDP | 网卡驱动层 | DDoS 防护、负载均衡前置过滤 |
| tc/cls_act | 网络接口队列 | 服务路由、网络策略、流量整形 |
| cgroup/connect4 | cgroup 连接 | 服务网格 Sidecar-less 模式 (zTunnel) |
| cgroup/sock_ops | Socket 操作 | 连接追踪、mTLS 加速 |
| tracepoint/sched | 调度事件 | CPU 性能剖析 (Parca) |
| kprobe/execve | 进程执行 | 运行时安全审计 (Tetragon) |

#### eBPF 扩展的优势与限制

| 维度 | 优势 | 限制 |
|------|------|------|
| 性能 | 内核态执行，无用户态切换开销 | Verifier 限制程序复杂度 |
| 安全 | 沙箱执行，不会 crash 内核 | 需要较新内核版本 (>= 5.4+) |
| 透明性 | 对应用完全无侵入 | 调试困难，需要 bpf工具链 |
| 灵活性 | 动态加载/卸载，无需重启 | 不同内核版本兼容性问题 |
| 生态 | Cilium/Falco/Pixie 等成熟项目 | 学习曲线陡峭 |

### 4.4 WebAssembly (WASM) 插件架构

> **WebAssembly** 为 Kubernetes 扩展提供了一种安全、跨语言、低延迟的插件机制。与 eBPF 互补：eBPF 运行在内核态，WASM 运行在用户态沙箱中。

#### WASM 在 K8s 中的扩展点

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    WASM 插件在 Kubernetes 中的扩展点                          │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  API Server 扩展                                                             │
│  ┌──────────────────────────────────────────────────────────────────┐       │
│  │  准入控制 (Admission Webhook)                                    │       │
│  │  • WASM 策略替代独立 Webhook 服务                               │       │
│  │  • 执行速度接近 CEL，但支持更复杂的逻辑                          │       │
│  │  • 实现示例: kwasm, kubewarden                                  │       │
│  └──────────────────────────────────────────────────────────────────┘       │
│                                                                              │
│  数据面代理扩展                                                               │
│  ┌──────────────────────────────────────────────────────────────────┐       │
...
│  │  • 轻量级工作负载 (毫秒级冷启动)                                │       │
│  │  • 跨平台兼容 (一次编译，到处运行)                               │       │
│  │  • 实现示例: WasmEdge, runwasi (containerd)                     │       │
│  └──────────────────────────────────────────────────────────────────┘       │
│                                                                              │
│  调度器扩展                                                                   │
│  ┌──────────────────────────────────────────────────────────────────┐       │
│  │  Scheduler Framework Plugin                                     │       │
│  │  • 自定义调度策略 (PreFilter/Filter/Score)                      │       │
│  │  • 沙箱执行，不影响调度器稳定性                                  │       │
│  │  • 实现示例: kwasm-scheduler                                    │       │
│  └──────────────────────────────────────────────────────────────────┘       │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

#### WASM vs eBPF vs Webhook 对比

| 维度 | WASM 插件 | eBPF 程序 | 传统 Webhook |
|------|-----------|-----------|-------------|
| 执行位置 | 用户态沙箱 | 内核态 | 独立网络服务 |
| 冷启动 | 毫秒级 | 微秒级 | 秒级 (HTTP) |
| 语言支持 | Rust/C++/Go/AssemblyScript | C | 任意语言 |
| 安全沙箱 | 是 (WASM 沙箱) | 是 (Verifier) | 依赖容器隔离 |
| 网络策略 L7 | 支持 (Envoy Filter) | 有限支持 | 支持 |
| 内核级操作 | 不支持 | 支持 | 不支持 |
| 适用场景 | 复杂业务逻辑插件 | 高性能网络/观测 | 需外部系统交互 |

#### Istio WasmPlugin 配置示例
