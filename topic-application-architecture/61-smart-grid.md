...
related_topics:
- topic-application-architecture/96-carbon-capture
...
配电网故障的快速定位、隔离和恢复供电。核心功能包括：馈线自动化（FA）故障定位隔离、配电网自愈重构、分布式电源孤岛检测、配电网状态估计。配电自动化需要在毫秒级检测故障，在秒级完成隔离和转供。
...
| 故障诊断 | CNN + 知识图谱 | 波形 + 告警 | 故障类型/位置 | T4 GPU |
