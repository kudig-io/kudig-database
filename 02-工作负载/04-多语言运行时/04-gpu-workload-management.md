...
本文系统覆盖 GPU 工作负载的完整生命周期：从 Device Plugin 部署、Pod GPU 申请、多 GPU 与 MIG 切分，到健康检查、节点亲和性、资源配额与故障自愈。AI 推理服务的容器配置可以结合 [[02-工作负载/04-多语言运行时/02-python-on-kubernetes-production.md|Python 应用 Kubernetes 生产实践]] 一起阅读。
