...
│  │  向量索引     │     │  意图路由     │     │  FTA 联动    │     │
│  │              │     │              │     │              │     │
│  │ vector-index │────▶│ hybrid-search │────▶│ topic-fta/  │     │
...
构建了 `topic-index/vector-index.json`：
...
构建了 `topic-index/hybrid-search-meta.json`：
...
# 加载向量索引
with open('topic-index/vector-index.json') as f:
...
# 加载混合搜索元数据
with open('topic-index/hybrid-search-meta.json') as f:
...
fta_path = f"topic-fta/list/{fta_code.lower().replace('_', '-')}.md"
...
topic-index
    ...
    │
    ├───▶ topic-fta/list/*.md                  (故障树)
    │
    ├───▶ topic-skills/*.md                   (技能卡片)
    │
    └───▶ topic-structural-trouble-shooting/  (结构化排查)
```
