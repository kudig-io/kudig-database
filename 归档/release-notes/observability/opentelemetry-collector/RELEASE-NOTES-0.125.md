...
- `[[Service|service]]`: Lowercase values for 'otelcol.component.kind' attributes. (#12865)
- `service`: Restrict the `telemetry.newPipelineTelemetry` feature gate to metrics. (#12856, #12933)
  ...
  On the other hand, the "on" state introduced an issue with the Collector's default internal metrics, because the [[Prometheus|Prometheus]] exporter does not currently support instrumentation scope attributes.
