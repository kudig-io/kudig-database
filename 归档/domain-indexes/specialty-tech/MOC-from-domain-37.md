...
intent_queries:
- domain-37-edge-computing MOC 是什么
- 如何 domain-37-edge-computing MOC
- Kubernetes 15 specialized tech 最佳实践
trigger_keywords:
- domain-37-edge-computing
- MOC
- specialized
- tech
prerequisites:
...
```mermaid
graph TD
    subgraph domain-37-edge-computing
        A["Domain-37 边缘计算 — 开源项目索引"]
    B["边缘计算架构概述 (Edge Computing Architecture Overview)"]
    C["云边协同设计模式 (Cloud-Edge Collaboration Design Patterns)"]
    D["KubeEdge 架构与部署 (KubeEdge Architecture and Deployment)"]
    E["KubeEdge 设备管理与边缘应用 (KubeEdge Device Management and Edge Applications)"]
    F["OpenYurt 边缘方案 (OpenYurt Edge Solution)"]
    end
