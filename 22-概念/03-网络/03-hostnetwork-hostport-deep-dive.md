...
> 本文聚焦 `hostNetwork`/`hostPort` 本身的机制与权衡。Service 的整体模型见 [[05-网络/01-K8s网络核心/07-service-concepts-types.md|Service 概念与类型]]，DNS 细节见 [[05-网络/01-K8s网络核心/12-dns-service-discovery-coredns.md|DNS 服务发现 CoreDNS]]，DaemonSet 作为最常见载体的实践见 [[22-概念/02-工作负载/daemonset.md|DaemonSet]]。
...
更多 DNS 调优细节（NDots 放大、autopath、NodeLocal DNSCache）见 [[05-网络/01-K8s网络核心/52-dns-advanced-external-integration.md|DNS 高级与外部集成]]。
...
网络安全策略的更广议题见 [[08-安全/02-网络安全/01-network-security-policies.md|网络安全策略]]。
...
- [[05-网络/01-K8s网络核心/02-network-architecture-overview.md|K8s 网络架构总览]] — Pod 网络命名空间、CNI 与 kube-proxy 的整体模型，是理解 hostNetwork「绕过了什么」的基础
- [[05-网络/01-K8s网络核心/07-service-concepts-types.md|Service 概念与类型]] — Service 的 ClusterIP/NodePort/LoadBalancer 模型，hostNetwork 与之的交互细节
- [[05-网络/01-K8s网络核心/12-dns-service-discovery-coredns.md|DNS 服务发现 CoreDNS]] — CoreDNS 工作机制，理解 dnsPolicy 各取值背后的解析路径
- [[05-网络/01-K8s网络核心/52-dns-advanced-external-integration.md|DNS 高级与外部集成]] — NDots、autopath、NodeLocal DNSCache，hostNetwork Pod 同样适用
- [[22-概念/02-工作负载/daemonset.md|DaemonSet]] — hostNetwork 最常见的载体，DaemonSet + hostNetwork 的部署模式
- [[08-安全/02-网络安全/01-network-security-policies.md|网络安全策略]] — NetworkPolicy 的能力边界，为何对 hostNetwork Pod 失效
