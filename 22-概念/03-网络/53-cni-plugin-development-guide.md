---
sources:
- "网络/K8s网络核心/53-cni-plugin-development-guide.md"
title: CNI 插件开发实战指南
...
本文**不是**一篇 CNI 规范教程（规范与配置见 [[05-网络/01-K8s网络核心/02-cni-architecture-fundamentals.md|CNI 架构基础]]），也**不是**一篇 CNI 选型指南（选型见 [[05-网络/02-网络基础/10-cni-plugin-comparison-selection.md|CNI 插件选型]]）。它假设你已经读过那两篇，了解 CNI 的位置与主流实现，现在想知道"如果我自己写一个，代码长什么样"。
...
- [[05-网络/01-K8s网络核心/02-cni-architecture-fundamentals.md|CNI 架构基础]] — CNI 规范、libcni、conflist 语法，本文的前置阅读。
- [[05-网络/01-K8s网络核心/03-cni-plugins-comparison.md|CNI 插件对比]] — Calico / Cilium / Flannel / Weave 架构与特性对比。
- [[05-网络/01-K8s网络核心/01-network-architecture-overview.md|K8s 网络架构总览]] — Kubernetes 网络模型整体视角。
- [[05-网络/02-网络基础/10-cni-plugin-comparison-selection.md|CNI 插件选型]] — 不同场景下选哪个 CNI 的决策矩阵。
- [[14-容器运行时/03-containerd-CRI-O/08-cri-interface-internals.md|CRI 接口内部]] — kubelet 如何通过 CRI 调用容器运行时，进而触发 CNI。
