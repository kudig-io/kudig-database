...
- **容器运行时演进** → [[22-概念/12-研究/container-runtime-evolution.md|container runtime evolution]]：机密容器（CoCo）依赖 Kata Containers 运行时；cgroup v2 影响所有运行时的资源限制实现。
- **AI/ML 基础设施** → [[22-概念/12-研究/k8s-ai-ml-infrastructure.md|k8s ai ml infrastructure]]：GPU 虚拟化、KSM 内存去重、NUMA 拓扑管理直接支撑 AI 训练/推理调度。
- **安全合规** → [[22-概念/05-安全/k8s-security-compliance.md|k8s security compliance]]：User Namespaces GA、机密计算、审计日志是合规基线的核心组件。
