...
description: '[[22-概念/03-网络/cilium-ebpf-networking.md|cilium ebpf networking]] 描述 eBPF
  在网络中的应用，[[23-实体/07-可观测性/prometheus-grafana.md|prometheus grafana]] 是监控栈。两者的交汇点是 **Hubble**：Cilium
  的可观测性子系统，它使用与网络策略相同的 eBPF 挂载点来收集流数据，无需额外的 sidecar 或代理。wiki 分别讨论了 eBPF 网络和 Prometheus '
summary: '[[22-概念/03-网络/cilium-ebpf-networking.md|cilium ebpf networking]] 描述 eBPF 在网络中的应用，[[23-实体/07-可观测性/prometheus-grafana.md|prometheus
...
- target: '[[23-实体/07-可观测性/prometheus.md]]'
  type: uses
- target: '[[23-实体/02-K8s核心组件/cni.md]]'
  type: related_to
status: reviewed
---
...
[[22-概念/03-网络/cilium-ebpf-networking.md|cilium ebpf networking]] 描述 eBPF 在网络中的应用，[[23-实体/07-可观测性/prometheus.md|prometheus]]-grafana]] 是监控栈。两者的交汇点是 **Hubble**：Cilium 的可观测性子系统，它使用与网络策略相同的 eBPF 挂载点来收集流数据，无需额外的 sidecar 或代理。wiki 分别讨论了 eBPF 网络和 Prometheus 监控，但没有指出 **eBPF 正在将网络可观测性从"采样推断"转变为"全量捕获"**。
...
传统网络监控的困境：
- **Sidecar 模式**：[[23-实体/04-网络/istio.md|Istio]] Envoy 提供丰富的 L7 指标，但每个 Pod 增加 ~100MB 内存和 ~5% CPU
...
- **eBPF 可观测性的标准化**：Hubble 的指标格式是 Cilium 特有的。是否应该有一个跨 [[23-实体/02-K8s核心组件/cni.md|CNI]] 的 eBPF 网络可观测性标准（如 OpenTelemetry 的网络语义约定）？
...
- [[23-实体/02-K8s核心组件/deployment.md|Deployment]] × Secret 管理.md|Deployment × Secret 管理]]
- [[22-概念/11-交叉分析/GitOps × 平台工程.md|GitOps x 平台工程]]
## Related

- [[22-概念/Deployment × Secret 管理.md|[[deployment]] × Secret 管理]]
