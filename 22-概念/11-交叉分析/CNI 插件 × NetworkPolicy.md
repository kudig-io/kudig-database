...
description: '[[23-实体/02-K8s核心组件/cni-plugins.md|cni plugins]] 实现 Pod 的网络连接，[[23-实体/02-K8s核心组件/networkpolicy.md|networkpolicy]]
...
summary: '[[23-实体/02-K8s核心组件/cni-plugins.md|cni plugins]] 实现 Pod 的网络连接，[[23-实体/02-K8s核心组件/networkpolicy.md|networkpolicy]]
  定义 Pod 级别的网络安全策略。wiki 将两者分属网络实现层和安全策略层，但它们是** inseparable 的共生关系**：NetworkPolicy
  只是 K8s API 中的一个资源定义—...'
category: synthesis
tags:
...
- cni-basics
relationships:
- target: '[[23-实体/04-网络/cilium.md]]'
  type: uses
- target: '[[23-实体/04-网络/istio.md]]'
  type: uses
- target: '[[23-实体/02-K8s核心组件/kubelet.md]]'
  type: uses
- target: '[[17-系统基础/06-知识字典/networking/service-mesh.md]]'
  type: uses
status: reviewed
---
...
[[23-实体/02-K8s核心组件/cni-plugins.md|cni plugins]] 实现 Pod 的网络连接，[[23-实体/02-K8s核心组件/networkpolicy.md|networkpolicy]] 定义 Pod 级别的网络安全策略。wiki 将两者分属网络实现层和安全策略层，但它们是** inseparable 的共生关系**：NetworkPolicy 只是 K8s API 中的一个资源定义——它描述了"哪些 Pod 可以通信"的意图，但完全没有定义"如何实现这种隔离"。真正执行策略的是 CNI 插件：Calico 用 iptables/eBPF 实现，[[23-实体/04-网络/cilium.md|Cilium]] 用 eBPF 实现，Flannel 完全不实现。
...
- 可能因节点资源耗尽而无法更新策略（[[23-实体/02-K8s核心组件/kubelet.md|kubelet]] 在 CPU 压力下跳过 CNI 调用）
...
- **NetworkPolicy 与 [[17-系统基础/06-知识字典/networking/service-mesh.md|Service Mesh]] 策略的层级关系**：NetworkPolicy（CNI 层，L3/L4）与 [[23-实体/04-网络/istio.md|Istio]] AuthorizationPolicy（Service Mesh 层，L7）同时存在时，哪个优先？两者的冲突如何检测？当前没有标准工具可以回答"这个请求是否会被允许"的问题
