...
- target: '[[23-实体/12-数据与消息/cloudnativepg.md]]'
  type: related_to
- target: '[[23-实体/11-AI与边缘/kubeflow.md]]'
  type: related_to
- target: '[[23-实体/12-数据与消息/strimzi.md]]'
  type: related_to
status: reviewed
---
...
- **数据库 Operator**：内置的 StatefulSet 控制器管理 Pod 的生命周期，但数据库的备份、恢复、版本升级、故障转移等业务逻辑需要自定义 Operator 来实现（如 [[23-实体/12-数据与消息/cloudnativepg.md|CloudNativePG]]、[[23-实体/12-数据与消息/strimzi.md|Strimzi]] Kafka Operator）。
- **AI 工作负载**：训练任务的弹性调度（如 Volcano、[[23-实体/11-AI与边缘/kubeflow.md|Kubeflow]] Training Operator）使用自定义控制器来管理 Gang Scheduling、弹性容错和检查点恢复。
...
| 控制器 | 管理的资源 | 编码的知识 |
|--------|-----------|-----------|
| [[23-实体/02-K8s核心组件/deployment.md|Deployment]] Controller | Deployment → ReplicaSet → Pod | 滚动更新、回滚策略 |
