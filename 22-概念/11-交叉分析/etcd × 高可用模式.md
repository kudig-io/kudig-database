...
- target: '[[17-系统基础/05-速查卡/k8s.md]]'
  type: related_to
- target: '[[37-归档/troubleshooting-diagnostics/kubernetes-fault-distribution-and-mttr-en.md]]'
  type: uses
...
etcd 是 Kubernetes 控制平面高可用的基石。[[17-系统基础/05-速查卡/k8s.md|K8s]] 的 HA 架构——无论是 API Server 的多实例负载均衡、Scheduler 的 leader election，还是工作负载的 PodAntiAffinity——最终都依赖 etcd 提供的**分布式一致性写入**能力。没有 etcd 的 Raft 共识，控制平面的"高可用"只是一个幻觉。
...
- **etcd v3 的 watch 机制在大规模集群中的扩展性**：当集群中有 10,000+ [[17-系统基础/06-知识字典/workloads/pods.md|Pods]] 时，watch 连接数对 etcd 内存的影响尚缺乏系统性的基准测试数据。
...
- [[23-实体/09-编排调度/armada.md|Armada (entities)]]
- [[log|Wiki Log]]
