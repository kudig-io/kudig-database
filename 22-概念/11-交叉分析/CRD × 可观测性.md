---
title: CRD × 可观测性
description: '[[23-实体/02-K8s核心组件/crd-custom-resources.md|crd custom resources]] 扩展 K8s API，[[23-实体/07-可观测性/prometheus-grafana.md|prometheus
  grafana]] 是监控栈。两者的交汇点是 **Prometheus Operator**：它通过 CRD（ServiceMonitor、PodMonitor、PrometheusRule）将监控配置从手动脚本维护转变为声明式
  GitOps 工作流。但 wiki '
summary: '[[23-实体/02-K8s核心组件/crd-custom-resources.md|crd custom resources]] 扩展 K8s API，[[23-实体/07-可观测性/prometheus-grafana.md|prometheus
...
[[23-实体/02-K8s核心组件/crd-custom-resources.md|crd custom resources]] 扩展 K8s API，[[23-实体/07-可观测性/prometheus.md|prometheus]]-grafana]] 是监控栈。两者的交汇点是 **Prometheus Operator**：它通过 CRD（ServiceMonitor、PodMonitor、PrometheusRule）将监控配置从手动脚本维护转变为声明式 GitOps 工作流。但 wiki 没有指出一个更深层的范式转变：**CRD 使可观测性配置成为了 [[23-实体/02-K8s核心组件/kubernetes.md|Kubernetes]] 的一等公民**。
...
- [[23-实体/04-网络/cilium.md|Cilium]] eBPF × 可观测性.md|Cilium eBPF × 可观测性]]
