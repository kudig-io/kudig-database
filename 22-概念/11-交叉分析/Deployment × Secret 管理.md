...
- target: '[[23-实体/02-K8s核心组件/etcd.md]]'
  type: uses
- target: '[[23-实体/06-安全/external-secrets.md]]'
  type: uses
- target: '[[17-系统基础/06-知识字典/networking/ingress.md]]'
  type: uses
- target: '[[23-实体/02-K8s核心组件/kubernetes.md]]'
  type: uses
- target: '[[20-最佳实践/01-best-practices/security/pod-security.md]]'
  type: uses
status: reviewed
---
...
tier: supporting
relationships:
- target: "[[23-实体/02-K8s核心组件/deployment.md|deployment]]"
  type: uses
---
...
- **TLS 证书挂载**：[[17-系统基础/06-知识字典/networking/ingress.md|Ingress]] Controller 或 API 服务的 Deployment 通过 Secret 卷挂载 TLS 证书。证书到期后需要滚动更新 Deployment 才能重新加载
- **Vault Agent Sidecar**：Deployment 的 Pod 模板中注入 Vault Agent Sidecar，将动态凭证以内存卷形式挂载到应用容器。这是生产环境推荐的模式，但增加了 Pod 复杂度和启动延迟
- **[[23-实体/06-安全/external-secrets.md|External Secrets]] Operator**：Deployment 引用由 ESO 自动同步的 K8s Secret，将外部密钥管理（Vault、AWS Secrets Manager）与 Deployment 的声明式配置解耦
...
| **Secret 大小限制** | K8s Secret 大小限制为 1MB（[[23-实体/02-K8s核心组件/etcd.md|etcd]] 的 value 大小限制）。大型 TLS 证书链或 CA 捆绑包可能超过此限制，需要拆分为多个 Secret 或使用 ConfigMap |
...
- **Secret 与 Pod 安全标准的冲突**：[[20-最佳实践/01-best-practices/security/pod-security.md|Pod Security]] Standards（Restricted）禁止以 root 运行、要求只读根文件系统。但某些旧版应用读取 Secret 文件时需要特定权限，导致安全策略与应用需求的冲突
...
- [[23-实体/02-K8s核心组件/kubernetes.md|kubernetes]]-events/13-security-admission-rbac-events|13 - 安全、准入控制与 RBAC 事件]] — Cross-reference
- [[22-概念/11-交叉分析/纵深防御 × 供应链安全.md|纵深防御 x 供应链安全]] — Cross-reference
- [[22-概念/05-安全/cloud-native-defense-in-depth.md|Cloud Native Defense in Depth]] — Cross-reference
