...
description: '[[23-实体/02-K8s核心组件/etcd.md|etcd]] 是 K8s 的心脏，[[22-概念/01-核心架构/operator-pattern.md|operator
...
summary: '[[23-实体/02-K8s核心组件/etcd.md|etcd]] 是 K8s 的心脏，[[22-概念/01-核心架构/operator-pattern.md|operator
  pattern]] 描述自定义控制器管理有状态应用。两者的交叉点是 **etcd Operator**：将 etcd 集群的生命周期管理（部署、扩容、备份、恢复、升级）自动化。但
...
- target: '[[23-实体/07-可观测性/prometheus.md]]'
  type: uses
- target: '[[22-概念/11-交叉分析/eBPF × 运行时安全.md]]'
  type: related_to
- target: '[[22-概念/11-交叉分析/etcd × 高可用模式.md]]'
  type: uses
- target: '[[22-概念/11-交叉分析/etcd × 可观测性.md]]'
  type: uses
...
[[23-实体/02-K8s核心组件/etcd.md|etcd]] 是 [[17-系统基础/05-速查卡/k8s.md|K8s]] 的心脏，[[22-概念/01-核心架构/operator-pattern.md|operator pattern]] 描述自定义控制器管理有状态应用。两者的交叉点是 **etcd Operator**：将 etcd 集群的生命周期管理（部署、扩容、备份、恢复、升级）自动化。但 wiki 没有指出一个关键矛盾：**etcd 是 Operator 想要管理的最危险的目标**——因为 etcd 问题直接导致整个集群不可用，而 Operator 本身的问题也可能引发 etcd 问题。
...
- **独立监控**：Operator 的健康监控必须独立于被管理的 etcd 集群（使用独立的 [[23-实体/07-可观测性/prometheus.md|Prometheus]] 实例）
