...
- [[17-系统基础/05-速查卡/networking.md|networking]]/service|service]]-mesh
- mtls
- zero-trust
- istio
- cilium
...
[[22-概念/03-网络/service-mesh-architecture.md|service-mesh-architecture]] 描述了服务网格的通信能力（mTLS、流量管理、可观测性），[[22-概念/05-安全/security-defense-depth.md|security-defense-depth]] 在零信任架构部分提到"每个 API 请求需要认证、每次访问需要授权、所有流量受 NetworkPolicy 约束、运行行为被监控"。但两者没有明确指出：**服务网格就是零信任理念在 K8s 服务间通信层的具体技术实现**。
...
- **Cilium [[17-系统基础/06-知识字典/networking/service-mesh.md|Service Mesh]]** 用 eBPF 实现 L4 mTLS，性能成本 <1%，使得零信任策略可以无性能代价地大规模部署
...
- **用户到服务的认证**（由 API Gateway / [[17-系统基础/06-知识字典/networking/ingress.md|Ingress]] 处理）
- **API 访问控制**（由 RBAC 处理）
