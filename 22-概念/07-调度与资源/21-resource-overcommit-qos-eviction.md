---
sources:
- "集群基础/设计原则/21-resource-overcommit-qos-eviction.md"
...
本文是**串联性专题**。知识库中 [[22-概念/07-调度与资源/resource-management.md|资源管理]]、[[22-概念/07-调度与资源/pod-overhead.md|Pod Overhead]]、[[17-系统基础/06-知识字典/scheduling/koordinator.md|Koordinator]]、[[17-系统基础/06-知识字典/scheduling/resource-bin-packing.md|Resource Bin Packing]] 各自讲了 overcommit 的一个侧面，但缺少把"**QoS → 可压缩/不可压缩资源 → 节点驱逐阈值 → Bin-Packing/overcommit ratio**"这条因果链一次打通的文档。本文即做这个串联。
...
更完整的信号列表、计算方式与配置示例见 [[01-集群基础/03-控制平面/33-kubelet-eviction-thresholds.md|kubelet 驱逐阈值]]。
...
reserved 越大 → Allocatable 越小 → 能超卖的越少 → 越安全。这是 overcommit 与稳定性的"旋钮"。建议按节点规格阶梯配置（见 [[01-集群基础/03-控制平面/33-kubelet-eviction-thresholds.md|kubelet 驱逐阈值]] 中的建议表）。
...
kube-scheduler 的 `NodeResourcesFit` 插件在 Score 阶段有三种策略（详见 [[17-系统基础/06-知识字典/scheduling/resource-bin-packing.md|Resource Bin Packing]]）：
...
[[17-系统基础/06-知识字典/scheduling/koordinator.md|Koordinator]]（阿里开源，CNCF Sandbox）核心能力：
...
- [[22-概念/07-调度与资源/resource-management.md|资源管理]] — requests/limits/QoS 的基础概念
- [[22-概念/07-调度与资源/pod-overhead.md|Pod Overhead]] — 运行时额外开销如何计入 overcommit 计算
- [[17-系统基础/06-知识字典/scheduling/qos.md|QoS]] — QoS 三类速查
- [[01-集群基础/03-控制平面/33-kubelet-eviction-thresholds.md|kubelet 驱逐阈值]] — 驱逐信号、soft/hard、预留配置完整手册
- [[17-系统基础/06-知识字典/scheduling/koordinator.md|Koordinator]] — 基于真实负载的混部与动态超卖
- [[17-系统基础/06-知识字典/scheduling/resource-bin-packing.md|Resource Bin Packing]] — MostAllocated / RequestedToCapacityRatio 装箱策略
- [[01-集群基础/02-设计原则/01-design-principles-foundations.md|设计原则基础]] — K8s 设计哲学总纲
