...
核心概念详见 [[22-概念/09-平台与发布/progressive-delivery-strategies.md|progressive delivery strategies]]。
...
- [[22-概念/09-平台与发布/gitops-production-operations.md|gitops production operations]] — GitOps（Argo CD、Flux）是 Kubernetes 声明式变更管理与发布自动化的事实标准
- [[22-概念/09-平台与发布/platform-engineering-idp.md|platform engineering idp]] — 内部开发者平台（IDP）将发布流程标准化为自助式工作流，提升交付效率
- [[22-概念/08-可靠性与运维/slo-error-budget-framework.md|slo error budget framework]] — Error Budget 决策框架指导发布频率与变更风险容忍度的平衡
- [[22-概念/06-可观测性/k8s-observability-stack.md|k8s observability stack]] — 发布可观测性（金丝雀分析、自动化回滚）依赖实时指标采集与评估引擎
