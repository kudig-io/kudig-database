---
sources:
- "存储/K8s存储/19-volume-attachment-controller.md"
...
详见 [[06-存储/01-K8s存储/17-csi-migration-in-tree-to-csi.md|CSI 迁移：in-tree 到 CSI]]。
...
任一层的 finalizer 卡住，都会让上层对象无法回收。详见 [[06-存储/01-K8s存储/02-pv-architecture-fundamentals.md|PV 架构基础]]。
...
- [[01-集群基础/03-控制平面/22-container-storage-deep-dive.md|容器存储深度剖析]] — 存储全链路（attach + mount）的总览视角，本文是其 attach 章节的源码级展开
- [[06-存储/01-K8s存储/06-csi-drivers-integration.md|CSI 驱动集成]] — CSI 生态全景（provisioner/attacher/resizer/snapshotter sidecar 协作）
- [[06-存储/01-K8s存储/02-pv-architecture-fundamentals.md|PV 架构基础]] — PV/PVC 绑定与 finalizer 保护机制
- [[06-存储/01-K8s存储/17-csi-migration-in-tree-to-csi.md|CSI 迁移：in-tree 到 CSI]] — in-tree 卷如何通过 CSIMigration 走 CSI 路径
- [[01-集群基础/03-控制平面/13-kube-controller-manager-deep-dive.md|kube-controller-manager 深度剖析]] — AD Controller 在 KCM 各 controller 中的定位
- [[01-集群基础/03-控制平面/15-kubelet-deep-dive.md|kubelet 深度剖析]] — 节点侧 VolumeManager 的 mount/unmount 阶段（attach 的下半场）
