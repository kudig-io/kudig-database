...
- target: '[[23-实体/02-K8s核心组件/kubernetes.md]]'
  type: uses
- target: '[[23-实体/05-存储/longhorn.md]]'
  type: related_to
status: stable
---
...
Velero 是 [[23-实体/02-K8s核心组件/kubernetes.md|Kubernetes]] 生态中事实标准的备份与灾难恢复工具。它将集群状态的持久化（[[23-实体/02-K8s核心组件/etcd.md|etcd]] 快照 + PV 快照）与灾难恢复流程（跨区域恢复、命名空间级恢复）结合，填补了"有备份"和"能恢复"之间的关键鸿沟。本页连接 存储 的存储备份技术与 可靠性 的灾备方法论，展示 Velero 如何在生产环境中构建可验证的恢复能力。
...
| **[[23-实体/05-存储/longhorn.md|Longhorn]]** | 分布式存储 | 内置快照与跨区域复制，可与 Velero 互补 |
