---
title: 核心架构 Concepts
summary: Kubernetes 核心架构与机制概念索引——架构总览、声明式 API、控制器模式、垃圾回收、字段所有权。
category: index
tags:
- index
- concepts
- architecture
- control-plane
tier: supporting
created: '2026-07-24'
last_updated: 2026-07
---

# 核心架构 Concepts

> Kubernetes 架构总览、控制平面核心机制、API 模型与扩展点概念索引。

## 文档

- [[22-概念/01-核心架构/19-garbage-collection-owner-reference.md|垃圾回收与 ownerReference 级联删除机制]]
- [[22-概念/01-核心架构/20-server-side-apply-field-ownership.md|Server-Side Apply 与字段所有权]]
- [[22-概念/01-核心架构/17-operator-sdk-go-helm-ansible-types.md|Operator SDK 三种类型 (Go/Helm/Ansible)]]
- [[22-概念/01-核心架构/Kubernetes Core Concepts.md|Kubernetes Core Concepts]]
- [[22-概念/01-核心架构/cloud-provider-k8s-integration.md|cloud-provider-k8s-integration]]
- [[22-概念/01-核心架构/controller-pattern.md|controller-pattern]]
- [[22-概念/01-核心架构/core-dependency-version-matrix.md|core-dependency-version-matrix]]
- [[22-概念/01-核心架构/declarative-api.md|declarative-api]]
- [[22-概念/01-核心架构/etcd Operational Reference.md|etcd Operational Reference]]
- [[22-概念/01-核心架构/eventual-consistency.md|eventual-consistency]]
- [[22-概念/01-核心架构/kubernetes-architecture-overview.md|kubernetes-architecture-overview]]
- [[22-概念/01-核心架构/kubernetes.md|kubernetes]]
- [[22-概念/01-核心架构/operator-pattern.md|operator-pattern]]
- [[22-概念/01-核心架构/watch-mechanism.md|watch-mechanism]]

## Related

- [[22-概念/index.md|Concepts 总索引]]
- [[01-集群基础/02-设计原则/index.md|设计原则（源文档）]]
