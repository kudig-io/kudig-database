---
sources:
- "可观测性/指标/19-kube-state-metrics-deep-dive.md"
...
2. **KSM 不参与 HPA 决策**：HPA 依赖的 resource metrics 走 `metrics.k8s.io`（Metrics Server 提供），而 custom.metrics / external.metrics 走 Prometheus Adapter（详见 [[09-可观测性/02-指标/06-custom-metrics-adapter.md|自定义指标适配器]]）。KSM 只把数据给 Prometheus，不直接服务 autoscaler。
...
完整的 pipeline 全景见 [[09-可观测性/02-指标/02-monitoring-metrics-system.md|监控指标体系]] 第 1 节。
...
| **查询习惯** | 必须带枚举 label 过滤，否则值被 0 污染 | 团队约定 PromQL 写法，见 [[09-可观测性/02-指标/09-monitoring-cost-optimization.md|监控成本优化]] |
...
> 完整指标清单（数百项）见 KSM 官方 `metrics.md`；告警阈值模板见 [[09-可观测性/02-指标/05-monitoring-metrics-prometheus.md|Prometheus 监控指标]] 第 6 节。
...
> ServiceMonitor 的完整语法与最佳实践见 [[09-可观测性/02-指标/01-prometheus-enterprise-monitoring.md|Prometheus 企业级监控]] 第 2 节。
...
> Recording Rules 与成本治理的深度结合见 [[09-可观测性/02-指标/09-monitoring-cost-optimization.md|监控成本优化]] 与 [[09-可观测性/02-指标/10-cost-optimization-observability.md|可观测性成本优化]]。
...
> 多集群联邦的完整架构与 Thanos 侧处理见 [[09-可观测性/02-指标/03-thanos-enterprise-metrics-federation.md|Thanos 企业级指标联邦]] 与 [[09-可观测性/02-指标/08-multi-cluster-monitoring-governance.md|多集群监控治理]]。
...
- [[09-可观测性/02-指标/01-prometheus-enterprise-monitoring.md|Prometheus 企业级监控]] — KSM 的下游消费者，ServiceMonitor 与告警体系全景
- [[09-可观测性/02-指标/02-monitoring-metrics-system.md|监控指标体系]] — metrics pipeline 总览，KSM 在其中的节点定位
- [[09-可观测性/02-指标/05-monitoring-metrics-prometheus.md|Prometheus 监控指标]] — KSM 关键指标速查表与告警阈值模板
- [[09-可观测性/02-指标/06-custom-metrics-adapter.md|自定义指标适配器]] — KSM 数据如何经 Prometheus Adapter 流向 HPA
- [[09-可观测性/02-指标/09-monitoring-cost-optimization.md|监控成本优化]] — allowlist 与基数治理的成本视角
- [[09-可观测性/02-指标/10-cost-optimization-observability.md|可观测性成本优化]] — KSM 容量指标驱动成本归因模型
- [[09-可观测性/02-指标/07-enterprise-scale-monitoring.md|企业级规模监控]] — 大规模集群的 KSM 分片与横向扩展策略
