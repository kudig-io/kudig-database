...
3. **CEL原生策略取代OPA/Rego**：Kubernetes 中 ValidatingAdmissionPolicy 于 1.30 GA、MutatingAdmissionPolicy 于 1.36 GA，CEL 成为策略表达的原生语言。Kyverno 2.0采用CEL作为后端引擎，Gatekeeper/OPA退居遗留场景。CEL的"嵌入式零依赖"特性消除了策略引擎的外部依赖风险。
...
相关核心概念详见 [[22-概念/05-安全/k8s-security-compliance.md|k8s security compliance]]，涵盖：

- [[22-概念/05-安全/supply-chain-security.md|supply chain security]] — Sigstore、SLSA与镜像签名验证
- eBPF 运行时安全 — eBPF运行时监控与Tetragon
- CEL 原生策略 — CEL策略引擎与Admission Policy GA
- 零信任 mTLS — mTLS、SPIFFE与零信任微分段
- [[22-概念/05-安全/secrets-management.md|secrets management]] — External Secrets Operator与CSI Driver
...
- [[22-概念/12-研究/k8s-networking-evolution.md|k8s networking evolution]] — 网络安全（Cilium NetworkPolicy、eBPF 可观测）是零信任架构的网络层核心
- [[22-概念/12-研究/container-runtime-evolution.md|container runtime evolution]] — 安全容器运行时（gVisor、Kata Containers、Confidential Containers）提供工作负载级隔离
- [[22-概念/09-平台与发布/gitops-production-operations.md|gitops production operations]] — GitOps 审计追踪与声明式配置管理是安全合规（SOC2、PCI-DSS）的技术支撑
- [[22-概念/06-可观测性/k8s-observability-stack.md|k8s observability stack]] — 安全可观测性（审计日志、运行时威胁检测）是安全运营中心（SOC）的数据基础
