---
title: gitops
description: GitOps 标签枢纽 — 涵盖 ArgoCD、Flux、渐进式交付、CI/CD 管道、声明式基础设施、多集群 GitOps 等全部 GitOps 领域知识
category: tag-index
tags:
- gitops
- argocd
- flux
- progressive-delivery
- ci-cd
- declarative
tier: core
difficulty: intermediate
domain: release-management
k8s_versions: ["1.28", "1.30", "1.32", "1.34"]
created: '2026-07-11'
last_updated: '2026-07-21'
---

# gitops Tag Hub

> GitOps 领域页面 — ArgoCD、Flux、GitOps 模式、渐进式交付、CI/CD 管道等。

## 核心定义

**GitOps** 是一种以 Git 仓库为唯一事实来源（Single Source of Truth）来管理基础设施和应用的运维方法论。通过声明式配置 + 自动化调和循环，实现可审计、可回滚、可重现的持续交付。

### GitOps 四大原则

| 原则 | 描述 |
|------|------|
| 声明式 | 系统期望状态以声明式方式定义 |
| 版本化 | 期望状态存储在 Git 中，可追溯 |
| 自动调和 | 持续将实际状态向期望状态收敛 |
| 可审计 | 所有变更通过 Git 历史可审计 |

### GitOps 工具对比

| 工具 | 模式 | 多集群 | 渐进式交付 | 特色 |
|------|------|--------|------------|------|
| ArgoCD | Pull | 支持 | Argo Rollouts | UI 友好、Application CRD |
| Flux | Pull | 支持 | Flagger | 轻量、Kustomize 原生 |
| Fleet | Pull | 原生 | 支持 | Rancher 生态、大规模 |

## GitOps 平台 (GitOps Platforms)

- [[发布变更/GitOps/01-argo-cd-enterprise-gitops|ArgoCD 企业级 GitOps]]
- [[发布变更/GitOps/02-jenkins-enterprise-cicd|Jenkins 企业级 CI/CD]]
- [[发布变更/GitOps/03-gitlab-enterprise-cicd|GitLab 企业级 CI/CD]]
- [[发布变更/GitOps/04-github-actions-enterprise|GitHub Actions 企业级]]
- [[发布变更/GitOps/05-tekton-cloud-native-cicd|Tekton 云原生 CI/CD]]
- [[发布变更/GitOps/06-flux-gitops-continuous-delivery|Flux GitOps 持续交付]]
- [[发布变更/GitOps/07-gitops-security-compliance|GitOps 安全合规]]
- [[发布变更/GitOps/08-cicd-pipeline-patterns|CI/CD 管道模式]]
- [[发布变更/GitOps/08-fleet-gitops-operations-guide|Fleet GitOps 运营指南]]
- [[发布变更/GitOps/99-argo-cd-gitops-guide|ArgoCD GitOps 指南]]
- [[发布变更/GitOps/99-flux-gitops-guide|Flux GitOps 指南]]
- [[发布变更/GitOps/99-tekton-cicd-guide|Tekton CI/CD 指南]]

## 清单模式 (Manifest Patterns)

- [[清单模式/04-gitops-patterns/01-argocd-app-of-apps|ArgoCD App-of-Apps]]
- [[清单模式/04-gitops-patterns/02-argocd-applicationset-multi-cluster|ArgoCD ApplicationSet 多集群]]
- [[清单模式/04-gitops-patterns/03-flux-kustomization-patterns|Flux Kustomization 模式]]
- [[清单模式/04-gitops-patterns/04-gitops-directory-structure|GitOps 目录结构]]
- [[清单模式/04-gitops-patterns/05-gitops-secret-management|GitOps 密钥管理]]
- [[清单模式/04-gitops-patterns/06-gitops-wave-sync|GitOps Wave 同步]]
- [[清单模式/04-gitops-patterns/07-gitops-drift-detection|GitOps 漂移检测]]
- [[清单模式/04-gitops-patterns/08-gitops-progressive-delivery|GitOps 渐进式交付]]

## 概念 (Concepts)

- [[概念/gitops-principles|GitOps 原则]]
- [[概念/gitops-production-operations|GitOps 生产运营]]
- [[概念/gitops-release-gate|GitOps 发布门禁]]
- [[概念/gitops-sre-release-gate|GitOps SRE 发布门禁]]
- [[概念/gitops-tool-evolution|GitOps 工具演进]]
- [[概念/helm-argocd-gitops|Helm ArgoCD GitOps]]
- [[概念/GitOps x 平台工程|GitOps 与平台工程]]
- [[概念/progressive-delivery-strategies|渐进式交付策略]]

## 部署方案 (Deployment)

- [[发布变更/部署方案/02-single-node-deployment|单节点部署]]
- [[发布变更/部署方案/04-production-environment-deployment|生产环境部署]]
- [[发布变更/变更管理/02-canary-release-strategy|金丝雀发布策略]]

## IaC (Infrastructure as Code)

- [[发布变更/IaC/11-infrastructure-as-code|基础设施即代码]]
- [[发布变更/IaC/99-crossplane-platform-guide|Crossplane 平台指南]]

## 集群基础 (Cluster Fundamentals)

- [[集群基础/控制平面/26-gitops-automation-operations|GitOps 自动化运营]]
- [[集群基础/设计原则/11-extensibility-design-patterns|扩展性设计模式]]

## 故障诊断 (Troubleshooting)

- [[故障诊断/高级排障/38-gitops-argocd-troubleshooting|GitOps ArgoCD 排障]]
- [[故障诊断/高级排障/structural-08-cluster-operations/03-helm-troubleshooting|Helm 排障]]
- [[故障诊断/高级排障/structural-11-gitops-devops/01-gitops-devops-troubleshooting|GitOps/DevOps 排障]]
- [[故障诊断/FTA故障树/list/gitops-argocd-fta|GitOps ArgoCD 故障树]]

## 技能 (Skills)

- [[技能/gitops-argocd-fta|GitOps ArgoCD FTA]]
- [[技能/排障实战/gitops-devops/ts-gitops-devops.md|GitOps/DevOps 排障]]
- [[最佳实践/scenarios/gitops-workflow|GitOps 工作流最佳实践]]

## 平台工程 (Platform Engineering)

- [[平台工程/运维/07-gitops-configuration-management|GitOps 配置管理]]
- [[平台工程/运维/08-automation-toolchain|自动化工具链]]

## 研究 (Research)

- [[研究/gitops-multi-cluster|GitOps 多集群]]

## 综合 (Synthesis)

- [[综合/argocd-gitops|ArgoCD GitOps]]
- [[综合/helm-gitops|Helm GitOps]]
- [[综合/crossplane-iac|Crossplane IaC]]

## 实体 (Entities)

- [[实体/argocd|ArgoCD]]
- [[实体/flux|Flux]]
- [[实体/argo|Argo]]
- [[实体/cncf-cicd|CNCF CI/CD]]
- [[实体/release-notes-cicd-gitops|Release Notes: CI/CD GitOps]]

## 扩展机制 (Extension Mechanisms)

- [[专项技术/扩展机制/08-cicd-pipelines|CI/CD 管道]]
- [[专项技术/扩展机制/09-gitops-workflow-argocd|GitOps 工作流 ArgoCD]]

## 知识字典 (Knowledge Dictionary)

- [[系统基础/知识字典/operations/gitops|GitOps]]
- [[系统基础/知识字典/operations/argo|Argo]]
- [[系统基础/知识字典/operations/flux|Flux]]
- [[系统基础/知识字典/operations/pipecd|PipeCD]]
- [[系统基础/知识字典/platform-engineering/gitops-and-continuous-delivery|GitOps 与持续交付]]
- [[系统基础/速查卡/gitops|GitOps 速查卡]]

## GitOps 技术全景

### GitOps 核心原则

| 原则 | 说明 |
|---|---|
| 声明式 | 系统状态以声明式配置描述 |
| 版本化 | 配置存储在 Git 中，可追溯 |
| 自动同步 | 自动将实际状态向期望状态调和 |
| 持续调和 | 检测漂移并自动修复 |

### GitOps 工具对比

| 工具 | 特点 | 适用场景 |
|---|---|---|
| ArgoCD | UI 丰富、多集群 | 复杂环境 |
| Flux | 轻量、原生 K8s | 简单环境 |
| Jenkins X | CI/CD 集成 | 全流程 |

## 面试要点

1. **Q：GitOps 的核心价值？**
   A：可审计、可回滚、一致性、自动化、协作友好。

2. **Q：ArgoCD vs Flux 如何选择？**
   A：ArgoCD：需要 UI、多集群、应用集。Flux：轻量、Helm 集成、简单环境。

3. **Q：GitOps 的安全控制？**
   A：分支保护、PR 审批、镜像签名、RBAC、审计日志、Secret 加密。

## Related Tags

- [[标签/helm|helm]]
- [[标签/operator|operator]]
- [[标签/multi-cluster|multi-cluster]]
- [[标签/production|production]]
