...
Docker 是学习 [[实体/kubernetes.md|[[Kubernetes|kubernetes]]]] 的第一块基石。Kubernetes 的核心功能就是编排和管理容器，而 Docker 是目前最流行的容器运行时之一（准确地说，K8s 1.24+ 使用 [[containerd|containerd]] 作为运行时，但 Docker 的概念和操作仍然是理解容器技术的基础）。
