...
| # | 症状描述 | 检测方法 | 置信度 |
|---|---------|---------|--------|
| S1 | `kubectl get nodes` 显示 NotReady | STATUS 列检查 | 0.95 |
| S2 | 节点状态在 Ready/NotReady 间频繁切换 | Events 交替出现 NodeReady/NodeNotReady | 0.85 |
| S3 | 节点上 Pod 被大量驱逐 | `kubectl get events --field-selector reason=Evicted` | 0.80 |
| S4 | DiskPressure/MemoryPressure/PIDPressure 为 True | `kubectl describe node` Conditions | 0.90 |
| S5 | kubelet 日志出现 apiserver 连接拒绝 | `journalctl -u kubelet` 检查 | 0.75 |
| S6 | Prometheus 告警 `KubeNodeNotReady` 触发 | `kube_node_status_condition{condition="Ready",status="false"}` | 0.95 |
| S7 | Node Lease 长时间未更新 | `kubectl get lease -n kube-node-lease <node>` | 0.90 |
