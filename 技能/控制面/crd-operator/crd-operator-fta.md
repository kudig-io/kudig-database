...
sources: ["故障诊断/FTA故障树/list/crd-operator-fta.md"]
created: 2026-05-21
updated: 2026-05-21
lifecycle: reviewed
...
### 案例 1: Operator Pod 崩溃导致 CR 无法协调

| 时间 | 事件 |
|------|------|
| 12:00 | 自定义资源状态停止更新，业务功能异常 |
| 12:05 | `kubectl get pods -n operator-system` 显示 CrashLoopBackOff |
| 12:08 | 日志: "failed to list CRD: the server could not find the requested resource" |
| 12:10 | CRD 被意外删除，Operator 无法找到资源定义 |
| 12:15 | 🔴 重新应用 CRD manifest，Operator 恢复 |

**根因**: 清理操作误删 CRD，导致 Operator 无法 watch 自定义资源。

### 案例 2: CRD 版本升级导致存量 CR 不兼容

**现象**: 升级 Operator 后，旧版 CR 无法被新版识别。

**诊断**: CRD 新增 required 字段，存量 CR 缺少该字段

**修复**: 🟡 使用 conversion webhook 或手动迁移存量 CR

## 升级决策点

| 级别 | 条件 | 动作 |
|------|------|------|
| P0 | Operator 崩溃影响核心业务 | 重启 Operator + 检查 CRD |
| P1 | CR 协调失败 | 检查 Operator 日志和 CR 状态 |
| P2 | CRD 版本迁移 | 规划 conversion webhook |

## 面试要点

1. **Q: Operator 模式的核心原理？**
   A: Operator = CRD(定义期望状态) + Controller(协调循环)。Controller watch CR 变更，对比期望状态与实际状态，执行 Reconcile 逻辑将实际状态向期望状态收敛。

2. **Q: CRD 版本管理的最佳实践？**
   A: ① 使用 conversion webhook 支持多版本共存 ② 新增字段设为 optional ③ 使用 storage version 标记存储版本 ④ 提供迁移工具 ⑤ 弃用字段保留至少 2 个版本。

3. **Q: Operator 开发框架对比？**
   A: Kubebuilder: 官方推荐，代码生成+脚手架；Operator SDK(Red Hat): 基于 Kubebuilder，额外支持 OLM 打包；controller-runtime: 底层库，灵活但复杂。新项目推荐 Kubebuilder。
