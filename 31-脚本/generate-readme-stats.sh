...
# 用法:
#   ./31-脚本/generate-readme-stats.sh             # 默认表格输出
#   ./31-脚本/generate-readme-stats.sh --json      # JSON 格式（可供其他脚本消费）
#   ./31-脚本/generate-readme-stats.sh --badges    # 输出 README 徽章建议
#   ./31-脚本/generate-readme-stats.sh --diff      # 输出指标并与 README 当前数字比对
...
PRODUCT_COUNT=$(ls 19-故障诊断/06-FTA故障树/list/*-fta.md 2>/dev/null | sed 's/.*\///' | sed 's/-fta\.md$//' | sort -u | wc -l | tr -d ' ')

# ═══════════════════════════════════════════════════
# 2. AI 相关指标
# ═══════════════════════════════════════════════════
AI_AGENT_DOCS=$(count_md_content "15-AI基础设施/02-AI-Agents")
FTA_TREES=$(ls 19-故障诊断/06-FTA故障树/list/*-fta.md 2>/dev/null | wc -l | tr -d ' ')
FTA_TOTAL_DOCS=$(count_md "19-故障诊断/06-FTA故障树")
FEBM_DOCS=$(count_md_content "19-故障诊断/07-FEBM方法论")
LEARN_DOCS=$(count_md "29-文档/learning-paths")
CNCF_PROJECTS=$(count_md_content "21-生态参考/01-CNCF全景")

# ═══════════════════════════════════════════════════
# 3. 运维专题指标
# ═══════════════════════════════════════════════════
TS_D12=$(count_md "19-故障诊断")
TS_STS=$(count_md "19-故障诊断/04-高级排障")
TROUBLESHOOT_DOCS=$((TS_D12 + TS_STS + FTA_TOTAL_DOCS))
SKILLS_COUNT=$(count_md_content "26-技能")
CHEAT_SHEET_COUNT=$(count_md_content "17-系统基础/05-速查卡")
PRESENTATION_COUNT=$(count_md_content "26-技能/培训学习/training-public/topic-presentations")
PAPERS_COUNT=$(count_md_content "21-生态参考/02-论文")

# ═══════════════════════════════════════════════════
# 4. 云厂商
# ═══════════════════════════════════════════════════
CLOUD_PROVIDERS=$(ls -d 18-云厂商/*/ 2>/dev/null | wc -l | tr -d ' ')
