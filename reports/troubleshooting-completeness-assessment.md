...
**问题**: 当前体系缺乏从"故障现象"快速定位到"排查路径"的映射层。
  ...
  related_docs:
    ...
    domain_12:
      - path: "domain-12/07-oom-memory-diagnosis.md"
        ...
        relevance: 0.90
    skills:
      - path: "topic-skills/oom-diagnosis-skill.md"
        description: "自动化诊断技能"
        auto_executable: true
    febm_case:
      - path: "topic-febm/FEBM-case-INC-2026-0215.md"
...
#### 改进5: 服务网格 (ASM/Istio) 故障覆盖不足

**问题**: TE-10 (ASM 服务网格故障) 的底事件和排查文档需要扩展。
...
| **P1** | 多集群故障场景扩展 | 中 | 中 | 多集群用户 |
| **P1** | ASM/Istio 故障覆盖扩展 | 中 | 中 | 服务网格用户 |
...
1. **增加症状快速映射层**
   ...
   - 参考 topic-structural-trouble-shooting/43-symptom-sop-mapping.md 的格式
...
3. **扩展 Terway 故障树排查**
   - 将 topic-terway/07-troubleshooting-fta.md 扩展为完整手册
...
| 方向 | 优先级 | 说明 |
|:---|:---:|:---|
| **症状快速映射** | P0 | 从故障现象到排查路径的快速定位 |
| **交叉引用完善** | P0 | FTA 与 Structural 的无缝跳转 |
| **ACK 特有场景** | P1 | Terway/ASM/ACK-One 多集群故障覆盖 |
