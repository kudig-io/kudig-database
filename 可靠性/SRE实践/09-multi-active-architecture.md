...
本文覆盖从同城双活到异地多活的完整架构设计，包括数据同步策略、流量调度机制、写冲突解决方案以及 Kubernetes 多集群多活的实现路径。多活架构是 [[可靠性/SRE实践/09-multi-active-architecture.md|灾难恢复]] 能力的终极保障，也是 [[可靠性/SRE实践/01-availability-calculation-model.md|高可用性]] 的核心支撑。
...
5. **定期演练**：每季度执行一次多活切换演练，验证 RTO/RPO 达标。参考 [[可靠性/灾难恢复]] 演练流程。
...
多活架构需要与以下体系深度集成：
- [[可观测性/指标/16-multi-cluster-monitoring-governance.md|多集群监控治理]]：统一监控所有活跃集群
- [[可靠性/SRE实践/03-incident-command-system.md|事件指挥系统]]：多活切换作为标准事件流程
- [[发布变更/GitOps/01-argo-cd-enterprise-gitops.md|Argo CD 多集群]]：多集群统一部署
- [[可靠性/SRE实践/01-availability-calculation-model.md|可用性计算]]：多活对可用性指标的影响

## Related

- [[可靠性/SRE实践/01-availability-calculation-model.md|可用性计算模型]]
- [[可靠性/灾难恢复]]
- [[可靠性/备份恢复]]
- [[可观测性/指标/16-multi-cluster-monitoring-governance.md|多集群监控治理]]
- [[发布变更/GitOps/01-argo-cd-enterprise-gitops.md|Argo CD 企业级 GitOps]]
- [[可靠性/SRE实践/08-resilience-patterns-circuit-breaker.md|弹性模式]]
