...
| 6 | SLO/SLI 已定义 | 关键用户旅程已设定可量化的 SLO，错误预算与 Burn Rate 告警已上线 | 见 [[可观测性/SLO-SLI/02-slo-implementation-guide.md|SLO 设定与实施指南]] |
| 7 | 发布门控生效 | 错误预算不足时自动阻断或降级发布，金丝雀/蓝绿策略已配置 | 见 [[可靠性/SRE实践/02-release-gate-slo-based.md|基于 SLO 的发布门控]] |
| 8 | 证书生命周期监控 | K8s CA、cert-manager、Ingress mTLS 证书有效期 ≥30 天告警 | `kubeadm certs check-expiration` / `kubectl get certificate -A` |
| 9 | 灾备演练记录 | AZ/Region 级、控制面、有状态应用三类场景已演练并有 RTO/RPO 数据 | 见 [[可靠性/灾难恢复/01-dr-scenarios-catalog.md|灾备场景目录]] |
| 10 | 混沌工程安全护栏 | 实验具备 blast-radius 控制、kill-switch、自动回滚，且避开生产高峰 | 见 [[可靠性/混沌工程/03-chaos-experiment-design.md|混沌实验设计]] |
| 11 | 可观测性闭环 | 日志、指标、链路覆盖关键路径，告警已关联 on-call 值班表与升级策略 | `kubectl get prometheusrules -A` |
| 12 | 事后复盘文化 | 近 90 天内 P1/P0 事故已完成无责复盘，改进项有 Owner 与截止日期 | 见 [[可靠性/事后复盘/01-blameless-postmortem-template.md|无责事后复盘模板]] |
...
- [[可靠性/SRE实践/02-release-gate-slo-based.md|基于 SLO 的发布门控]]
- [[可靠性/备份恢复/01-etcd-backup-restore.md|etcd 备份与恢复]]
- [[可靠性/灾难恢复/99-velero-backup-recovery-guide.md|Velero 备份恢复指南]]
- [[可靠性/混沌工程/03-chaos-experiment-design.md|混沌实验设计]]
- [[可靠性/灾难恢复/01-dr-scenarios-catalog.md|灾备场景目录]]
- [[可靠性/灾难恢复/02-az-failure-playbook.md|AZ 故障 Playbook]]
