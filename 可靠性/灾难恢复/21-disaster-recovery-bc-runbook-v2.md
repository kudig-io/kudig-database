...
详细操作参考 [[可靠性/灾难恢复/03-etcd-corruption-recovery-playbook.md|etcd 损坏恢复手册]]。恢复完成后，应持续观察 etcd 的 leader 切换次数、提交延迟与 compact 状态，确认集群已稳定。若原故障由存储损坏引起，需替换故障节点的磁盘并重新加入集群，避免再次触发相同问题。
...
- [[可靠性/灾难恢复/03-etcd-corruption-recovery-playbook.md|etcd 损坏恢复手册]]
- [[可靠性/灾难恢复/02-az-failure-playbook.md|可用区故障手册]]
- [[可靠性/灾难恢复/06-control-plane-loss-recovery-playbook.md|控制平面丢失恢复]]
- [[集群基础/控制平面/10-plane-backup-disaster-recovery.md|控制平面备份与灾难恢复]]
