---
title: System Foundation
description: 系统基础知识域 — Linux 架构、硬件基础设施、K8s 事件、速查卡、知识字典
summary: 系统基础知识域入口，涵盖 Linux 进程/文件系统/网络、硬件服务器/GPU/网络设备、K8s Events 全域事件、运维速查卡
category: domain
tags:
- linux
- hardware
- kernel
- kubernetes-events
- sysadmin
tier: core
created: '2026-05-23'
last_updated: '2026-07-21'
difficulty: intermediate
audience:
- 所有工程师
- SRE
estimated_read_time: 5min
---
# 系统基础 System Foundation

> Linux、硬件、K8s 事件、速查卡与知识字典。

## 二级子目录

| 子目录 | 内容 | 核心话题 |
|--------|------|----------|
| [[17-系统基础/01-Linux/README.md\|Linux/]] | Linux | 进程管理、文件系统、网络配置、内核参数 |
| [[17-系统基础/02-硬件/README.md\|硬件/]] | 硬件 | 服务器、GPU、网络设备、存储硬件 |
| [[17-系统基础/04-K8s事件/README.md\|K8s事件/]] | 事件 | K8s Events 全域事件大全、事件分析 |
| [[17-系统基础/05-速查卡/README.md\|速查卡/]] | 速查 | kubectl/网络/存储/安全速查表 |
| [[17-系统基础/06-知识字典/README.md\|知识字典/]] | 字典 | 术语定义、缩写解释、概念映射 |

## 跨域导航

- [[15-AI基础设施/README.md|AI基础设施]]
- [[16-专项技术/README.md|专项技术]]
- [[18-云厂商/README.md|云厂商]]
- [[11-发布变更/README.md|发布变更]]
- [[09-可观测性/README.md|可观测性]]
- [[12-可靠性/README.md|可靠性]]
- [[06-存储/README.md|存储]]
- [[08-安全/README.md|安全]]
- [[14-容器运行时/README.md|容器运行时]]
- [[02-工作负载/README.md|工作负载]]
- [[10-平台工程/README.md|平台工程]]
- [[04-应用模式/README.md|应用模式]]
- [[19-故障诊断/README.md|故障诊断]]
- [[07-数据库中间件/README.md|数据库中间件]]
- [[03-清单模式/README.md|清单模式]]
- [[13-生产运维/README.md|生产运维]]
- [[21-生态参考/README.md|生态参考]]
- [[05-网络/README.md|网络]]
- [[01-集群基础/README.md|集群基础]]
