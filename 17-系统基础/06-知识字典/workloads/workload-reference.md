...
## 概述
Workload Reference 是 [[23-实体/kubernetes.md|[[Kubernetes|kubernetes]]]] v1.35 引入的 Alpha 特性（默认禁用，需启用 `GenericWorkload` 特性门控）。它允许将 Pod 链接到一个 Workload 对象，使调度器能够按组进行协同调度决策，而不是将 Pod 视为独立个体。
...
- [[17-系统基础/06-知识字典/workloads/workload-api.md|Workload API]]](workload-api.md) — Workload 资源的 API 定义和结构
- [[17-系统基础/06-知识字典/workloads/pod-group-policies.md|Pod Group Policies]]](pod-group-policies.md) — basic 和 gang 策略详解
- [[17-系统基础/06-知识字典/workloads/jobs.md|Jobs]]](jobs.md) — Job 控制器与 Workload 配合使用
