...
## 概述
ReplicationController 是一种遗留 API，用于确保指定数量的 Pod 副本始终处于运行状态。它已被 Deployment 和 [[replicaset|ReplicaSet]] 取代，仅在维护旧系统或学习 [[kubernetes|Kubernetes]] 历史时可能遇到。
...
- **与 [[service|Service]] 配合**：多个 ReplicationController 可共享同一个 Service，实现金丝雀发布或多版本共存。
...
- 对于预期自行终止的任务，使用 Job；对于节点级服务，使用 [[daemonset|DaemonSet]]。
