...
[[23-实体/kubernetes.md|[[kubernetes|kubernetes]]]] 调度器（kube-scheduler）是控制平面的关键组件之一。配置错误的调度器可能产生安全影响，例如被用于针对特定节点并驱逐共享该节点及其资源的工作负载或应用，从而协助攻击者实施 **Yo-Yo 攻击**（针对脆弱自动扩缩容器的攻击）。本文档介绍如何提高调度器的安全态势。
