...
| 维度 | BFE | NGINX | Envoy |
|------|-----|-------|-------|
| 架构 | 模块化多租户（Go） | 进程模型 | xDS 动态 |
| 配置热加载 | 支持（无中断） | 需 reload | 支持（xDS） |
| 多租户隔离 | 原生支持 | 需插件 | 需配置 |
| 七层能力 | HTTP/2、TLS、路由、限流 | 成熟 | 最强（扩展） |
| 社区生态 | 百度主导，国内使用广 | 全球最大 | CNCF 顶级 |

**选型建议**：大规模流量接入 + 多租户场景选 BFE；标准 Ingress/反代用 NGINX；需要可编程扩展选 Envoy。

## 故障排查速查

| 症状 | 排查命令 | 常见根因 |
|------|----------|----------|
| 配置热加载失败 | `bfe -c <conf>` 校验；查看 log | YAML 语法错误、模块依赖缺失 |
| 流量不均衡 | 检查转发规则与权重 | 后端实例权重配置错误 |
| TLS 握手慢 | 检查证书链与协议配置 | 证书链不完整、会话复用关闭 |
| 租户间互相影响 | 查看租户资源隔离配置 | 共享连接池、限流未按租户拆分 |

## 生产部署清单

- [ ] 多租户配置（product/route）已规划并验证隔离
- [ ] 配置热加载流程走 CI（先校验后发布）
- [ ] TLS 证书自动化轮转（cert-manager/内部 CA）
- [ ] 监控接入（`bfe_*` metrics、访问日志落盘归档）
- [ ] 容量规划：按峰值 QPS 预留 30% 冗余

## 常见误区与设计要点

- **误区 1**：直接改线上配置不校验——BFE 配置热加载前务必用 `bfe -c` 本地校验。
- **误区 2**：所有租户共享一套限流——多租户场景必须按 product 隔离配额。
- **设计要点**：接入层与业务层分离（BFE 只做接入）；配置版本化走 GitOps；按地域拆分接入集群降低爆炸半径。

## 性能参考

- 单实例 QPS：纯转发场景 10w+（4C8G 规格），TLS 终结后约 5w。
- 连接数：单实例支持 100w+ 长连接（受 fd 与内存限制）。
- 延迟：P99 < 5ms（无后端竞争时）；开启限流/鉴权后增加 1-2ms。
- 扩展：水平扩容无状态，前置 L4 LB 分发即可线性扩展。
