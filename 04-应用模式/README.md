---
title: Application Patterns
description: 应用模式知识域 — 微服务子模式、生产级模式、行业架构案例（电商/小程序/IM/教育）
summary: 应用模式知识域入口，涵盖 Sidecar/Ambassador/Scatter-Gather 子模式、生产级部署模式、电商/金融/教育行业架构案例
category: domain
tags:
- application-architecture
- microservices
- patterns
- ecommerce
- production
tier: core
created: '2026-05-23'
last_updated: '2026-07-21'
difficulty: intermediate
audience:
- 所有工程师
- 架构师
estimated_read_time: 5min
---
# 应用模式 Application Patterns

> 子模式、行业架构与生产模式。

## 二级子目录

| 子目录 | 内容 | 核心话题 |
|--------|------|----------|
| [[04-应用模式/01-子模式/README.md\|子模式/]] | 微服务模式 | Sidecar/Ambassador/Scatter-Gather/CQRS |
| [[04-应用模式/03-生产模式/README.md\|生产模式/]] | 生产 | 蓝绿/金丝雀/滚动发布、健康检查、优雅关闭 |
| [[04-应用模式/02-行业架构/README.md\|行业架构/]] | 行业 | 电商/小程序/IM/教育/金融场景架构 |

## 跨域导航

- [[15-AI基础设施/README.md|AI基础设施]]
- [[16-专项技术/README.md|专项技术]]
- [[18-云厂商/README.md|云厂商]]
- [[11-发布变更/README.md|发布变更]]
- [[09-可观测性/README.md|可观测性]]
- [[12-可靠性/README.md|可靠性]]
- [[06-存储/README.md|存储]]
- [[08-安全/README.md|安全]]
- [[14-容器运行时/README.md|容器运行时]]
- [[02-工作负载/README.md|工作负载]]
- [[10-平台工程/README.md|平台工程]]
- [[19-故障诊断/README.md|故障诊断]]
- [[07-数据库中间件/README.md|数据库中间件]]
- [[03-清单模式/README.md|清单模式]]
- [[13-生产运维/README.md|生产运维]]
- [[21-生态参考/README.md|生态参考]]
- [[17-系统基础/README.md|系统基础]]
- [[05-网络/README.md|网络]]
- [[01-集群基础/README.md|集群基础]]
