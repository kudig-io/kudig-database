---
title: topic-scenarios MOC
description: topic-scenarios 专题导航页，覆盖 20 篇文档
summary: topic-scenarios 专题导航页，覆盖 20 篇文档
category: moc
tags:
- k8s
- moc
- k8s
- rag
- gpu
tier: supporting
created: '2026-05-23'
last_updated: '2026-05-21'
difficulty: intermediate
reading_level: intermediate
audience:
- 所有工程师
estimated_read_time: 5min
intent_queries:
- topic-scenarios MOC 是什么
- 如何 topic-scenarios MOC
- Kubernetes 11 production operations 最佳实践
trigger_keywords:
- topic-scenarios
- MOC
- production
- operations
- best
- practices
prerequisites:
- kubectl-basics
- gpu-ml-basics
- gpu-scheduling-basics
---

> **生产环境安全提示**
>
> 本文档包含可直接执行的运维命令。执行前请确认：当前目标集群与 Namespace 是否正确；是否具备足够的 RBAC 权限；是否已在非生产环境验证。命令风险等级标注：🔴 高风险（可能造成数据丢失或服务中断）、🟡 中风险（会修改集群状态，但通常可回滚）、🟢 低风险/只读（信息收集，无副作用）。




# topic-scenarios MOC.md|MOC]]

> **MOC 版本**: 1.0
> **专题**: topic-scenarios
> **文档数量**: 20 篇
> **最后更新**: 2026-05-21
> **用途**: 本专题的导航入口，汇总所有相关文档

---

## 专题概述

topic-scenarios 专题

### 专题定位

| 维度 | 说明 |
|---|---|
| **专题** | topic-scenarios |
| **文档数量** | 20 篇 |
| **难度分布** | 入门 0 / 进阶 0 / 高级 0 / 专家 0 |

---

## 文档清单

| # | 文档 | 难度 | 标签 | 估计阅读时间 |
|---|---|---|---|---|
| 1 | 场景: AI 基础设施运维 |  | k8s, scenario, ai |  |
| 2 | deployment.md|场景: 应用部署]] |  | k8s, scenario, deployment |  |
| 3 | 场景: 备份恢复 |  | k8s, scenario, backup-restore |  |
| 4 | 场景: 容量规划 |  | k8s, scenario, capacity-planning |  |
| 5 | 场景: 集群部署 |  | k8s, scenario, deployment |  |
| 6 | 场景: 合规审计 |  | k8s, scenario, compliance |  |
| 7 | 场景: 成本优化 |  | k8s, scenario, cost-optimization |  |
| 8 | 场景: 日常运维 |  | k8s, scenario, daily-ops |  |
| 9 | 场景: 边缘计算运维 |  | k8s, scenario, edge |  |
| 10 | 场景: GitOps 工作流 |  | k8s, scenario, gitops |  |
| 11 | 场景: Service Mesh 运维 |  | k8s, scenario, mesh |  |
| 12 | 场景: 监控告警 |  | k8s, scenario, monitoring |  |
| 13 | 场景: 多集群管理 |  | k8s, scenario, cloud |  |
| 14 | 场景: 网络诊断 |  | k8s, scenario, networking |  |
| 15 | 场景: 性能调优 |  | k8s, scenario, performance |  |
| 16 | 场景: 安全加固 |  | k8s, scenario, security |  |
| 17 | 场景: 安全事件响应 |  | k8s, scenario, security |  |
| 18 | 场景: 存储问题排查 |  | k8s, scenario, storage |  |
| 19 | 场景: 故障排查 |  | k8s, scenario, troubleshooting |  |
| 20 | 场景: 升级迁移 |  | k8s, scenario, migration |  |

---

## 统计信息

| 指标 | 数值 |
|---|---|
| 文档总数 | 20 |

---

*本文档由 scripts/generate-mocs.py 自动生成，最后更新 2026-05-21。*

## 场景分类导航

### 按运维阶段

| 阶段 | 场景 |
|---|---|
| 部署 | cluster-deployment, app-deployment |
| 运行 | daily-ops, monitoring-alerting, performance-tuning |
| 故障 | troubleshooting, network-diagnosis, storage-issues |
| 变更 | upgrade-migration, gitops-workflow |
| 安全 | security-hardening, compliance-audit, security-incident |
| 容灾 | backup-restore, multi-cluster |

### 按紧急程度

- **P0 紧急**：security-incident, troubleshooting
- **P1 重要**：backup-restore, upgrade-migration
- **P2 常规**：daily-ops, monitoring-alerting
- **P3 优化**：cost-optimization, performance-tuning

## 面试要点

1. **Q：如何选择运维场景的最佳实践？**
   A：根据业务影响、紧急程度、团队能力选择。P0 场景需自动化+Runbook，P3 场景可定期优化。

2. **Q：场景化运维的价值？**
   A：标准化流程、降低人为错误、加速故障恢复、知识传承、持续改进。

3. **Q：如何扩展新的运维场景？**
   A：识别场景→定义流程→编写 Runbook→自动化→培训→审计→迭代。

## Related

- [[17-系统基础/05-速查卡/k8s.md|k8s]]
- backup-restore
- cost-optimization
- network-diagnosis
- Wiki Lint Report — 2026-05-21 — Cross-reference
- [[23-实体/15-参考与索引/release-notes-storage.md|发布说明索引 — 存储]] — Cross-reference
- [[23-实体/15-参考与索引/release-notes-observability.md|发布说明索引 — 可观测性]] — Cross-reference
- [[23-实体/15-参考与索引/release-notes-networking.md|发布说明索引 — 网络]] — Cross-reference
- [[23-实体/15-参考与索引/release-notes-kubernetes.md|发布说明索引 — Kubernetes]] — Cross-reference
- [[23-实体/15-参考与索引/release-notes-security.md|发布说明索引 — 安全]] — Cross-reference
- [[23-实体/15-参考与索引/k8s-knowledge-map.md|Kubernetes Knowledge Map]] — Cross-reference
- [[23-实体/15-参考与索引/release-notes-cicd-gitops.md|发布说明索引 — CI/CD 与 GitOps]] — Cross-reference
- [[23-实体/15-参考与索引/release-notes-cli-tools.md|发布说明索引 — CLI 工具]] — Cross-reference
- [[23-实体/15-参考与索引/release-notes-core-deps.md|发布说明索引 — 核心依赖]] — Cross-reference
- [[23-实体/15-参考与索引/k8s-difficulty-index.md|Kubernetes Difficulty Index]] — Cross-reference
- 网络 MOC — Cross-reference
- [[05-网络/01-K8s网络核心/03-cni-architecture-fundamentals.md|CNI 架构与核心原理]] — Cross-reference
- [[09-可观测性/01-总览/01-observability-architecture-overview.md|Kubernetes 可观测性架构体系]] — Cross-reference
- [[15-AI基础设施/01-基础设施/03-gpu-scheduling-management.md|GPU 调度与管理]] — Cross-reference
- [[15-AI基础设施/01-基础设施/05-distributed-training-frameworks.md|分布式训练框架]] — Cross-reference
- 发布变更 MOC — Cross-reference
- [[01-集群基础/05-kubectl/02-kubectl-commands-reference.md|kubectl 命令完整参考]] — Cross-reference
- [[01-集群基础/01-架构总览/02-core-components-deep-dive.md|Kubernetes 核心组件深度剖析]] — Cross-reference
- [[06-存储/01-K8s存储/02-pv-architecture-fundamentals.md|PV/PVC 核心概念与企业级实践]] — Cross-reference
- [[06-存储/01-K8s存储/01-storage-architecture-overview.md|存储架构概览与核心组件]] — Cross-reference


<!-- risk-assessed -->
