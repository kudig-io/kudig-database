...
本手册聚焦华为云 CCE（Cloud Container Engine）生产运维的可执行命令与标准流程。CCE 作为华为云托管 Kubernetes 服务，在控制平面高可用、节点管理、网络、存储、安全与可观测性方面与开源 Kubernetes 有较大差异。建议与 [[云厂商/99-production-readiness-operations-guide.md|云厂商托管 Kubernetes 生产就绪运维指南]] 配套使用，形成跨云一致的 SRE 操作基线。
