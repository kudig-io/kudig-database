...
- **[[集群基础/README.md|集群基础架构]]** — 控制平面高可用、版本偏斜、API 废弃、节点生命周期由本域落地，但架构设计原则由该域定义。
- **[[网络/README.md|网络与流量]]** — VPC/CNI、负载均衡、Ingress、Service Mesh、跨云网络互联依赖该域的网络模型与排障方法。
- **[[安全/README.md|安全合规]]** — IAM/workload identity、NetworkPolicy、Pod Security Standards、镜像签名与审计在该域与云 IAM 之间形成交叉责任面。
- **[[可观测性/README.md|可观测性]]** — Metrics/Logs/Traces 采集、告警体系、SLI/SLO 定义由该域提供方法论，本域负责对接云厂商监控服务（CloudWatch、SLS、Azure Monitor）。
- **[[平台工程/README.md|平台工程]]** — GitOps、多集群 fleet 管理、标准化交付模板、FinOps 由该域主导，本域提供各云厂商的具体实现参数。
- **[[可靠性/README.md|可靠性工程]]** — 灾难恢复、备份演练、混沌工程、SLO/SLA 由该域统筹，本域补充厂商特定的 RTO/RPO 方案。
- **[[生产运维/README.md|生产运维]]** — 值班、变更管理、事件响应流程由该域定义，本域提供云厂商相关的 runbook 与回滚命令。
...
- [[集群基础/99-kubernetes-production-architecture-blueprint.md|Kubernetes 生产架构蓝图]] — 生产集群整体架构设计原则。
- [[安全/README.md|安全合规域]] — RBAC、NetworkPolicy、Pod Security、镜像安全与审计。
- [[可靠性/README.md|可靠性工程域]] — 灾备、备份演练、SLO/SLA 与混沌工程。
