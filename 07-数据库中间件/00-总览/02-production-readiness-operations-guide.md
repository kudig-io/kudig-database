...
在将任一数据库或中间件实例标记为 `Production Ready` 之前，建议逐项核对以下检查点。本清单基于 [[07-数据库中间件/01-数据库/04-database-middleware-kubernetes.md|数据库中间件 Kubernetes 企业级实践]] 与 [[07-数据库中间件/05-Operator管理/01-database-operator-patterns.md|数据库 Operator 设计模式]] 中的生产要求提炼而成。
...
- **[[06-存储/README.md|存储]]（存储与数据）**: 负责 StorageClass、CSI 驱动、PVC 扩容、快照与备份存储。数据库层应复用其存储治理与容量规划结论，不自行维护底层存储 SOP。
- **[[08-安全/README.md|安全]]（安全合规）**: 负责 NetworkPolicy 默认拒绝、Secret/凭据生命周期、Pod Security Standards、TLS/mTLS 策略。数据库安全加固的具体实践见 数据库安全加固（待补充）。
- **[[09-可观测性/README.md|可观测性]]（可观测性）**: 负责 Prometheus、Grafana、Loki、Jaeger 的统一采集与告警路由。数据库层需提供Exporter/ServiceMonitor，并定义自身的 SLO/SLI，详情参考 数据库 SLO 与容量规划（待补充）。
- **[[10-平台工程/README.md|平台工程]]（平台工程）**: 负责 Operator 注册、多租户命名空间模板、GitOps 交付、资源配额。数据库实例通过平台目录发布，避免业务团队自行部署。
- **[[12-可靠性/README.md|可靠性]]（可靠性工程）**: 负责跨域灾难恢复、PDB、混沌演练、RTO/RPO 定义。数据库的跨集群 DR 方案见 数据库多集群灾备手册（待补充）。
- **[[13-生产运维/README.md|生产运维]]（生产运维）**: 负责事件响应、值班手册、变更管理。数据库 P0/P1 故障应接入统一事件流程。
- **[[15-AI基础设施/README.md|AI基础设施]]（AI/ML 基础设施）**: 负责 AI 数据管道、特征存储、向量数据库集成。当数据库层为 AI 工作负载提供数据服务时，需协同评估高并发读取与向量索引性能。
...
- [[07-数据库中间件/01-数据库/01-mysql-enterprise-database.md|MySQL 企业级数据库运维管理]]
- [[07-数据库中间件/01-数据库/02-postgresql-enterprise-database.md|PostgreSQL 企业级数据库运维管理]]
- [[07-数据库中间件/01-数据库/04-database-middleware-kubernetes.md|数据库中间件 Kubernetes 企业级实践]]
- [[07-数据库中间件/01-数据库/06-redis-enterprise-cache.md|Redis 企业级缓存]]
- [[07-数据库中间件/01-数据库/08-kafka-kubernetes-strimzi.md|Kafka Kubernetes Strimzi 实践]]
- [[07-数据库中间件/01-数据库/19-cloudnativepg-enterprise-guide.md|CloudNativePG 企业指南]]
- [[07-数据库中间件/03-消息队列/03-message-queue-comparison.md|消息队列选型]]
- [[07-数据库中间件/04-时序数据库/01-prometheus-tsdb-deep-dive.md|Prometheus TSDB 深度解析]]
- [[07-数据库中间件/05-Operator管理/01-database-operator-patterns.md|数据库 Operator 设计模式]]
- [[07-数据库中间件/05-Operator管理/03-operator-lifecycle-management.md|Operator 生命周期管理]]
- [[07-数据库中间件/06-数据流/01-cdc-change-data-capture.md|CDC 变更数据捕获]]
...
- [[06-存储/README.md|存储]] — 存储基础与 PVC 治理
- [[08-安全/README.md|安全]] — 安全基线与合规
- [[09-可观测性/README.md|可观测性]] — 监控告警与 SLO
- [[12-可靠性/README.md|可靠性]] — 灾难恢复与韧性设计
- [[13-生产运维/README.md|生产运维]] — 事件响应与值班手册
