---
title: Database & Middleware
description: 数据库与中间件知识域 — 关系型/NoSQL 数据库、缓存、消息队列、时序数据库、搜索引擎、Operator 管理、数据流
summary: 数据库与中间件知识域入口，涵盖 MySQL/PostgreSQL/MongoDB、Redis、Kafka/Pulsar、Prometheus/InfluxDB、Elasticsearch、Operator 管理与数据流
category: domain
tags:
- database
- middleware
- message-queue
- time-series
- operator
- streaming
- search-engine
- cache
tier: core
created: '2026-05-23'
last_updated: '2026-07-21'
difficulty: intermediate
audience:
- 所有工程师
estimated_read_time: 5min
---
# 数据库中间件 Database & Middleware

> 数据库、缓存、消息队列、时序数据库、搜索引擎、Operator 管理与数据流。

## 二级子目录

| 子目录 | 内容 | 核心话题 |
|--------|------|----------|
| [[07-数据库中间件/01-数据库/index.md\|数据库/]] | 关系型/NoSQL | MySQL/PostgreSQL/MongoDB/TiDB |
| [[07-数据库中间件/02-缓存/index.md\|缓存/]] | 缓存系统 | Redis/Memcached/缓存策略 |
| [[07-数据库中间件/03-消息队列/index.md\|消息队列/]] | 消息与事件 | Kafka/Pulsar/NATS/RabbitMQ |
| [[07-数据库中间件/04-时序数据库/index.md\|时序数据库/]] | 时序数据 | Prometheus/InfluxDB/VictoriaMetrics/Thanos |
| [[07-数据库中间件/07-搜索引擎/index.md\|搜索引擎/]] | 搜索与分析 | Elasticsearch/OpenSearch/向量搜索 |
| [[07-数据库中间件/05-Operator管理/index.md\|Operator管理/]] | Operator 模式 | 数据库 Operator 设计与对比 |
| [[07-数据库中间件/06-数据流/index.md\|数据流/]] | 流处理 | Flink/Spark Streaming/CDC |
| [[07-数据库中间件/08-新型数据库/index.md\|新型数据库/]] | 新型数据库 | NewSQL/图数据库/向量数据库 |

## 跨域导航

- [[15-AI基础设施/README.md|AI基础设施]]
- [[16-专项技术/README.md|专项技术]]
- [[18-云厂商/README.md|云厂商]]
- [[11-发布变更/README.md|发布变更]]
- [[09-可观测性/README.md|可观测性]]
- [[12-可靠性/README.md|可靠性]]
- [[06-存储/README.md|存储]]
- [[08-安全/README.md|安全]]
- [[14-容器运行时/README.md|容器运行时]]
- [[02-工作负载/README.md|工作负载]]
- [[10-平台工程/README.md|平台工程]]
- [[04-应用模式/README.md|应用模式]]
- [[19-故障诊断/README.md|故障诊断]]
- [[03-清单模式/README.md|清单模式]]
- [[13-生产运维/README.md|生产运维]]
- [[21-生态参考/README.md|生态参考]]
- [[17-系统基础/README.md|系统基础]]
- [[05-网络/README.md|网络]]
- [[01-集群基础/README.md|集群基础]]
