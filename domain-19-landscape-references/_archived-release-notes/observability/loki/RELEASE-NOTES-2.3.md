---
title: loki v2.3 Release Notes
description: loki v2.3 Release Notes — Kubernetes 生产运维知识库
summary: loki v2.3 Release Notes — Kubernetes 生产运维知识库
...
intent_queries:
- loki v2.3 Release Notes 是什么
- 如何 loki v2.3 Release Notes
trigger_keywords:
- loki
- v2.3
- Release
...
# loki v2.3 Release Notes

Source: [v2.3.0](https://github.com/grafana/loki/releases/tag/v2.3.0)
...
* Loki now has the ability to apply [custom retention](https://grafana.com/docs/loki/v2.3.0/operations/storage/retention/) based on stream selectors! This will allow much finer control over log retention all of which is now handled by Loki, no longer requiring the use of object store configs for retention.
* Coming along hand in hand with storing logs for longer durations is the ability to [delete log streams](https://grafana.com/docs/loki/v2.3.0/operations/storage/logs-deletion/). The initial implementation lets you submit delete request [[Jobs|jobs]] which will be processed after 24 hours.
* A very exciting new LogQL parser has been introduced: the [pattern parser](https://grafana.com/docs/loki/v2.3.0/logql/#parser-expression). Much simpler and faster than regexp for log lines that have a little bit of structure to them such as the [Common Log Format](https://en.wikipedia.org/wiki/Common_Log_Format). This is now Loki's fastest parser so try it out on any of your log lines!
* Extending on the work of Alerting Rules, Loki now accepts [recording rules](https://grafana.com/docs/loki/v2.3.0/rules/#recording-rules). This lets you turn your logs into metrics and push them to [[Prometheus|Prometheus]] or any Prometheus compatible remote_write endpoint.
* LogQL can understand [IP addresses](https://grafana.com/docs/loki/v2.3.0/logql/ip/)! This enables filtering on IP addresses and subnet ranges.
...
* We created an index gateway](https://grafana.com/docs/loki/v2.3.0/operations/storage/boltdb-shipper/#index-gateway) which takes on the task of downloading the boltdb-shipper index files allowing you to run your queriers without any local disk requirements, this is really helpful in Kubernetes environments where you can return your queriers from Statefulsets back to Deployments and save a lot of PVC costs and operational headaches.
...
For the full list of changes (almost 300!!) check out the [CHANGELOG](https://github.com/grafana/loki/blob/main/CHANGELOG.md#230-20210806)
