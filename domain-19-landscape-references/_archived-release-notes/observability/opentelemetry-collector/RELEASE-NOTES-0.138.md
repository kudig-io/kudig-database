...
summary: opentelemetry-collector v0.138 Release Notes — Kubernetes 生产运维知识库
...
- `all`: Add unified print-config command with mode support (redacted, unredacted), json support (unstable), and validation support. (#11775)
  This replaces the `print-initial-config` command. See the `[[Service|service]]` package README for more details. The original command name `print-initial-config` remains an alias, to be retired with the feature flag.
