...
Before version 1.2, you could clone a [[Kubernetes|Kubernetes]] namespace by backing it up and then restoring it to a different namespace in the same cluster by using the `--namespace-mappings` flag with the `velero restore create` command. However, in this scenario, Velero was unable to clone [[domain-17-system-foundation/topic-dictionary/storage/persistent-volumes|persistent volumes]] used by the namespace, leading to errors for users.
