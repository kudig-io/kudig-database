---
title: kind v0.14 Release Notes
description: kind v0.14 Release Notes — Kubernetes 生产运维知识库
summary: kind v0.14 Release Notes — Kubernetes 生产运维知识库
...
intent_queries:
- kind v0.14 Release Notes 是什么
- 如何 kind v0.14 Release Notes
trigger_keywords:
- kind
- v0.14
- Release
...
- **systemd cgroups driver will be used for [[Kubernetes|Kubernetes]] v1.24.0+** (rather than 1.21.0+ when [kubeadm changed the default](https://github.com/kubernetes/kubernetes/pull/99471), which we previously overrode).
...
with a previous kind version you will need to re-built when switching to kind v0.13.0+.
  - **NOTE**: **You do not need to be using systemd on the host machine**, In kind v0.13.0 we had a bug related to this, it should be fixed this release. systemd is used *inside* the kind node containers but should not be necessary on the host. We are now using it for Kubernetes [[Pods|pods]] in addition to running [[kubelet|kubelet]], containerd etc.
