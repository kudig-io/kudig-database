...
summary: kind v0.26 Release Notes — Kubernetes 生产运维知识库
...
This release brings [[Kubernetes|Kubernetes]] v1.32.0 to kind by default and fixes `kind build node-image` for building for Kubernetes releases <v1.31.0 when not building Kubernetes from source (e.g. `kind build node-image v1.30.0`).
