#!/usr/bin/env python3
"""
第二轮修复剩余 broken wikilinks。
更宽松的匹配策略，并处理 .comate/ 等目录中的伪链接。
"""
...
def is_excluded(rel: str) -> bool:
    excluded = (
        ...
        '_archives/', '_raw/', '_staging/', '_reports/',
    )
...
def main():
    ...
    print("\nScanning remaining broken links (excluding _reports)...")
    ...
    for src, target in broken:
        ...
        # .comate/ 等工具目录中的伪链接直接转文本
        if src.startswith('.comate/') or src.startswith('.claude/') or src.startswith('.codebuddy/'):
            if fix_link_in_file(src_path, target, '', 'text'):
                converted.append((src, target, 'tool-config pseudo-link'))
            else:
    ...
    print(f"\nRemaining broken links after fix: {len(remaining)}")

    # 报告
    report_out = vault / '_reports/broken-links-round2-fix-2026-06-26.md'
    lines = [
        "---",
        "title: 第二轮 Broken Wikilinks 修复报告（2026-06-26）",
        "description: 修复剩余 83 个 broken wikilinks",
        "category: reports",
        "tags:",
        "- wiki-lint",
        "- broken-links",
