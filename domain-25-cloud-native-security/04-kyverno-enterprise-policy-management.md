...
echo "## Summary" >> "$REPORT_DIR/$DATE/report.md"
...
echo "## Cluster-level Violations" >> "$REPORT_DIR/$DATE/report.md"
...
echo "" >> "$REPORT_DIR/$DATE/report.md"
echo "## Namespace-level Violations" >> "$REPORT_DIR/$DATE/report.md"
for ns in $(kubectl get namespaces -o jsonpath='{.items[*].metadata.name}'); do
  violations=$(kubectl get policyreport -n "$ns" -o json 2>/dev/null | \
    jq -r '.results[] | select(.result == "fail")')
  if [ -n "$violations" ]; then
    echo "### Namespace: $ns" >> "$REPORT_DIR/$DATE/report.md"
    echo "| Policy | Rule | Resource | Message |" >> "$REPORT_DIR/$DATE/report.md"
    echo "|:---|:---|:---|:---|" >> "$REPORT_DIR/$DATE/report.md"
    echo "$violations" | \
...
echo "## Policy Exceptions" >> "$REPORT_DIR/$DATE/report.md"
