...
这个设计揭示了 [[系统基础/速查卡/k8s.md|K8s]] 的核心架构决策：存储置备不应该先于调度。如果先置备存储（Immediate 模式），Pod 可能被调度到与 PV 不同可用区的节点，导致挂载失败。WaitForFirstConsumer 将存储模型的决策延迟到 Pod 生命周期中的调度阶段，解决了这一矛盾。
