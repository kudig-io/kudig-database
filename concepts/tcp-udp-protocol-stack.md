...
DNS (typically UDP port 53) resolves names to IP addresses through recursive and iterative queries. In K8s, CoreDNS handles in-cluster DNS resolution, translating Service names to ClusterIPs and Pod names to Pod IPs. DNS failures are a common source of Service connectivity issues.
...
| L7 (Application) | Content-based routing | Nginx, HAProxy | Ingress Controller |
