...
tier: supporting
relationships:
- target: "[[entities/deployment.md|deployment]]"
  type: uses
---
...
- **TLS 证书挂载**：[[domain-17-system-foundation/topic-dictionary/networking/ingress.md|Ingress]] Controller 或 API 服务的 Deployment 通过 Secret 卷挂载 TLS 证书。证书到期后需要滚动更新 Deployment 才能重新加载
...
| **Secret 大小限制** | K8s Secret 大小限制为 1MB（[[entities/etcd.md|etcd]] 的 value 大小限制）。大型 TLS 证书链或 CA 捆绑包可能超过此限制，需要拆分为多个 Secret 或使用 ConfigMap |
...
- **Secret 与 Pod 安全标准的冲突**：[[skills/best-practices/best-practices/security/pod-security.md|Pod Security]] Standards（Restricted）禁止以 root 运行、要求只读根文件系统。但某些旧版应用读取 Secret 文件时需要特定权限，导致安全策略与应用需求的冲突
