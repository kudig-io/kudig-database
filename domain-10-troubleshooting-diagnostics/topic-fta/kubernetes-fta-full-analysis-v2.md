...
> **文档版本**: v2.0 Enhanced
> **适用范围**: Kubernetes 生产环境全量故障场景 + ACK 特有场景
> **更新日期**: 2026-05-18
...
| TE-9 | Terway 网络故障 | 🟠 P1 | Pod 网络 | Pod 无法获取 IP/网络不通 | Terway 独有 |
| TE-10 | ASM 服务网格故障 | 🟠 P1 | 网格流量 | .sidecar 无法连接/mTLS 失败 | ASM 独有 |
| TE-11 | ACK-One 多集群异常 | 🟠 P1 | 多集群 | 集群注册失败/配置同步延迟 | ACK-One 独有 |
| TE-12 | 资源配额超限 | 🟡 P2 | 账户级 | API 对象创建失败/配额耗尽 | ACK 独有 |
| TE-13 | 变更管理故障 | 🟠 P1 | 变更过程 | 升级失败/回滚/配置漂移 | GitOps/RAC |
...
│       └── BE-1.4.2 控制器循环故障
│
├── IE-1.2 工作节点批量故障 [AND门 - 需多数节点同时故障]
│   ├── BE-1.5 Kubelet服务故障
...
│       └── BE-2.10.2 后端权重配置错误
│
├── IE-2.4 ASM 服务网格故障 [OR门] ← ACK 特有层
│   ├── BE-2.11 Envoy sidecar 故障
...
│
└── IE-2.5 ARMS 应用监控故障 [OR门] ← ACK 特有层
...
│
└── IE-8.4 ARMS/MSP 特有故障 [OR门] ← ACK 特有层
    ├── BE-8.9 ACK 托管 Prometheus 故障
    │   ├── BE-8.9.1 MSP 服务临时不可用
    │   │   └── BE-8.9.1.1 阿里云 MSP 控制面故障导致采集中断
...
<!-- chunk: 十、TE-9: Terway 网络故障 🟠 P1 （新增） -->## 十、TE-9: Terway 网络故障 🟠 P1 （新增）
...
<!-- chunk: 十一、TE-10: ASM 服务网格故障 🟠 P1 （新增） -->## 十一、TE-10: ASM 服务网格故障 🟠 P1 （新增）

```
TE-10: ASM 服务网格故障 [OR门] 🟠 P1
│
├── IE-10.1 数据面故障 [OR门]
...
│
├── IE-10.2 控制面故障 [OR门]
│   ├── BE-10.3 Istiod (Pilot/ Citadel/ Galley) 故障
...
│
└── IE-10.5 ASM 特有配置故障 [OR门]
    ├── BE-10.10 阿里云 ASM 控制面故障
...
<!-- chunk: 十四、TE-13: 变更管理故障 🟠 P1 （新增） -->## 十四、TE-13: 变更管理故障 🟠 P1 （新增）
...
│
├── IE-16.4 OpenTelemetry 集成故障 [OR门]
...
#<!-- chunk: 18.2 故障传播路径示例 -->## 18.2 故障传播路径示例
...
| IaaS 层故障 | 无 | ECS/ENI/ESSD/SLB/VPC |
| 服务网格故障 | 无 | ASM/Istiod/Envoy/xDS |
| 多集群故障 | 无 | ACK-One/Federation/GitOps |
| 容量/DR 故障 | 无 | 备份/恢复/扩容 |
