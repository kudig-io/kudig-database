...
- calico FTA 树：Calico CNI 故障诊断 故障排查
- calico FTA 树：Calico CNI 故障诊断 排障步骤
...
title: [[skills/calico-fta.md|Calico Fta]] 树：Calico CNI 故障诊断
...
```mermaid
graph TD
    A["故障: Calico 网络异常<br/>Pod 无法通信 / 网络策略不生效 / BGP 会话断开"]'"]
    B{"检查组件状态"}
    C["修复: calico 配置/重启"]
    D{"检查日志和事件"}
    E["修复: calico 深度诊断"]
