...
<!-- condition: kubectl get pods -n [[entities/istio.md|Istio]]-system -o jsonpath='{range .items[?(@.status.phase!="Running")]}{.metadata.name}{\"\n\"}{end}' 显示 Istio 控制面异常 -->
...
flowchart TD
  ...
  A_OR --> A3["A3. istiod 版本不兼容<br/>与 [[domain-17-system-foundation/topic-cheat-sheet/k8s.md|k8s]]/Envoy 版本冲突"]
...
```mermaid
graph TD
    A["故障: Service Mesh 异常<br/>流量中断 / 注入失败 / 策略不生效"]"]
    B{"检查组件状态"}
    C["修复: service-mesh-istio 配置/重启"]
    D{"检查日志和事件"}
    E["修复: service-mesh-istio 深度诊断"]
    F{"检查资源配置"}
    G["修复: service-mesh-istio 专项处理"]
    I["验证修复"]
    J["记录根因，关闭"]
    H["升级到专家"]
