...
- kubeadm FTA 树：集群生命周期故障诊断 故障排查
- kubeadm FTA 树：集群生命周期故障诊断 排障步骤
...
title: [[skills/kubeadm-fta.md|Kubeadm Fta]] 树：集群生命周期故障诊断
...
```mermaid
graph TD
    A["故障: kubeadm 操作异常<br/>init/join/reset/upgrade 失败"]"]
    B{"检查组件状态"}
    C["修复: kubeadm 配置/重启"]
    D{"检查日志和事件"}
    E["修复: kubeadm 深度诊断"]
