  ...
  "flow_steps": [
    ...
    { "name": "版本异常 OR 门", "action": "gate_or", "step": "gate_cp_ver_or", "control": "or_gate", "gate_type": "OR", "next_steps": ["event_cp_ver_skip", "event_cp_ver_mismatch"] },
    {
      ...
      "metadata": {
        ...
        "remediation": {
          "manual_steps": ["[[domain-17-system-foundation/topic-cheat-sheet/k8s.md|k8s]] 仅支持逐版本升级 (N -> N+1)", "必须先升级到中间版本", "检查升级路径文档"],
          "auto_actions": []
        },
        "version_notes": {
...
graph TD
    A["故障: 集群升级异常]"]
    B{"检查组件状态"}
    C["修复: cluster-upgrade 配置/重启"]
    D{"检查日志和事件"}
    E["修复: cluster-upgrade 深度诊断"]
    F{"检查资源配置"}
    G["修复: cluster-upgrade 专项处理"]
    I["验证修复"]
    J["记录根因，关闭"]
    H["升级到专家"]
