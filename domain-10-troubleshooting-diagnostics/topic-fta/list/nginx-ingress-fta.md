...
cross_refs:
- type: domain
  path: ../domain-03-networking-traffic/21-nginx-ingress-complete-guide.md
  label: '核心文档: 21-nginx-ingress-complete-guide'
- type: index
  path: ../../domain-19-landscape-references/topic-index/nginx-ingress-index.md
...
```mermaid
graph TD
    A["故障: nginx-ingress 异常]"]
    B{"检查组件状态"}
    C["修复: nginx-ingress 配置/重启"]
    D{"检查日志和事件"}
    E["修复: nginx-ingress 深度诊断"]
    F{"检查资源配置"}
    G["修复: nginx-ingress 专项处理"]
    I["验证修复"]
    J["记录根因，关闭"]
    H["升级到专家"]
...
- [[domain-19-landscape-references/topic-index/nginx-ingress-index.md|nginx-ingress-index]]
