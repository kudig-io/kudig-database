...
cross_refs:
- type: domain
  path: ../domain-01-cluster-fundamentals/13-kube-controller-manager-deep-dive.md
  label: '深度文档: 13-kube-controller-manager-deep-dive'
- type: structural
  path: ../domain-10-troubleshooting-diagnostics/topic-structural-trouble-shooting/01-control-plane/04-controller-manager-troubleshooting.md
...
```mermaid
graph TD
    A["故障: Controller Manager 异常]"]
    B{"检查组件状态"}
    C["修复: controller-manager 配置/重启"]
    D{"检查日志和事件"}
    E["修复: controller-manager 深度诊断"]
    F{"检查资源配置"}
    G["修复: controller-manager 专项处理"]
    I["验证修复"]
    J["记录根因，关闭"]
    H["升级到专家"]
