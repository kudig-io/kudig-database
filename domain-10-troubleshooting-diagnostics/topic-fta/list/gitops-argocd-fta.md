...
<!-- condition: [[entities/argocd.md|argocd]] app list 2>/dev/null | grep -E 'OutOfSync|Error|Degraded' 显示 ArgoCD 应用异常 -->
...
```mermaid
graph TD
    A["故障: GitOps 同步异常<br/>Sync 失败 / 漂移 / 回滚失败"]"]
    B{"检查组件状态"}
    C["修复: gitops-argocd 配置/重启"]
    D{"检查日志和事件"}
    E["修复: gitops-argocd 深度诊断"]
    F{"检查资源配置"}
    G["修复: gitops-argocd 专项处理"]
    I["验证修复"]
    J["记录根因，关闭"]
    H["升级到专家"]
