...
cross_refs:
- type: domain
  path: ../domain-02-workloads-applications/02-deployment-production-patterns.md
...
- type: structural
  path: ../domain-10-troubleshooting-diagnostics/topic-structural-trouble-shooting/05-workloads/02-deployment-troubleshooting.md
...
```mermaid
graph TD
    A["故障: Deployment 更新异常]"]
    B{"检查组件状态"}
    C["修复: deployment 配置/重启"]
    D{"检查日志和事件"}
    E["修复: deployment 深度诊断"]
    F{"检查资源配置"}
    G["修复: deployment 专项处理"]
    I["验证修复"]
    J["记录根因，关闭"]
    H["升级到专家"]
