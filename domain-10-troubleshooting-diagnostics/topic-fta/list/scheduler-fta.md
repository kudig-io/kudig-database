...
cross_refs:
- type: domain
  path: ../domain-01-cluster-fundamentals/20-kube-scheduler-deep-dive.md
  label: '深度文档: 20-kube-scheduler-deep-dive'
- type: structural
  path: ../domain-10-troubleshooting-diagnostics/topic-structural-trouble-shooting/01-control-plane/03-scheduler-troubleshooting.md
...
```json
{
  "flow_steps": [
    ...
    { "name": "网络分区", "action": "event", "step": "evt_network_partition", "severity": "critical", "probability": "rare", "mttr_minutes": 30, "detection": { "events": [], "metrics": ["apiserver_request_duration_seconds 异常高"], "logs": ["kube-scheduler: context deadline exceeded"] }, "remediation": { "manual_steps": ["检查控制面网络", "排查网络设备故障"], "auto_actions": ["网络恢复后自动重连"] } },
