...
cross_refs:
- type: structural
  path: ../domain-10-troubleshooting-diagnostics/topic-structural-trouble-shooting/08-cluster-operations/05-crd-operator-troubleshooting.md
  ...
  "flow_steps": [
    ...
    },
    {
      ...
      "metadata": {
        ...
        "version_notes": "1.25+ 强制 structural schema，不允许 x-kubernetes-preserve-unknown-fields: true 在顶层"
      }
    },
    {
    ...
    },
    {
      ...
      "metadata": {
        ...
        "version_notes": "1.20+ 建议使用 coordination.[[domain-17-system-foundation/topic-cheat-sheet/k8s.md|k8s]].io/v1 Lease 替代 ConfigMap/Endpoint 选主"
      }
    },
    {
...
```mermaid
graph TD
    A["故障: CRD/Operator 异常<br/>资源不收敛 / CR 操作失败"]"]
    B{"检查组件状态"}
    C["修复: crd-operator 配置/重启"]
    D{"检查日志和事件"}
    E["修复: crd-operator 深度诊断"]
    F{"检查资源配置"}
    G["修复: crd-operator 专项处理"]
    I["验证修复"]
    J["记录根因，关闭"]
    H["升级到专家"]
...
- [[log.md|log]]
- [[domain-17-system-foundation/topic-dictionary/security/pod-security-admission.md|pod-security-admission]]
