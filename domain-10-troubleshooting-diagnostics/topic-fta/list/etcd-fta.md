---
title: etcd 异常 FTA 树
description: 'description: ''- **范围**：成员可用性、读写性能、磁盘与 IO、网络与时钟、证书与访问控制、碎片与压缩。'''
category: fta
tags:
- k8s
- fault-tree
...
agent_notes:
  decision_tree_entry: kubectl get pods -n kube-system -l component=etcd 检查 etcd Pod
    状态
  critical_commands:
...
```json
{
  "flow_steps": [
    ...
    { "name": "节点宕机", "action": "event", "step": "evt_node_crash", "severity": "critical", "probability": "rare", "mttr_minutes": 30, "detection": { "events": ["NodeNotReady"], "metrics": ["etcd_server_has_leader == 0", "up{job=\"etcd\"} == 0"], "logs": ["etcd: member unreachable"] }, "remediation": { "manual_steps": ["检查节点状态", "恢复或替换故障节点"], "auto_actions": ["自动故障转移"] } },
