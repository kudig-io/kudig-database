...
| 主题 | 引用路径 | 适用场景 |
|------|---------|---------|
| Prometheus 架构与 TSDB | `domain-06-observability/` | 理解 TSDB 存储机制、高基数问题根因 |
| AlertManager 告警路由 | `domain-06-observability/` | 深入理解路由、抑制、静默机制 |
| 监控告警排障方法论 | `domain-10-troubleshooting-diagnostics/30-monitoring-alerting-troubleshooting.md` | 系统化排障方法 |
| 告警风暴治理 | `domain-06-observability/05-alert-noise-reduction.md` | 告警降噪最佳实践 |
| Thanos 架构 | `domain-06-observability/` | 理解 Thanos 组件交互和查询路径 |
| 高基数指标治理 | `domain-06-observability/` | 指标设计最佳实践和治理方案 |
| ServiceMonitor 配置 | `domain-15-specialized-tech/` | Prometheus Operator CRD 使用指南 |
| Grafana 数据源管理 | `domain-06-observability/` | Grafana 配置和排障 |
