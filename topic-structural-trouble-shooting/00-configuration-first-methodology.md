...
在生产环境的日常运维中，绝大多数故障可以通过标准排查流程快速定位。但当遇到**症状模糊、影响范围不清晰、多个组件交叉关联**的疑难问题时，SRE 工程师常常陷入"东查一下、西查一下"的无序排查状态，浪费大量时间在网络链路、内核参数等深层排查上，而忽略了最简单也最高频的根因——**配置错误**。
...
│  ├── 适用于：疑难问题、复杂故障、多组件交叉故障                              │
│  ├── 解决：排查顺序问题（先查什么？后查什么？）                              │
│  └── 输出：结构化排查路径 + 配置验证清单                                   │
├─────────────────────────────────────────────────────────────────────────┤
│  topic-fta/          → 为什么出问题（故障树因果分析，演绎法）                 │
│  topic-febm/         → 如何从证据推导结论（取证循证，归纳法）                 │
│  topic-structural/   → 具体怎么查（按组件的详细排查步骤）                    │
│  topic-skills/       → Agent 怎么做（自动化诊断-修复闭环）                  │
└─────────────────────────────────────────────────────────────────────────┘
```
...
**近一半的故障根因是配置错误**，而配置检查是所有排查手段中成本最低、速度最快的。如果跳过配置检查直接深入网络/内核排查，平均会浪费 30-120 分钟。
...
工单/告警输入
    ...
    ▼
┌──────────────────────┐
│ Skill 路由（症状匹配）  │  ← topic-skills/ YAML front matter
└──────────┬───────────┘
           │
           ▼
┌──────────────────────┐
...
└──────────┬───────────┘
           ...
           ▼
┌──────────────────────┐
│ FTA 故障树遍历         │  ← topic-fta/ 因果关系模型
└──────────┬───────────┘
           │
           ▼
┌──────────────────────┐
│ 修复 → 验证 → 闭环    │  ← topic-skills/ Section 6-7
└──────────────────────┘
```
...
| 资源 | 路径 | 关系 |
|------|------|------|
| **DNS 故障树分析** | [topic-fta/list/dns-fta.md](../topic-fta/list/dns-fta.md) | FTA 因果分析模型 |
| **DNS 结构化排查指南** | [03-networking/02-dns-troubleshooting.md](./03-networking/02-dns-troubleshooting.md) | 详细排查步骤 |
| **DNS 故障排查（domain-12）** | [domain-12-troubleshooting/26-dns-troubleshooting.md](../domain-12-troubleshooting/26-dns-troubleshooting.md) | 按组件分类的完整指南 |
| **DNS Skill（Agent 可执行）** | [topic-skills/04-dns-resolution-failure.md](../topic-skills/04-dns-resolution-failure.md) | Agent 运行时 Runbook |
| **FEBM 取证方法论** | [topic-febm/](../topic-febm/) | 事后复盘取证分析 |
| **FTA 方法论合集** | [topic-fta/fta-methodology-and-agentic-practices.md](../topic-fta/fta-methodology-and-agentic-practices.md) | 故障树分析完整方法论 |
