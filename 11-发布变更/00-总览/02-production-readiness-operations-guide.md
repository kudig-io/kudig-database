...
本指南聚焦 [[11-发布变更/README.md|发布与变更管理]] 域，覆盖 GitOps、CI/CD、IaC、发布策略、变更回滚与质量保障。目标是让 SRE 在投产前、投产中与日常运维中，能够快速确认该域是否满足生产要求，并在异常时有章可循。
...
- 与 [[09-可观测性/README.md|可观测性域]] 的 Prometheus/Tempo 联动，确保分析模板可访问实时指标。
...
```
更多回滚场景参见 [[11-发布变更/04-变更管理/03-change-rollback-playbook.md|变更回滚操作手册]]。
...
- **[[10-平台工程/README.md|平台工程]]（平台工程）**：负责平台组件（Argo CD、Harbor、Tekton、镜像仓库）的部署、升级与容量规划；本域聚焦这些工具之上的发布流程与策略。
- **[[12-可靠性/README.md|可靠性]]（可靠性工程）**：提供 SLO、PDB、混沌工程、灾备恢复与回滚可靠性框架；本域负责将可靠性要求嵌入发布门禁与变更流程。
- **[[13-生产运维/README.md|生产运维]]（生产运维）**：承担值班、事件响应、容量与 FinOps；本域的变更回滚、发布窗口与审计记录是其核心输入。
- **[[08-安全/README.md|安全]]（安全合规）**：定义 RBAC、Secret 管理、镜像签名、审计与合规策略；本域负责在 CI/CD 与 GitOps 流程中落地这些策略。
- **[[09-可观测性/README.md|可观测性]]（可观测性）**：提供指标、日志、追踪与告警；本域依赖其实现金丝雀分析、部署后验证与 DORA 指标采集。
- **[[05-网络/README.md|网络]]（网络流量）**：负责 Ingress、Service Mesh、API Gateway 的流量切分；本域通过金丝雀/蓝绿发布与其联动。
...
- [[11-发布变更/04-变更管理/02-canary-release-strategy.md|金丝雀发布策略与回滚]]
- [[11-发布变更/04-变更管理/08-change-management-process.md|变更管理流程]]
- [[11-发布变更/06-部署方案/04-production-environment-deployment.md|生产环境部署]]
...
*本指南作为 发布变更 的生产就绪入口文档，应随发布工具链与组织流程的演进每季度复核一次。*
