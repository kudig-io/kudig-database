...
- **PSP（PodSecurityPolicy）**：已在 Kubernetes 1.21 弃用，1.25 移除。如果集群中仍有自定义 PSP，必须在升级前完成迁移。
- **PSS（Pod Security Standards）**：内置的三种策略级别：
  - **privileged**：无限制，仅用于系统级基础设施组件。
  - **baseline**：禁止已知危险配置（如 hostPath、hostNetwork、privileged），适合大多数生产业务命名空间。
  - **restricted**：最严格，要求非 root、只读根文件系统、禁止特权提升等，适合高安全要求场景。
- **PSA（Pod Security Admission）**：内置准入控制器，通过 Namespace 标签实施 PSS，无需额外安装第三方组件。

### 3.2 Secret 轮换

Secret 轮换是降低凭据泄露影响的关键控制，应覆盖以下类型：
- **ServiceAccount token**：清理 legacy long-lived token，使用 Bound ServiceAccount Token（自动轮换）。
- **镜像仓库凭据（imagePullSecrets）**：定期更换镜像仓库密码或 token。
- **TLS 证书**：使用 cert-manager 实现自动续期与轮换。
- **etcd 加密密钥**：定期轮换 encryption config 中的密钥，降低历史数据泄露风险。

### 3.3 CIS 基线

CIS Kubernetes Benchmark 覆盖控制面组件配置、节点配置、网络策略、RBAC、日志与审计。生产环境应至少每季度执行一次扫描，并对 FAIL 项制定修复计划。

### 3.4 运行时威胁检测

Falco 与 Tetragon 可以检测容器内的异常行为，例如：
- 特权提升或容器逃逸尝试。
- 敏感目录挂载或文件访问。
- 反向 shell 或可疑网络连接。
- 未授权进程执行。
...
记录每个 PSP 对应的命名空间与工作负载，确认哪些特权是业务必需的。对于必须保留的特权，应在迁移后通过 PSS 的 privileged 级别或自定义策略（Kyverno/OPA）显式授权。
...
观察审计日志中的 `violation` 事件，识别需要调整的工作负载。建议持续观察至少一个业务周期，确保覆盖所有发布与批处理任务。
...
warn 模式会在用户创建违反策略的 Pod 时返回警告，但不会阻止创建。此阶段用于推动开发团队修复清单，同时不影响现有业务运行。
...
enforce 模式会阻止违反策略的 Pod 创建。切换前应确保所有工作负载已通过 warn 阶段验证，并准备回滚标签的预案。
...
Kubernetes 1.24+ 默认不再自动创建 long-lived ServiceAccount token。对于旧集群，应手动清理并改用 TokenRequest API。
...
对于无法立即修复的项，应记录风险接受理由与计划修复时间。高风险项应优先处理，必要时申请临时例外并设置过期时间。
...
审计日志应集中采集到 SIEM 或日志平台，并配置告警规则检测异常操作，如 RBAC 变更、特权 Pod 创建、Secret 大规模读取等。
...
事件隔离后，应立即启动 [[domain-11-production-operations/04-incident-response-template.md|事故响应流程]]，并按 [[domain-11-production-operations/03-on-call-playbook.md|值班手册]] 进行升级。证据保存应包括容器镜像、运行时日志、网络连接记录与节点状态快照。
...
8. **持续监控比一次性扫描更重要**：安全状态会随时间漂移，应建立持续监控与定期复核机制。
