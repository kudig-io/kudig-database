...
- **与 [[domain-06-observability/README.md|可观测性域]] 协作**：本域负责基于已有 metrics/logs/traces 进行根因定位；可观测性域负责采集、存储、告警质量与 SLO 看板。排查时优先使用可观测性域定义的 golden signal 与告警链接。
- **与 [[domain-05-security-compliance/README.md|安全合规域]] 协作**：安全事件（异常 RBAC、证书泄露、容器逃逸）的初步隔离由本域执行（如 cordon 节点、隔离 namespace），深度取证、合规报告与修复由安全域主导。涉及紧急锁定时参考 紧急隔离预案（待补充）（规划中）。
- **与 [[domain-09-reliability-engineering/README.md|可靠性工程域]] 协作**：本域解决「已发生故障」的短期止血；可靠性域负责事后复盘、SLO 修复、混沌工程演练与灾备切换。postmortem 应同步到可靠性域的知识库。
- **与 [[domain-03-networking-traffic/README.md|网络域]] 协作**：CNI、Service、Ingress、DNS、NetworkPolicy 的网络层根因由网络域提供专家意见；本域负责现象确认、信息收集与初步包捕获。
- **与 [[domain-11-production-operations/README.md|生产运维域]] 协作**：值班响应、变更审批、事件升级与生产公告由生产运维域统一调度；本域提供技术判断与恢复命令。
- **与 [[domain-12-cloud-providers/README.md|云厂商域]] 协作**：当 symptom 指向底层 IaaS（如节点 NotReady 由云盘延迟导致、API throttling）时，本域收集证据并提交给云厂商域进行厂商侧工单与升级。
...
- [[domain-06-observability/README.md|可观测性域 — 监控与告警]]
- [[domain-05-security-compliance/README.md|安全合规域 — 安全事件响应]]
- [[domain-09-reliability-engineering/README.md|可靠性工程域 — SRE 与灾备]]
- [[domain-11-production-operations/README.md|生产运维域 — 值班与事件管理]]

---

*本文件作为 domain-10-troubleshooting-diagnostics 的生产就绪入口，建议在每次重大演练或架构变更后复审并更新检查清单。*
