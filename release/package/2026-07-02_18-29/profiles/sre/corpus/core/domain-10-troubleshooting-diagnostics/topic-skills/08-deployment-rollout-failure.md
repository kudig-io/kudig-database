...
| 主题 | 引用路径 | 适用场景 |
|------|---------|---------|
| Deployment Controller 原理 | `domain-02-workloads-applications/` | 理解 Deployment 如何管理 ReplicaSet 和滚动更新逻辑 |
| Deployment 综合故障排查 | `domain-10-troubleshooting-diagnostics/11-deployment-comprehensive-troubleshooting.md` | 深度排查 Deployment 相关问题 |
| StatefulSet 更新策略 | `domain-02-workloads-applications/` | StatefulSet 有序更新和分区更新机制 |
| DaemonSet 更新机制 | `domain-02-workloads-applications/` | DaemonSet 在不同节点上的更新行为 |
| PodDisruptionBudget | `domain-07-platform-engineering/` | PDB 配置最佳实践和与滚动更新的交互 |
| Admission Webhooks | `domain-15-specialized-tech/03-admission-webhook-configuration.md` | Webhook 如何影响 Pod 创建 |
| 容器探针配置 | `domain-02-workloads-applications/` | readinessProbe/livenessProbe 最佳实践 |
| 镜像拉取策略 | `domain-02-workloads-applications/` | imagePullPolicy 配置和镜像仓库认证 |
