...
> **专项排查文档**：
> - [StorageClass 配置与动态供给专项排查]([[domain-10-troubleshooting-diagnostics/高级排障/04-storage/05-storageclass-troubleshooting.md|05-storageclass-troubleshooting]].md) — StorageClass 参数、volumeBindingMode、拓扑约束、扩容、性能等级
...
- **PVC 长时间 Pending**：StorageClass/拓扑约束不匹配或后端配额耗尽。
- **Multi-Attach 错误**：RWO 卷残留在旧节点导致新 Pod 无法挂载。
- **挂载超时/失败**：节点侧挂载点异常、文件系统损坏或设备不可见。
- **扩容无效**：PVC/PV 容量已更新但 Pod 内文件系统未扩展。

### 影响面分析

- **业务不可用**：Pod 卡在 `ContainerCreating` 或反复重启。
- **数据风险**：强制解绑或误删卷可能导致数据丢失。
- **容量风险**：磁盘满导致写入失败、应用异常。
...
- **变更前**：备份重要卷（快照或导出），保存对象描述信息。
- **回滚策略**：恢复上一版本配置或重新调度至原节点。
- **验证**：检查 `df -h`、应用读写与告警恢复。
...
- [[domain-10-troubleshooting-diagnostics/高级排障/04-storage/04-storage-performance-troubleshooting.md|04-storage-performance-troubleshooting]]
- [[domain-10-troubleshooting-diagnostics/高级排障/04-storage/05-storageclass-troubleshooting.md|05-storageclass-troubleshooting]]
- [[domain-10-troubleshooting-diagnostics/高级排障/04-storage/02-csi-troubleshooting.md|02-csi-troubleshooting]]
- [[domain-10-troubleshooting-diagnostics/高级排障/04-storage/03-snapshot-backup-troubleshooting.md|03-snapshot-backup-troubleshooting]]
