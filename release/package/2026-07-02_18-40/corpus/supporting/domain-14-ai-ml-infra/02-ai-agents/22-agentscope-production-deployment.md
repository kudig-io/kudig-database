...
从开发环境到生产环境，Agent 系统需要解决状态管理、安全执行、弹性伸缩和全链路可观测等关键问题。AgentScope 通过独立的 **agentscope-runtime** 项目提供生产级运行时，支持 Agent-as-a-Service（AaaS）模式部署，内置[[domain-14-ai-ml-infra/Agent运行时/12-agent-sandbox-isolation.md|沙箱安全执行]]、OpenTelemetry 追踪和 K8s 原生部署能力。
