...
- **驱动未注册**：`CSINode` 中缺少驱动条目，Pod 无法挂载卷。
- **Create/Attach 超时**：后端 API 限流或驱动阻塞导致 `DeadlineExceeded`。
- **NodeStage/NodePublish 失败**：挂载点异常或权限不足。
- **快照/扩容失败**：驱动能力声明不完整或 Sidecar 版本不兼容。

### 影响面分析

- **存储不可用**：PVC 无法绑定或 Pod 卡在挂载阶段。
- **业务中断**：状态ful 工作负载无法启动或升级。
- **数据一致性风险**：异常卸载可能导致文件系统损坏。
...
- **变更前**：备份 CSI 配置与相关对象 YAML，保存关键日志。
- **回滚策略**：恢复旧版本驱动或撤回不兼容 Sidecar 版本。
- **验证**：确认 `CSINode` 正常、卷可挂载、应用恢复。
...
- [[domain-10-troubleshooting-diagnostics/高级排障/04-storage/01-pv-pvc-troubleshooting.md|01-pv-pvc-troubleshooting]]
