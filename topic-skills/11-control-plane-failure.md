...
| 主题 | 引用路径 | 适用场景 |
|------|---------|---------|
| API Server 架构与内部机制 | `domain-12-troubleshooting/01-control-plane-apiserver-troubleshooting.md` | 深度理解 API Server 请求处理、限流、Webhook 集成 |
| etcd 故障排查深度指南 | `domain-12-troubleshooting/02-control-plane-etcd-troubleshooting.md` | 深度理解 etcd 集群运维、数据恢复、性能调优 |
| 控制平面组件架构 | `domain-3-control-plane/` | 理解 Scheduler、Controller Manager 工作原理 |
| Kubernetes 架构基础 | `domain-1-architecture-fundamentals/` | 控制平面整体架构和组件交互 |
