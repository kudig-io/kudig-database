...
| FTA 调度故障树 | `topic-fta/list/scheduler-fta.md` | 调度器故障的完整 FTA 分析模型，含概率和因果链 |
| FTA Pod 故障树 | `topic-fta/list/pod-fta.md` | Pod 生命周期中所有可能的故障点 |
| 结构化故障排查 — 调度问题 | `topic-structural-trouble-shooting/` | 人类可读的深度排查指南 |
| 通用故障排查方法论 | `domain-12-troubleshooting/` | 系统化故障排查的理论基础和方法 |
| 节点资源管理 | `domain-4-workloads-scheduling/` | Node allocatable、eviction threshold、resource requests/limits 的关系 |
| 存储体系 | `domain-6-storage/` | PVC/PV/StorageClass 的工作机制和常见问题 |
| 集群弹性伸缩 | `domain-9-platform-ops/` | Cluster Autoscaler、Karpenter 等自动化扩容机制 |
