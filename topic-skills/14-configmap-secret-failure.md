...
| RC-011 | **配置漂移（手动修改 vs GitOps 源）** — ConfigMap/Secret 被手动修改后与 GitOps 源不一致，导致同步覆盖或冲突 | ~4% | ArgoCD/Flux 显示 OutOfSync；kubectl get 与 Git 仓库内容不一致 | 🟡 |
...
| 主题 | 引用路径 | 适用场景 |
|------|---------|---------|
| ConfigMap/Secret 深度排查 | `domain-12-troubleshooting/19-configmap-secret-troubleshooting.md` | 超出本 Skill 覆盖的复杂场景 |
| Kubernetes Secret 加密 | `domain-7-security/` | KMS Provider、EncryptionConfiguration 详细配置 |
| External Secrets Operator | External Secrets 官方文档 | ESO 高级配置和故障排查 |
| HashiCorp Vault 集成 | Vault 官方文档 + `domain-7-security/` | Vault Agent Injector 深度配置 |
| kubelet 配置同步机制 | `domain-1-architecture-fundamentals/` | 理解 ConfigMap Volume 更新的内部机制 |
| GitOps 配置管理 | `domain-10-extensions/09-gitops-workflow-argocd.md` | 配置漂移检测和一致性管理 |
