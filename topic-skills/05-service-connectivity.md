...
| 主题 | 引用路径 | 适用场景 |
|------|---------|---------|
| Kubernetes 网络模型与 Service 实现 | `domain-5-networking/` | 理解 Service 的底层实现（iptables/IPVS/nftables 规则生成机制） |
| Service 故障树分析 | `topic-fta/list/service-fta.md` | 理解 Service 连通性故障的完整因果链和概率模型 |
| Ingress 故障树分析 | `topic-fta/list/ingress-fta.md` | 当问题涉及 Ingress → Service 链路时的参考 |
| 网络故障排查深度指南 | `topic-structural-trouble-shooting/` | 超出本 Skill 覆盖范围的深度网络排查方法 |
...
| kube-proxy 架构与实现 | `domain-5-networking/` | 理解 kube-proxy 的三种模式（iptables/IPVS/nftables）及其规则同步机制 |
| NetworkPolicy 原理与实现 | `domain-5-networking/` | 理解不同 CNI 插件对 NetworkPolicy 的实现差异 |
