...
| 主题 | 引用路径 | 适用场景 |
|------|---------|---------|
| CSI 架构与工作原理 | `domain-6-storage/` | 理解 Provisioner、Attacher、Mounter 的分工 |
| 存储类故障排查 | `domain-12-troubleshooting/04-storage-csi-troubleshooting.md` | CSI 驱动级别的深度排查 |
| PVC 生命周期详解 | `domain-12-troubleshooting/14-pvc-storage-troubleshooting.md` | PVC 各状态转换的详细说明 |
| 云厂商存储集成 | `domain-17-cloud-provider/` | ACK/EKS/GKE 特定的存储配置 |
| Volume Snapshot 使用 | `domain-6-storage/` | 快照创建和恢复的详细步骤 |
| StatefulSet 存储最佳实践 | `domain-4-workloads/` | 有状态应用的存储配置建议 |
