...
| 主题 | 引用路径 | 适用场景 |
|------|---------|---------|
| 性能瓶颈深度排查 | `domain-12-troubleshooting/33-performance-bottleneck-troubleshooting.md` | 超出本 Skill 覆盖的复杂性能问题 |
| 性能调优指南 | `domain-1-architecture-fundamentals/13-performance-tuning-guide.md` | 系统性的性能优化方法论 |
| cgroup 详解 | `domain-14-linux/08-linux-container-fundamentals.md` | cgroup v1/v2 深入理解 |
| 网络性能分析 | `domain-5-networking/` | 网络层面性能问题深入分析 |
| 存储性能 | `domain-6-storage/` | 磁盘 I/O 性能深入分析 |
| 可观测性 | `domain-8-observability/` | 监控指标和告警配置 |
| API Server 架构 | `domain-3-control-plane/` | API Server 性能调优 |
| etcd 运维 | `domain-3-control-plane/` | etcd 性能问题排查 |
