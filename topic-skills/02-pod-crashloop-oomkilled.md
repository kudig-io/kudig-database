...
- **判断规则**:
  ...
  - 检查 sidecar 依赖: 某些应用依赖 sidecar proxy（如 Istio envoy）才能正常通信
...
| Linux OOM Killer 机制与 cgroup 内存管理 | `topic-structural-trouble-shooting/` | RC-002, RC-003, RC-011 |
| Kubernetes 资源管理 (requests/limits/QoS) | `domain-4-workloads-scheduling/` | RC-002, RC-003 |
| Container Runtime (containerd) 行为 | `domain-12-troubleshooting/` | RC-001, RC-004 |
| Pod FTA 故障树分析 | `topic-fta/list/pod-fta.md` | 所有根因 |
| Java 内存模型与容器化最佳实践 | `topic-structural-trouble-shooting/` | RC-011 |
| Init Container 与 Sidecar 行为 | `domain-4-workloads-scheduling/` | RC-010 |
| Probe 配置最佳实践 | `topic-structural-trouble-shooting/` | RC-008 |
