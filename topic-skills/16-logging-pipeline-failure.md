...
日志管道故障是 Kubernetes 可观测性体系中**影响最广泛**的问题类型之一。当日志采集、传输或存储环节出现故障时，会导致应用日志缺失、审计日志不完整、告警延迟甚至安全事件无法追溯。在云原生环境中，日志管道通常由采集层（Fluentd/Fluent Bit/Vector）、传输层（Kafka/直接推送）和存储层（Elasticsearch/Loki/ClickHouse）组成，任一环节的故障都可能导致日志数据丢失。
...
| 主题 | 引用路径 | 适用场景 |
|------|---------|---------|
| 日志管理架构与最佳实践 | `domain-21-logging-management-analytics/` | 理解日志管道设计原理 |
| 企业级监控告警系统 | `domain-12-troubleshooting/39-enterprise-monitoring-alerting-system.md` | 日志系统与监控集成 |
| Kubernetes 可观测性 | `domain-8-observability/` | 日志、指标、链路追踪的综合方案 |
| Elasticsearch 运维指南 | `domain-28-enterprise-database-middleware/` | ES 集群深度运维 |
