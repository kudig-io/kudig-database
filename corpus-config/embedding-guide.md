...
python3 scripts/embedding-pipeline.py --profile corpus-config/profiles/rag-full-profile.yaml
```
...
# 加载 chunks
chunks = []
with open("corpus-config/profiles/.vector-cache/sre-ops-agent/chunks.jsonl") as f:
    for line in f:
        chunks.append(json.loads(line))

# 加载 embeddings
embeddings = np.load("corpus-config/profiles/.vector-cache/sre-ops-agent/embeddings.npy")
...
- [[corpus-config/rag-chunking-strategy|RAG 分块策略指南]]
- [[corpus-config/README|AI 语料库配置]]
- topic-index 向量索引
