...
KServe 的核心价值在于：统一的模型部署接口、生产级自动伸缩、金丝雀发布、模型版本管理和可观测性。对于需要管理数十到数百个模型的平台团队，KServe 提供了比裸 Deployment 更高层次的抽象和运维能力。关于模型部署的基础概念，参见 [[15-AI基础设施/05-K8s-AI基础设施/05-kserve-model-serving-platform|10-model-deployment-serving]]；GPU 调度参见 [[22-概念/07-调度与资源/gpu-scheduling-ai-workloads]]。
