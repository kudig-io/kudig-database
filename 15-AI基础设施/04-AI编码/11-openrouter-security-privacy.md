...
> **文档类型**: 安全指南 | **最后更新**: 2026-03 | **关键词**: OpenRouter, [[08-安全/README.md|security]], Privacy, Data Collection, Zero Data Retention, ZDR, EU Residency, BYOK, API Key, OAuth PKCE
...
| 问题现象 | 可能原因 | 排查方法 | 解决方案 |
|---------|---------|---------|--------|
| API Key 泄漏告警 | 代码中硬编码或日志打印 | 搜索代码库中 `sk-or-` 前缀 | 立即轮换 Key，启用 Secret 管理 |
| 异常费用飙升 | Key 被盗用或循环调用 | 查看 OpenRouter Dashboard 用量 | 设置用量上限，启用 IP 白名单 |
| 响应包含敏感数据 | 提示词注入攻击 | 审计输入日志 | 添加输入过滤层 |
| Provider 返回错误 | 上游服务故障或认证失败 | 检查 HTTP 状态码和错误体 | 配置 fallback 模型 |
| 速率限制触发 | 并发请求超过配额 | 检查 429 响应头 | 实现指数退避重试 |

## 合规检查清单

| 检查项 | 要求 | 工具 |
|--------|------|------|
| 数据分类 | 禁止发送 PII/PHI 到外部模型 | DLP 扫描 |
| 审计日志 | 所有 API 调用可追溯 | 集中式日志 |
| 密钥轮换 | 每 90 天强制轮换 | Vault/Secret Manager |
| 访问控制 | 最小权限原则 | RBAC + API Key 作用域 |
| 数据驻留 | 符合 GDPR/等保要求 | 区域化 Provider 选择 |
...
| OpenRouter 版本 | 安全特性 | 合规认证 |
|----------------|---------|--------|
| 2026-07 | ZDR + EU Routing | SOC 2 Type II |
| 2026-04 | + Prompt Guard | GDPR |
| 2026-01 | + IP 白名单 | - |
| 2025-10 | 基础 API Key | - |
