---
title: Agent 作为技术赋能新方式：设计思路与落地路径 (AI基础设施)
...
- **结构化故障排查 Agent**：`故障诊断/`（42 个文件）和 `19-故障诊断/04-高级排障/structural-` 提供了决策树——Agent 可以交互式地引导排查
- **FTA 故障树驱动 Agent**：`19-故障诊断/06-FTA故障树/` 包含完整的故障树分析方法论和 37 个组件级故障树，天然适合 Agent 按树结构逐步推理
...
- **多集群 Agent**：基于 `云厂商/` 知识，协调跨集群操作
- **灾备演练 Agent**：基于 `可靠性/` 自动编排灾备演练
...
- **演示文稿生成器**：`生产运维/topic-presentations/`（12 个文件）+ 知识库内容，动态组装培训演讲材料
- **Runbook 生成 Agent**：基于 `系统基础/topic-dictionary/12-incident-management-runbooks.md` 自动生成标准操作手册

**个性化路径示例**：

```
新入职 K8s 运维工程师 → Agent 评估后推荐:
  Week 1: 集群基础 架构基础 + 系统基础 Linux 基础 + topic-cheat-sheet
  Week 2: 工作负载 工作负载 + 网络 网络 + 清单模式 YAML 手册
  Week 3: 故障诊断 故障排查 + topic-fta 故障树分析
  Week 4: 可观测性 可观测性 + 生产运维 生产运维实践

资深 SRE → Agent 评估后推荐:
  Week 1: topic-fta 故障树方法论 + topic-febm 取证循证
  Week 2: AI基础设施 AI 基础设施 + 网络 eBPF
  Week 3: 平台工程 平台工程 + 安全 供应链安全
```
...
- **自助服务 Agent**：开发者用自然语言描述需求 → Agent 转译为 YAML 清单（借助 `清单模式/`——36 个模板）
- **策略执行 Agent**：基于 `安全/` 和 `安全/`，自动审查并建议安全改进
- **成本优化 Agent**：利用 `26-cost-optimization-overview.md` 和 `27-cost-management-kubecost.md` 主动建议节省方案
- **合规审计 Agent**：基于 `安全/` 自动检查供应链安全合规
...
Agent:
  1. 从 清单模式 检索 Deployment、Service、Ingress、HPA 模板
  2. 从 安全 检索 Pod Security Standards
...
| 类别 | 文档路径 | 与 Agent 的关系 |
|------|---------|---------------|
| FTA 故障树分析 | `19-故障诊断/06-FTA故障树/` | Agent 推理的知识骨架 |
| FEBM 取证循证 | `故障诊断/FEBM方法论/` | Agent 诊断的方法论基础 |
| 结构化故障排查 | `19-故障诊断/04-高级排障/structural-` | Agent 决策树的直接输入 |
| YAML 清单手册 | `清单模式/` | YAML 生成 Agent 的模板库 |
| 故障排查大全 | `故障诊断/` | 排障 Agent 的核心语料 |
| 运维词典 | `系统基础/topic-dictionary/` | Agent 的专业术语和最佳实践 |
| 速查卡 | `系统基础/topic-cheat-sheet/` | Agent 快速回答的参考 |
| 培训演示 | `生产运维/topic-presentations/` | 学习 Agent 的内容来源 |
| 迁移指南 | `发布变更/topic-migration/` | 迁移 Agent 的执行蓝本 |
| K8s Events 大全 | `系统基础/` | Agent 事件解读的知识来源 |
