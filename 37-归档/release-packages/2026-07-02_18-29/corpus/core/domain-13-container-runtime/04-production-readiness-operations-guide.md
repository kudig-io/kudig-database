...
- **domain-05-security-compliance（安全合规）**：负责镜像签名、漏洞扫描、准入控制、seccomp/AppArmor/SELinux 策略以及供应链安全。运行时的安全加固必须与其策略保持一致，避免各自为政导致配置冲突。
- **domain-07-platform-engineering（平台工程）**：负责镜像构建流水线、开发者门户、监控告警体系与节点基线镜像。运行时的高可用配置应纳入平台统一基线，避免每个集群自行拼凑。
- **domain-08-release-change-management（发布变更管理）**：负责镜像版本晋升、GitOps 流水线与变更窗口。生产镜像 tag 规范、不可变 artifact 与回滚策略需要与其协同，防止“latest”流入生产。
- **domain-10-troubleshooting-diagnostics（故障诊断）**：提供结构化的 `ImagePullBackOff`、`SandboxCreate` 等排障路径与 FTA。运行时团队应复用其统一模板，避免重复造轮子。
- **domain-11-production-operations（生产运维）**：负责事件响应、FinOps 与容量管理。节点磁盘、镜像拉取成本与运行时升级窗口需要与其对齐，确保变更在业务低峰期执行。
- **domain-04-storage-data（存储数据）**：当使用 overlayfs、nydus、stargz 等 snapshotter 或节点本地盘时，需要评估存储性能、IOPS 与快照策略，避免存储成为扩容瓶颈。
...
- [[02-containerd-production-operations|containerd 生产运维指南]] — ACK/ASO 场景下的安装、迁移、配置优化与升级回滚。
...
- [[32-发布/package/2026-07-02_18-29/corpus/peripheral/domain-13-container-runtime/01-docker/06-docker-security-best-practices|Docker 安全最佳实践]] — 容器运行时安全基线与限制策略。
- [[32-发布/package/2026-07-02_18-29/corpus/peripheral/domain-13-container-runtime/01-docker/07-docker-troubleshooting-guide|Docker 故障排查指南]] — 容器、网络与存储的通用排错思路。

### 跨域参考

- [[domain-05-security-compliance/README.md|domain-05-security-compliance]] — 镜像安全、供应链与合规。
- [[32-发布/package/2026-07-02_18-29/corpus/core/domain-07-platform-engineering/operate/01-monitoring-alerting-system|监控告警体系]] — 运行时指标接入与告警治理。
