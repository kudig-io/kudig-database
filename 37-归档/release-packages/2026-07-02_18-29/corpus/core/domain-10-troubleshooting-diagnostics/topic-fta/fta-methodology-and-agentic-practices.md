...
├──────────────┼──────────────────┼───────────────────────────────┤
│ 结构          │ 线性步骤列表      │ 树形决策图                    │
│ 数量          │ N个问题=N本Runbook │ 8个顶事件=8棵树覆盖全场景    │
│ 适应性        │ 场景固定          │ 动态选择路径                  │
    ...
    - step: 2
      ...
      conditions:
        ...
        - if: "所有 endpoint 均 healthy"
          action: "排除 etcd 问题，检查下一个 OR 分支"
          
    - step: 3
      fta_event: BE-1.2
      description: "深入诊断 etcd 问题类型"
      commands:
        - name: "检查 etcd 日志"
...
| 指标 | 定义 | 计算方式 | 目标值 | 数据来源 |
|------|------|---------|-------|---------|
| **覆盖率** | FTA 覆盖的故障模式比例 | (FTA 包含的问题 / 历史问题总数) × 100% | > 95% | 问题工单系统 |
...
8. 追踪：跨组件问题是否产出 Trace ID 并贯穿证据链？
