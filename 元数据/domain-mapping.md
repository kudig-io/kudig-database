...
## 中文支撑域（14，已中文化，结构保持稳定）

| 目录 | 原名 | 用途 |
|---|---|---|
| `资产/` | `assets/` | 图片、PDF 等静态资产 |
| `概念/` | `concepts/` | 概念页（obsidian-wiki 分类） |
| `文档/` | `docs/` | 项目级说明文档 |
| `实体/` | `entities/` | 实体页：工具/产品/公司（obsidian-wiki 分类） |
| `发布/` | `release/` | 发布包与打包产物（冻结） |
| `研究/` | `research/` | 研究性笔记 |
| `脚本/` | `scripts/` | 维护脚本（含 `maintenance/rewrite-links-after-rename.py`、`rename-support-dirs.py`） |
| `技能/` | `skills/` | Skill 定义（obsidian-wiki 分类） |
| `综合/` | `synthesis/` | 跨域综合文章（obsidian-wiki 分类） |
| `标签/` | `tags/` | 标签索引页 |
| `站点/` | `web/` | Astro 站点源文件 |
| `归档/` | `_archives/` | 归档内容（含 `domain-indexes/`、`release-notes/`） |
| `元数据/` | `_meta/` | 元数据（本表、索引、dashboard），本表所在位置 |
| `报告/` | `_reports/` | 冻结的报告文件 |

> `code/`（源码）目录因内含大量 vendor 源码（Kubernetes release、terway 等）已被 .gitignore 忽略，不在主库追踪范围。
