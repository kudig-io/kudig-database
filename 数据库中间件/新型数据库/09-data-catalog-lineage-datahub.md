...
本文覆盖 DataHub、Apache Atlas 和 Marquez 三款主流数据目录/血缘系统在 K8s 上的部署与集成实践，帮助数据平台团队构建完整的数据治理体系。数据目录通常与 [[数据库中间件/数据流/]] 中的管线编排配合使用。
...
7. **与数据管线集成**：在 [[数据库中间件/数据流/]] 的 Flink/Airflow 管线中嵌入 OpenLineage SDK
8. **监控告警**：监控 GMS 延迟、Kafka 消费 lag、ES 索引大小，接入 [[可观测性/]] 平台
9. **数据库元数据同步**：定期从 [[数据库中间件/数据库/]] 中的 PostgreSQL/MySQL 同步 Schema 元数据
