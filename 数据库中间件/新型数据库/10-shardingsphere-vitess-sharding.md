...
ShardingSphere 和 Vitess 是两款主流的分片中间件：ShardingSphere 更贴近 Java 生态，提供 JDBC 和 Proxy 两种接入模式；Vitess 由 YouTube 开源，是 MySQL 分片的工业级方案，原生支持 Kubernetes。本文覆盖两者在 K8s 上的部署、分片策略设计和运维实践。分片方案通常与 [[数据库中间件/数据库/]] 中的 MySQL/PostgreSQL 集群配合使用。
...
6. **监控**：关注各分片的 QPS/连接数/磁盘使用均衡性，接入 [[可观测性/]] 平台
7. **备份**：每个分片独立备份，参考 [[可靠性/备份恢复/]] 确保一致性快照
8. **连接池**：Proxy 模式下配合连接池使用，参考 [[数据库中间件/新型数据库/05-connection-pooling-pgbouncer-proxysql.md]]
9. **Operator 管理**：Vitess 使用官方 Operator，参考 [[数据库中间件/Operator管理/]] 中的 CRD 管理
