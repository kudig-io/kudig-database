...
本文聚焦 Neo4j 和 NebulaGraph 两款主流图数据库在 Kubernetes 上的生产级部署与运维，涵盖集群架构、查询优化、备份恢复和故障排查。图数据库通常作为 [[AI基础设施/]] 中知识图谱层的核心组件，与向量数据库配合构建完整的 RAG 系统。
...
5. **备份策略**：参考 [[可靠性/备份恢复/]] 建立定期备份机制，Neo4j 使用 `neo4j-admin backup`，NebulaGraph 使用 Snapshot + BR 工具
6. **监控集成**：将 Neo4j Metrics / NebulaGraph Stats 接入 [[可观测性/]] 平台
7. **数据导入**：大批量数据导入使用 `neo4j-admin import`（离线）或 NebulaGraph Importer，避免逐条 INSERT
8. **Operator 管理**：使用 Nebula Operator 管理集群生命周期，参考 [[数据库中间件/Operator管理/]]
