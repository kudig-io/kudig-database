...
本文覆盖数据库监控体系设计、PMM 在 K8s 上的部署、查询性能分析工具和告警策略，是 [[可观测性/]] 体系在数据库层的延伸实践。
...
6. **面板标准化**：建立统一的数据库 Grafana Dashboard 模板，参考 [[可观测性/]] 中的面板设计规范
7. **与备份联动**：监控备份任务状态，参考 [[可靠性/备份恢复/]] 设置备份失败告警
8. **连接池监控**：如使用 PgBouncer/ProxySQL，同步监控连接池指标，参考 [[数据库中间件/新型数据库/05-connection-pooling-pgbouncer-proxysql.md]]
