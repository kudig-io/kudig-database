...
本文覆盖这三款引擎在 Kubernetes 上的生产部署、数据导入策略、查询优化和故障排查，帮助数据平台团队构建高性能分析基础设施。OLAP 引擎通常与 [[数据库中间件/数据流/]] 中的 Kafka/Flink 管线配合，实现实时数据入仓。
...
2. **物化视图策略**：对高频聚合查询创建物化视图，定期刷新，参考 [[数据库中间件/数据流/]] 中的增量计算模式
...
6. **监控告警**：关注 Compaction 积压、Tablet 不健康数、查询 P99 延迟，接入 [[可观测性/]] 平台
7. **备份恢复**：使用 `BACKUP/RESTORE` 命令备份到 S3/HDFS，参考 [[可靠性/备份恢复/]]
8. **资源隔离**：OLAP 集群使用独立 Node Pool，避免与 OLTP 工作负载争抢资源，参考 [[数据库中间件/数据库/]] 中的资源管理
