...
向量数据库是 AI 基础设施的核心组件，用于存储和检索高维向量数据（Embedding），支撑语义搜索、推荐系统、RAG（Retrieval-Augmented Generation）等场景。随着大语言模型在企业中的广泛应用，向量数据库已成为 [[AI基础设施/]] 中不可或缺的一环。
...
5. **备份策略**：每日全量快照 + WAL 增量，参考 [[可靠性/备份恢复/]] 制定 RPO/RTO
6. **监控集成**：将向量数据库 metrics 接入 [[可观测性/]] 平台，设置 P99 延迟和召回率告警
7. **数据管线**：Embedding 生成管线参考 [[数据库中间件/数据流/]] 中的流处理模式
8. **Operator 管理**：如使用 Milvus Operator，参考 [[数据库中间件/Operator管理/]] 中的 CRD 管理实践
