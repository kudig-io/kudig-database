...
## 可观测性 — Prometheus, Grafana, Viz 集成
...
在生产环境中验证服务网格的弹性能力是确保系统可靠性的关键步骤。Linkerd 提供了内置的故障注入功能，允许运维团队在不修改应用代码的情况下模拟服务故障。通过渐进式地增加故障比例（从 0.1% 到 1% 到 5% 到 10%），可以安全地观察系统在故障条件下的行为，验证熔断器、重试和降级策略是否按预期工作。建议在非业务高峰期执行故障注入测试，并准备好快速回滚方案。
