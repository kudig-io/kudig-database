...
在生产环境中，API 网关是所有外部流量的入口点，其可用性直接影响整个系统的可用性。APISIX 的高可用架构包含以下关键设计：第一，部署 3 个或更多 APISIX 网关副本，通过 Kubernetes Service 的 LoadBalancer 类型暴露服务，云厂商的负载均衡器会自动进行健康检查和流量分配；第二，使用 etcd 集群（至少 3 节点）作为配置存储，确保配置数据的强一致性和高可用性；第三，配置 Pod 反亲和性规则，确保网关副本分布在不同节点上，防止单节点故障导致所有网关实例不可用；第四，配置 HPA 自动扩缩容，基于 CPU 和内存指标动态调整网关副本数量。
