...
熔断器是微服务弹性模式中最核心的机制。它的工作原理类似于电路中的保险丝：当下游服务的错误率超过阈值时，熔断器"跳闸"，后续请求不再发送到已故障的下游服务，而是快速失败或执行降级逻辑。经过一段冷却期后，熔断器进入"半开"状态，允许少量探测请求通过，如果探测成功则恢复，否则继续断开。
...
重试是处理瞬时故障最直接的机制。但不当的重试配置（如无限重试、不区分错误类型）反而会加重系统负载，甚至引发重试风暴。合理的重试策略需要考虑：哪些错误可重试、重试次数、重试间隔、退避策略。
...
在生产环境中验证弹性模式的有效性是确保系统可靠性的关键步骤。以下脚本通过模拟各种故障场景（服务不可用、高延迟、连接超时），验证熔断器、重试、超时和降级策略是否按预期工作。建议在非业务高峰期执行此测试脚本，并确保监控告警已正确配置，以便在测试过程中观察告警触发情况。
...
echo "--- Step 1: 注入 503 故障 (通过 VirtualService) ---"
kubectl apply -f - <<EOF
apiVersion: networking.istio.io/v1
kind: VirtualService
metadata:
...
## 十二、Istio 与 Resilience4j 协同部署配置
