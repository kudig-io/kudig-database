...
if ! [[ "$0" =~ 脚本/generate_release_notes.sh ]]; then
	echo "must be run from repository root"
	exit 1
fi

if [[ $# -ne 1 ]]; then
	echo "usage: 脚本/generate_release_notes.sh <version>"
	echo "examples:"
	echo "  脚本/generate_release_notes.sh 3.11.0"
	echo "  脚本/generate_release_notes.sh 3.11.0-rc.0"
	echo "  脚本/generate_release_notes.sh 3.11.0-rc.1"
	echo "  脚本/generate_release_notes.sh 3.11.1"
	exit 1
fi
