...
static const struct ne6x_debug_info ne6x_device_info[] = {
	{0xE120, "N5E010P2", "10G"},  {0xE140, "N5E010P4", "10G"},
	{0xE12C, "N5C010P2", "10G"},  {0xE220, "N5E025P2", "25G"},
	{0xE22C, "N5C025P2", "25G"},  {0xE221, "N5S025P2", "25G"},
	{0xE222, "N5E025P2-PAU", "25G"}, {0xEA21, "N6S100P2", "100G"},
	{0xE421, "N6S040P2", "40G"},  {0xE281, "N6S100P2", "25G"},
	{0xE22E, "N6S025P2", "25G"},
};
...
static void ne6x_dbg_set_perm_mac(struct ne6x_pf *pf, char *cmd_buf, int count)
{
	u8 mac_addr[6];
	int port = 0;
	int ret;
	...
	ret = ne6x_dev_set_perm_mac(pf, port, mac_addr);
	if (!ret)
		memcpy(&pf->fw_info.fw_port[port].mac, &mac_addr, 6);
	dev_info(&pf->pdev->dev, "%s: %s\n", __func__,
...
static void ne6x_dbg_get_mac(struct ne6x_pf *pf, char *cmd_buf, int count)
{
	...
	memcpy(&mac_addr, &pf->fw_info.fw_port[port].mac, 6);
...
static void ne6x_dbg_tab_insert(struct ne6x_pf *pf, char *cmd_buf, int count)
{
	...
	p_str_array = kcalloc(ARRAY_P_MAX_COUNT, sizeof(u8 *), GFP_KERNEL);
	if (!p_str_array)
		return;
...
static const struct ne6x_dbg_cmd_wr deg_cmd_wr[] = {
	...
	{"set_port_mac",        ne6x_dbg_set_perm_mac},
				       ...
				       size_t count, loff_t *ppos)
{
	...
	if (!pru_name) {
		...
		sprintf(name_pre, "Product Name: NorthLink");
