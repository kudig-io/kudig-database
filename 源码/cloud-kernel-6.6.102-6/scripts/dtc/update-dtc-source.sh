...
# source files and generated source file(s) into the 脚本/dtc directory
...
get_last_dtc_version() {
	git log --oneline 脚本/dtc/ | grep 'upstream' | head -1 | sed -e 's/^.* \(.*\)/\1/'
}
...
commit_msg=$(cat << EOF
脚本/dtc: Update to upstream version ${dtc_version}
