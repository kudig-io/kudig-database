...
#
# usage: Run '脚本/find-unused-docs.sh directory' from top level of kernel
# 	 tree.
#
...
if [ "$#" -ne 1 ]; then
	echo "Usage: 脚本/find-unused-docs.sh directory"
	exit 1
fi
...
for file in `find $1 -name '*.c'`; do
	...
	str=$(脚本/kernel-doc -export "$file" 2>/dev/null)
	if [[ -n "$str" ]]; then
	echo "$file"
	fi
	done
