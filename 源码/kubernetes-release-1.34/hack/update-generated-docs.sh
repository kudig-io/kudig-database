...
generate_docs() {
  ...
  touch 文档/.generated_docs
  find . -type f | cut -sd / -f 2- | LC_ALL=C sort > 文档/.generated_docs
...
remove_generated_docs() {
  if [ -e "${KUBE_ROOT}/docs/.generated_docs" ]; then
    ...
    # The 文档/.generated_docs file lists itself, so we don't need to explicitly
    # delete it.
  fi
}
...
# the shopt is so that we get 文档/.generated_docs from the glob.
shopt -s dotglob
