...
description: '- 运维工程师'
summary: 'Job 和 CronJob 是 [[实体/kubernetes.md|[[Kubernetes|kubernetes]]]] 批处理工作负载的核心控制器。Job 用于一次性任务执行，CronJob 用于定时触发 Job。'
category: skills
tags:
- k8s
- skills
...
cross_refs:
  ...
  - type: "domain"
    ...
    label: "CronJob 深度排查"
  - type: "domain"
    path: "../故障诊断/22-job-troubleshooting.md"
    label: "Job 深度排查"
authors:
  - name: KUDIG Team
    role: contributor
...
Job 和 CronJob 是 [[实体/kubernetes.md|[[Kubernetes|kubernetes]]]] 批处理工作负载的核心控制器。Job 用于一次性任务执行，CronJob 用于定时触发 Job。它们的故障模式与长期运行的工作负载（Deployment/StatefulSet/DaemonSet）有显著差异：任务完成后 Pod 会终止、CronJob 的调度依赖控制器时间同步、Job 的完成条件涉及并行度和成功计数、历史 Job 的清理依赖 TTL 机制。
