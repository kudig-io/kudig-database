...
<!-- chunk: 2. PVC 创建失败故障排查 (PVC Provisioning Failure) -->
...
| 错误信息 | 可能原因 | 解决方案 |
|---------|---------|---------|
| `no [[系统基础/topic-dictionary/storage/persistent-volumes.md|persistent volumes]]es（卷）|volumes]] available` | 无匹配的PV/StorageClass配置错误 | 检查StorageClass参数配置 |
...
<!-- chunk: 3. 卷挂载失败故障排查 (Volume Mount Failure) -->
