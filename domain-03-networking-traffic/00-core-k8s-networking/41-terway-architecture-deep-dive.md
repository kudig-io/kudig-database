...
<!-- chunk: 2. 控制面组件 -->## 2. 控制面组件

#<!-- chunk: 2.1 组件总览表 -->## 2.1 组件总览表
...
#<!-- chunk: 2.2 Terway DaemonSet 详解 -->## 2.2 Terway DaemonSet 详解
...
#<!-- chunk: 2.3 Terway Controller 详解 -->## 2.3 Terway Controller 详解
...
#<!-- chunk: 2.4 eni-config ConfigMap 示例 -->## 2.4 eni-config ConfigMap 示例
...
<!-- chunk: 3. 数据面模式详解 -->## 3. 数据面模式详解

Terway 支持五种数据面模式, 每种模式在性能、密度、功能上各有取舍, 可根据业务场景灵活选择.

#<!-- chunk: 3.1 模式对比总览 -->## 3.1 模式对比总览
...
#<!-- chunk: 3.2 VPC 路由模式 -->## 3.2 VPC 路由模式
...
#<!-- chunk: 3.3 ENI 独占模式 -->## 3.3 ENI 独占模式
...
##<!-- chunk: ENI 数量限制 (按实例规格) -->## ENI 数量限制 (按实例规格)
...
#<!-- chunk: 3.4 ENIIP 模式 (推荐) -->## 3.4 ENIIP 模式 (推荐)
...
#<!-- chunk: 3.5 ENIIP-Trunking 模式 -->## 3.5 ENIIP-Trunking 模式
...
#<!-- chunk: 3.6 IPVlan 模式 -->## 3.6 IPVlan 模式
...
##<!-- chunk: IPVlan 模式要求与限制 -->## IPVlan 模式要求与限制
...
<!-- chunk: 4. IPAM 机制 -->## 4. IPAM 机制

Terway 实现了本地 IP 池 + 云端 OpenAPI 两级 IPAM, 兼顾分配速度和资源利用率.

#<!-- chunk: 4.1 IP 分配流程 (CNI ADD) -->## 4.1 IP 分配流程 (CNI ADD)
...
#<!-- chunk: 4.2 IP 回收流程 (CNI DEL) -->## 4.2 IP 回收流程 (CNI DEL)
...
#<!-- chunk: 4.3 IP 池管理策略 -->## 4.3 IP 池管理策略
...
<!-- chunk: 5. CRD 资源模型 -->## 5. CRD 资源模型

Terway 通过自定义资源 (CRD) 声明式管理网络资源, 实现与 Kubernetes 控制循环的深度集成.

#<!-- chunk: 5.1 CRD 总览表 -->## 5.1 CRD 总览表
...
#<!-- chunk: 5.2 PodENI 示例 -->## 5.2 PodENI 示例
...
#<!-- chunk: 5.3 IPInstance 状态流转 -->## 5.3 IPInstance 状态流转
...
#<!-- chunk: 6.2 RAM 权限策略 -->## 6.2 RAM 权限策略
...
#<!-- chunk: 6.3 NetworkPolicy 实现方式 -->## 6.3 NetworkPolicy 实现方式
...
#<!-- chunk: 7.2 CNI 操作生命周期 -->## 7.2 CNI 操作生命周期
...
#<!-- chunk: 7.3 CNI ADD 详细流程 -->## 7.3 CNI ADD 详细流程
...
<!-- chunk: 8. 持久化与状态管理 -->## 8. 持久化与状态管理

Terway 需要在节点本地维护网络资源的状态映射关系, 确保节点重启后能正确恢复.

#<!-- chunk: 8.1 状态存储架构 -->## 8.1 状态存储架构
...
#<!-- chunk: 8.2 ResRelation.db 数据结构 -->## 8.2 ResRelation.db 数据结构
...
#<!-- chunk: 9.1 本主题内引用 -->## 9.1 本主题内引用

| 文件 | 说明 |
|------|------|
| [01-product.md](./01-product.md) | Terway 产品概览, 版本历史, CNI 对比, ECS 规格速查 |
| [03-usage.md](./03-usage.md) | 安装配置, 模式切换, NetworkPolicy, 固定 IP, Annotation 速查 |
...
#<!-- chunk: 9.2 跨域引用 -->## 9.2 跨域引用

| 文件 | 说明 |
|------|------|
| [domain-03-networking-traffic/05-terway-advanced-guide.md](../domain-03-networking-traffic/05-terway-advanced-guide.md) | Terway 高级指南, 模式对比, ENIIP 详解 |
| [domain-03-networking-traffic/37-terway-resources-crud-operations.md](../domain-03-networking-traffic/37-terway-resources-crud-operations.md) | 网络相关 CRD 的详细 API 定义与 CRUD 操作 |
| [domain-03-networking-traffic/38-terway-gc-mechanism.md](../domain-03-networking-traffic/38-terway-gc-mechanism.md) | GC 垃圾回收机制详解, 设计原则, 触发链路 |
...
| [domain-03-networking-traffic/34-network-performance-tuning.md](../domain-03-networking-traffic/34-network-performance-tuning.md) | 网络性能调优通用指南 |
...
<!-- chunk: 附录 B: 内核版本兼容性 -->## 附录 B: 内核版本兼容性
