...
description: 'description: ''<!-- chunk: 1. 网络模式性能对比'' -->## 1. 网络模式性能对比'''
category: general
tags:
- k8s
- kubelet
...
tier: peripheral---
title: 06 - Terway 性能调优 (Performance Tuning)
description: '<!-- chunk: 1. 网络模式性能对比' -->## 1. 网络模式性能对比'
category: terway
tags:
- k8s
- terway
...
<!-- chunk: 1. 网络模式性能对比 -->## 1. 网络模式性能对比

#<!-- chunk: 1.1 吞吐量与延迟基准 -->## 1.1 吞吐量与延迟基准
...
#<!-- chunk: 1.2 模式选型决策矩阵 -->## 1.2 模式选型决策矩阵
...
<!-- chunk: 2. Pod 容量计算 -->## 2. Pod 容量计算

#<!-- chunk: 2.1 ENIIP 模式容量表 -->## 2.1 ENIIP 模式容量表
...
#<!-- chunk: 2.2 ENI 独占模式容量表 -->## 2.2 ENI 独占模式容量表
...
#<!-- chunk: 2.3 查询节点实际容量 -->## 2.3 查询节点实际容量
...
<!-- chunk: 3. 内核与系统调优 -->## 3. 内核与系统调优

#<!-- chunk: 3.1 网卡多队列 (Multi-Queue) -->## 3.1 网卡多队列 (Multi-Queue)
...
#<!-- chunk: 3.3 IPVlan 模式优化 -->## 3.3 IPVlan 模式优化
...
#<!-- chunk: 3.4 NUMA 感知 (大规格实例) -->## 3.4 NUMA 感知 (大规格实例)
...
<!-- chunk: 4. 大规格实例优化 -->## 4. 大规格实例优化

#<!-- chunk: 4.1 实例规格选择指南 -->## 4.1 实例规格选择指南
...
#<!-- chunk: 4.2 Trunking 模式 (高密度场景) -->## 4.2 Trunking 模式 (高密度场景)
...
<!-- chunk: 5. IP 分配性能优化 -->## 5. IP 分配性能优化

#<!-- chunk: 5.1 IP 池预热 -->## 5.1 IP 池预热
...
#<!-- chunk: 5.2 OpenAPI 调用优化 -->## 5.2 OpenAPI 调用优化
...
#<!-- chunk: 5.3 IP 分配耗时分析 -->## 5.3 IP 分配耗时分析
...
<!-- chunk: 6. eBPF 加速 -->## 6. eBPF 加速

#<!-- chunk: 6.1 Cilium eBPF 替代 kube-proxy -->## 6.1 Cilium eBPF 替代 kube-proxy
...
#<!-- chunk: 6.2 eBPF 加速效果 -->## 6.2 eBPF 加速效果
...
#<!-- chunk: 6.4 iptables → eBPF 迁移指南 -->## 6.4 iptables → eBPF 迁移指南
...
<!-- chunk: 7. ENI 预热 -->## 7. ENI 预热

#<!-- chunk: 7.1 预分配机制 -->## 7.1 预分配机制
...
#<!-- chunk: 7.2 ENI 预热配置 -->## 7.2 ENI 预热配置
...
<!-- chunk: 8. 监控与观测 -->## 8. 监控与观测

#<!-- chunk: 8.1 关键性能指标 -->## 8.1 关键性能指标
...
#<!-- chunk: 8.3 iperf3 基准测试 -->## 8.3 iperf3 基准测试
...
#<!-- chunk: 8.4 Ping 延迟测试 -->## 8.4 Ping 延迟测试
...
<!-- chunk: 9. 生产环境性能基线 -->## 9. 生产环境性能基线

#<!-- chunk: 9.1 参考基线表 -->## 9.1 参考基线表
...
<!-- chunk: 10. 性能故障排查 -->## 10. 性能故障排查

#<!-- chunk: 10.1 ENI 分配延迟诊断 -->## 10.1 ENI 分配延迟诊断
...
#<!-- chunk: 10.2 网络延迟与丢包诊断 -->## 10.2 网络延迟与丢包诊断
...
#<!-- chunk: 10.5 TCP 重传与拥塞分析 -->## 10.5 TCP 重传与拥塞分析
...
| [domain-03-networking-traffic/37-terway-resources-crud-operations.md](../domain-03-networking-traffic/37-terway-resources-crud-operations.md) | Terway 实例 CRUD 操作、CRD 资源管理 |
| [domain-03-networking-traffic/38-terway-gc-mechanism.md](../domain-03-networking-traffic/38-terway-gc-mechanism.md) | Terway GC 垃圾回收机制 |
