...
description: 'description: ''<!-- chunk: 1. Pod 网络基础验证'' -->## 1. Pod 网络基础验证'''
category: general
tags:
- k8s
- cilium
...
tier: peripheral---
title: 05 - Terway 测试验证 (Testing & Validation)
description: '<!-- chunk: 1. Pod 网络基础验证' -->## 1. Pod 网络基础验证'
category: terway
tags:
- k8s
- terway
...
<!-- chunk: 1. Pod 网络基础验证 -->## 1. Pod 网络基础验证

#<!-- chunk: 1.1 创建测试 Pod -->## 1.1 创建测试 Pod
...
#<!-- chunk: 1.2 验证 Pod IP (ENIIP 模式) -->## 1.2 验证 Pod IP (ENIIP 模式)
...
#<!-- chunk: 1.3 验证 Pod Annotation (已分配 IP) -->## 1.3 验证 Pod Annotation (已分配 IP)
...
#<!-- chunk: 1.4 Pod 内网络接口检查 -->## 1.4 Pod 内网络接口检查
...
| 默认路由 | 指向节点网关或 ENI 接口 |
| nameserver | 集群 CoreDNS Service ClusterIP（通常为 10.96.0.10 或 172.16.0.10） |
...
<!-- chunk: 2. 跨节点连通性测试 -->## 2. 跨节点连通性测试

#<!-- chunk: 2.1 同节点 Pod 通信 -->## 2.1 同节点 Pod 通信
...
#<!-- chunk: 2.2 跨节点 Pod 通信 -->## 2.2 跨节点 Pod 通信
...
#<!-- chunk: 2.3 Pod → Node 通信 -->## 2.3 Pod → Node 通信
...
#<!-- chunk: 2.4 Pod → 外网通信 -->## 2.4 Pod → 外网通信
...
#<!-- chunk: 2.5 Pod → VPC 元数据服务 -->## 2.5 Pod → VPC 元数据服务
...
#<!-- chunk: 2.6 连通性测试汇总脚本 -->## 2.6 连通性测试汇总脚本
...
<!-- chunk: 3. NetworkPolicy 测试 -->## 3. NetworkPolicy 测试

#<!-- chunk: 3.1 默认拒绝所有入站流量 -->## 3.1 默认拒绝所有入站流量
...
#<!-- chunk: 3.2 放行特定标签 Pod -->## 3.2 放行特定标签 Pod
...
#<!-- chunk: 3.3 出站策略 (Egress) -->## 3.3 出站策略 (Egress)
...
#<!-- chunk: 3.4 清理所有测试策略和 Pod -->## 3.4 清理所有测试策略和 Pod
...
<!-- chunk: 4. ENI 配额验证 -->## 4. ENI 配额验证

#<!-- chunk: 4.1 节点级配额查看 -->## 4.1 节点级配额查看
...
#<!-- chunk: 4.2 所有节点配额总览 -->## 4.2 所有节点配额总览
...
<!-- chunk: 5. 固定 IP 验证 -->## 5. 固定 IP 验证

#<!-- chunk: 5.1 创建 PodNetworking (Fixed 分配策略) -->## 5.1 创建 PodNetworking (Fixed 分配策略)
...
#<!-- chunk: 5.2 创建使用固定 IP 的 Pod -->## 5.2 创建使用固定 IP 的 Pod
...
#<!-- chunk: 5.3 删除 Pod 并验证 IP 保留 -->## 5.3 删除 Pod 并验证 IP 保留
...
#<!-- chunk: 5.4 重建 Pod 验证 IP 一致性 -->## 5.4 重建 Pod 验证 IP 一致性
...
<!-- chunk: 6. GC (垃圾回收) 验证 -->## 6. GC (垃圾回收) 验证

#<!-- chunk: 6.1 记录当前 IP 分配状态 -->## 6.1 记录当前 IP 分配状态
...
#<!-- chunk: 6.2 创建并删除测试 Pod -->## 6.2 创建并删除测试 Pod
...
#<!-- chunk: 6.3 等待 GC 周期并验证 -->## 6.3 等待 GC 周期并验证
...
#<!-- chunk: 6.4 验证 IP 已回收 -->## 6.4 验证 IP 已回收
...
<!-- chunk: 7. 安全组验证 -->## 7. 安全组验证

#<!-- chunk: 7.1 节点安全组规则检查 -->## 7.1 节点安全组规则检查
...
#<!-- chunk: 7.3 Pod 级安全组隔离测试 -->## 7.3 Pod 级安全组隔离测试
...
#<!-- chunk: 7.4 安全组连通性验证 -->## 7.4 安全组连通性验证
...
> 安全组配置参考: [03-usage.md](./03-usage.md) 第 2 节

---

<!-- chunk: 8. 性能基准测试 -->## 8. 性能基准测试

> 详细性能分析参考: [06-performance.md](./06-performance.md)

#<!-- chunk: 8.1 iperf3 吞吐量测试 -->## 8.1 iperf3 吞吐量测试
...
#<!-- chunk: 8.4 DNS 解析性能测试 -->## 8.4 DNS 解析性能测试
...
<!-- chunk: 9. MTU 测试 -->## 9. MTU 测试

#<!-- chunk: 9.1 检查各接口 MTU -->## 9.1 检查各接口 MTU
...
#<!-- chunk: 9.2 各模式预期 MTU 值 -->## 9.2 各模式预期 MTU 值
...
#<!-- chunk: 9.3 大包测试 (DF 位) -->## 9.3 大包测试 (DF 位)
...
#<!-- chunk: Domain 知识库 -->## Domain 知识库

| 文档 | 说明 |
|:---|:---|
| [domain-03-networking-traffic/05-terway-advanced-guide.md](../domain-03-networking-traffic/05-terway-advanced-guide.md) | Terway 高级指南（模式对比、容量规划） |
| [domain-03-networking-traffic/37-terway-resources-crud-operations.md](../domain-03-networking-traffic/37-terway-resources-crud-operations.md) | Terway CRD 资源 CRUD 操作 |
...
| [domain-03-networking-traffic/03-cni-plugins-comparison.md](../domain-03-networking-traffic/03-cni-plugins-comparison.md) | CNI 插件对比 |

#<!-- chunk: Topic 专题 -->## Topic 专题

| 文档 | 说明 |
|:---|:---|
| [domain-11-production-operations/topic-learn/inner-training/week-4-network-storage/day-24-terway-cni.md](../domain-11-production-operations/topic-learn/inner-training/week-4-network-storage/day-24-terway-cni.md) | Terway CNI 入门学习任务 |
| [domain-10-troubleshooting-diagnostics/topic-structural-trouble-shooting/03-networking/07-terway-troubleshooting.md](../domain-10-troubleshooting-diagnostics/topic-structural-trouble-shooting/03-networking/07-terway-troubleshooting.md) | Terway 结构化故障排查 |
| [domain-10-troubleshooting-diagnostics/topic-fta/list/terway-fta.md](../domain-10-troubleshooting-diagnostics/topic-fta/list/terway-fta.md) | Terway 异常 FTA 故障树 |
...
- [[43-terway-crd-operations.md|03b - Terway CRD 深度操作指南 (CRD Operations Deep Dive)]]
...
- [[domain-03-networking-traffic/43-terway-crd-operations.md|43-terway-crd-operations]]
