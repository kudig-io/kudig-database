...
#<!-- chunk: 1.2 terway-cli 诊断工具 -->## 1.2 terway-cli 诊断工具
...
#<!-- chunk: 1.3 完整健康检查脚本 -->## 1.3 完整健康检查脚本
...
<!-- chunk: 2. GC (垃圾回收) 机制 -->## 2. GC (垃圾回收) 机制

> 本节内容综合整理自 [domain-03-networking-traffic/38-terway-gc-mechanism.md](../domain-03-networking-traffic/38-terway-gc-mechanism.md)

#<!-- chunk: 2.1 为什么需要 GC -->## 2.1 为什么需要 GC
...
#<!-- chunk: 2.5 关键 ConfigMap 参数 -->## 2.5 关键 ConfigMap 参数
...
#<!-- chunk: 2.7 GC 参数调整场景 -->## 2.7 GC 参数调整场景
...
#<!-- chunk: 2.9 GC 源码级执行流程 -->## 2.9 GC 源码级执行流程

> 本节内容提取自 [domain-03-networking-traffic/38-terway-gc-mechanism.md](../domain-03-networking-traffic/38-terway-gc-mechanism.md) 第 4 节。

##<!-- chunk: 2.9.1 源码默认值表 -->## 2.9.1 源码默认值表
...
##<!-- chunk: 2.9.2 gcPods() 4 阶段流程 -->## 2.9.2 gcPods() 4 阶段流程
...
##<!-- chunk: 2.9.3 ENI GC 决策树 -->## 2.9.3 ENI GC 决策树
...
##<!-- chunk: 2.9.4 CRD GC 流程 -->## 2.9.4 CRD GC 流程
...
##<!-- chunk: 2.9.5 IP 孤儿判定 5 条件 -->## 2.9.5 IP 孤儿判定 5 条件
...
##<!-- chunk: 2.9.6 GC 清理优先级 -->## 2.9.6 GC 清理优先级
...
##<!-- chunk: 2.9.7 GC 相关监控指标 -->## 2.9.7 GC 相关监控指标
...
##<!-- chunk: 2.9.8 TerwayGCSlowExecution 告警规则 -->## 2.9.8 TerwayGCSlowExecution 告警规则
...
<!-- chunk: 3. 监控与告警 -->## 3. 监控与告警

#<!-- chunk: 3.1 核心监控指标 -->## 3.1 核心监控指标
...
#<!-- chunk: 3.2 PrometheusRule 完整告警规则 -->## 3.2 PrometheusRule 完整告警规则
...
#<!-- chunk: 3.5 Grafana 仪表盘模板 -->## 3.5 Grafana 仪表盘模板
...
<!-- chunk: 5. 常见故障排查 -->## 5. 常见故障排查

#<!-- chunk: 5.1 故障现象速查表 -->## 5.1 故障现象速查表
...
#<!-- chunk: 5.2 排障流程 (5 步法) -->## 5.2 排障流程 (5 步法)
...
#<!-- chunk: 5.5 NetworkPolicy 问题 -->## 5.5 NetworkPolicy 问题
...
#<!-- chunk: 5.7 Calico + Terway 集成排查 -->## 5.7 Calico + Terway 集成排查
...
<!-- chunk: 6. IP 泄漏紧急处理 -->## 6. IP 泄漏紧急处理

#<!-- chunk: 6.1 紧急响应流程 -->## 6.1 紧急响应流程
...
#<!-- chunk: 6.3 Finalizer 阻塞处理 -->## 6.3 Finalizer 阻塞处理
...
#<!-- chunk: 6.4 CRD Finalizer 阻塞深度排查 -->## 6.4 CRD Finalizer 阻塞深度排查
...
<!-- chunk: 7. 日常巡检清单 -->## 7. 日常巡检清单

#<!-- chunk: 7.1 每日巡检 -->## 7.1 每日巡检
...
<!-- chunk: 附录 A: 错误信息速查目录 -->## 附录 A: 错误信息速查目录
...
| [03-usage.md](./03-usage.md) | 使用指南: 安装配置、NetworkPolicy、固定 IP |
...
| 文档 | 说明 |
|:---|:---|
| [domain-03-networking-traffic/38-terway-gc-mechanism.md](../domain-03-networking-traffic/38-terway-gc-mechanism.md) | GC 机制详解: 架构、源码分析、执行流程 |
| [domain-03-networking-traffic/37-terway-resources-crud-operations.md](../domain-03-networking-traffic/37-terway-resources-crud-operations.md) | CRD 资源 CRUD 操作 |
| [domain-03-networking-traffic/05-terway-advanced-guide.md](../domain-03-networking-traffic/05-terway-advanced-guide.md) | Terway 高级指南: 容量规划、模式对比 |

#<!-- chunk: 关联 Topic -->## 关联 Topic
