...
### 案例 1: 节点内存压力导致批量 Pod 驱逐

| 时间 | 事件 |
|------|------|
| 03:00 | 监控告警: 节点 MemoryPressure=True |
| 03:01 | kubelet 开始驱逐 BestEffort Pod |
| 03:05 | 10+ Pod 被 Evicted，业务降级 |
| 03:08 | `kubectl top nodes` 显示内存 95% |
| 03:10 | 🟡 紧急扩容节点 + 调整 Pod memory limit |

**根因**: 日志收集 DaemonSet 内存泄漏，逐渐耗尽节点内存。

### 案例 2: 磁盘压力导致 Pod 无法写入

**现象**: Pod 日志写入失败，应用报错 "No space left on device"。

**诊断**: `df -h` 显示 /var/lib/kubelet 100%，容器日志未轮转

**修复**: 🟡 配置 containerLogMaxSize + 清理旧日志

## 升级决策点

| 级别 | 条件 | 动作 |
|------|------|------|
| P0 | 多节点资源压力 | 紧急扩容 + 检查泄漏 |
| P1 | 单节点压力 | 驱逐非必要 Pod |
| P2 | 资源规划 | 调整 request/limit |

## 面试要点

1. **Q: kubelet 的驱逐机制是怎样的？**
   A: kubelet 监控节点资源，达到 eviction threshold 时按 QoS 等级驱逐: BestEffort > Burstable > Guaranteed。驱逐顺序: 超出 request 最多的优先。

2. **Q: 三种 QoS 等级的划分标准？**
   A: Guaranteed: 所有容器设置 request=limit；Burstable: 至少一个容器设置 request 或 limit；BestEffort: 无任何资源设置。生产推荐 Guaranteed 或 Burstable。

3. **Q: 如何预防节点资源压力？**
   A: ① 设置合理的 Pod request/limit ② 配置 kubelet eviction-hard 阈值 ③ 监控节点资源使用率 ④ 日志轮转和清理策略 ⑤ 定期审查资源分配。

## Related

- [[26-技能/03-节点/node/skill-notready/skill-assets-escalation-template.md|skill-assets-escalation-template]] — Escalation Template
- [[26-技能/01-集群运维/cluster-upgrade/诊断排障/ts-cluster-operations.md|ts-cluster-operations]] — 集群运维故障排查
- [[26-技能/06-存储/csi-storage/诊断排障/ts-storage.md|ts-storage]] — 存储故障排查
