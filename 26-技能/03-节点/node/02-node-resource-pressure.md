...
|---|---------|---------|--------|---------|
| S1 | 节点 Condition `MemoryPressure=True`，Events `NodeHasInsufficientMemory` | `kubectl describe node` Conditions / Prometheus | 0.95 | 瞬时峰值已回落属 P3 观察项 |
| S2 | 节点 Condition `DiskPressure=True`，Events `NodeHasDiskPressure`/`EvictionThresholdMet` | `kubectl describe node` Conditions / Prometheus | 0.95 | 仅 inode 耗尽需单独确认 `df -i` |
| S3 | 节点 Condition `PIDPressure=True` | `kubectl describe node` Conditions / Prometheus | 0.95 | 硬限制过低属配置问题非泄漏 |
| S4 | Pod 状态为 `Evicted`，事件 `Evicted: The node was low on resource: memory/ephemeral-storage` | `kubectl get pods -A \| grep Evicted` + describe 看驱逐原因行 | 0.90 | 手动 drain/污点驱逐属维护操作 |
| S5 | Pod 被 `OOMKilled`（exit 137）且节点内存高水位 | `kubectl get events --field-selector reason=OOMKilled` + 节点内存指标 | 0.85 | 节点内存充足时属容器 limits 过低 → 转 SKILL-POD-001 |
| S6 | `ImagePullBackOff` + Events `failed to pull image ... no space left on device` | Pod Events + 节点 DiskPressure 同时成立 | 0.80 | 仓库侧失败（unauthorized/not found）转镜像技能 |
| S7 | Events `FailedScheduling: ... Insufficient memory/cpu` 且目标节点有 Pressure | Pod Events + 节点 Conditions 交叉确认 | 0.85 | 全集群容量不足 → 转集群扩缩容技能 |
