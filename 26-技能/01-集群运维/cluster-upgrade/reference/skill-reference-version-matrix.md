...
summary: '- 检查 `shutdownGracePeriod` 和 `shutdownGracePeriodCriticalPods` 配置'
category: skills
tags:
- k8s
- troubleshooting
...
| K8s 版本 | 支持状态 | 关键变更 |
|---|---|---|
| 1.34 | 当前稳定 | Gateway API GA |
| 1.33 | 维护中 | Sidecar 容器 GA |
| 1.32 | 维护中 | AppArmor GA |
| 1.31 | 已 EOL | nftables 后端 |
| 1.30 | 已 EOL | Pod Scheduling Readiness |

### CNI 插件兼容性

| CNI | K8s 1.32+ | K8s 1.30-1.31 | 备注 |
|---|---|---|---|
| Calico 3.28+ | ✅ | ✅ | 推荐 |
| Cilium 1.16+ | ✅ | ✅ | eBPF |
| Flannel 0.25+ | ✅ | ✅ | 简单场景 |
...
1. **Q：K8s 版本支持策略？**
   A：每个 minor 版本支持约 14 个月，同时维护 3 个 minor 版本。升级需逐版本进行。

2. **Q：如何规划集群升级？**
   A：关注 Release Notes→测试环境验证→检查废弃 API→备份 etcd→灰度升级→验证工作负载。

3. **Q：组件版本不兼容的风险？**
   A：API 废弃、功能异常、安全漏洞、性能问题。建议保持组件版本与 K8s 版本匹配。

## Related

- [[26-技能/03-节点/node/诊断排障/troubleshoot-node-issues.md|troubleshoot-node-issues]] — [[26-技能/03-节点/node/诊断排障/troubleshoot-node-issues.md|Troubleshoot Node Issues]]
- [[26-技能/04-工作负载/pod/方法论/FTA Diagnostic Execution Engine.md|[[FTA Diagnostic Execution Engine|FTA Diagnostic Execution Engine]]]] — FTA Diagnostic Execution Engine
- [[26-技能/04-工作负载/job-cronjob/skill-23-job-cronjob-failure.md|skill-23-job-cronjob-failure]] — Job/CronJob 故障诊断与修复 / Job & CronJob Failure Diagnosis & Remediation Remediation
