...
3. **安装 [[23-实体/02-K8s核心组件/kubernetes.md|Kubernetes 组件]]**：[[kubelet|kubelet]]、kubeadm、kubectl
...
### 案例 1: API Server 请求限流导致 kubectl 超时

| 时间 | 事件 |
|------|------|
| 15:00 | 所有 kubectl 命令超时，监控告警 |
| 15:05 | API Server 日志: "Too many requests" |
| 15:08 | 某 Operator 循环调用 API 耗尽限流配额 |
| 15:12 | 🟡 修复 Operator 逻辑 + 调整 --max-requests-inflight |

**根因**: Operator 代码 bug 导致无限循环 list/watch，耗尽 API Server 限流配额。

### 案例 2: etcd 磁盘 IOPS 不足导致集群响应慢

**现象**: kubectl 命令延迟 5-10s，API Server 日志显示 etcd 请求超时。

**诊断**: `etcdctl endpoint status` 显示 raft 延迟高，磁盘 IOPS 不足

**修复**: 🔴 升级 etcd 磁盘为 SSD/NVMe，或迁移到独立 etcd 集群

## 升级决策点

| 级别 | 条件 | 动作 |
|------|------|------|
| P0 | 控制平面不可用 | 检查 API Server + etcd |
| P1 | 响应延迟高 | 检查 etcd 磁盘和 API 限流 |
| P2 | 配置优化 | 调整限流和资源 |

## 面试要点

1. **Q: API Server 的关键配置参数？**
   A: --max-requests-inflight(默认 400)、--max-mutating-requests-inflight(默认 200)、--request-timeout(默认 60s)、--etcd-servers、--service-cluster-ip-range、--audit-log-path。

2. **Q: etcd 的性能要求？**
   A: etcd 对磁盘 IOPS 敏感，推荐 SSD(>10000 IOPS)；网络延迟 <1ms(RTT)；定期 compact 和 defrag；监控 etcd_disk_wal_fsync_duration_seconds(<10ms)。

3. **Q: 大规模集群的配置调优？**
   A: ① API Server: 增加副本数、调整限流 ② etcd: SSD + 独立集群 ③ Controller Manager: 增加并发数 ④ Scheduler: 调整百分比 ⑤ kubelet: 增加 maxPods。
