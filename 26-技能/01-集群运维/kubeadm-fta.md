...
### 案例 1: kubeadm init 失败——端口被占用

| 时间 | 事件 |
|------|------|
| 09:00 | 执行 `kubeadm init` 报错: "port 6443 is already in use" |
| 09:05 | `ss -tlnp | grep 6443` 发现旧 kube-apiserver 进程残留 |
| 09:08 | 🔴 `kubeadm reset -f` 清理残留 + 手动 kill 进程 |
| 09:12 | 重新 init 成功 |

**根因**: 上次 `kubeadm reset` 未完全清理，apiserver 静态 Pod 残留。

### 案例 2: 节点加入失败——token 过期

**现象**: `kubeadm join` 报错: "token has expired"。

**诊断**: kubeadm token 默认 24h 有效期

**修复**: 🟢 控制平面执行 `kubeadm token create --print-join-command` 生成新 join 命令

## 升级决策点

| 级别 | 条件 | 动作 |
|------|------|------|
| P0 | 控制平面初始化失败 | kubeadm reset + 重新 init |
| P1 | 节点加入失败 | 重新生成 join token |
| P2 | 集群配置需调整 | 修改 kubeadm-config |

## 面试要点

1. **Q: kubeadm init 的完整流程？**
   A: ① preflight 检查(端口/内核模块/swap) ② 生成 PKI 证书 ③ 生成 kubeconfig ④ 创建静态 Pod(apiserver/cm/scheduler/etcd) ⑤ 安装 DNS 和 kube-proxy ⑥ 生成 join token。

2. **Q: kubeadm 高可用集群的架构方案？**
   A: ① Stacked etcd: 控制平面节点同时运行 etcd(简单，3/5 节点) ② External etcd: 独立 etcd 集群(生产推荐，解耦) ③ 前置 LB 负载均衡 API Server VIP。

3. **Q: kubeadm reset 会清理哪些内容？**
   A: 停止容器、删除 /etc/kubernetes、/var/lib/etcd、/var/lib/kubelet、~/.kube/config；不会清理 CNI 配置、iptables/ipvs 规则、网络接口，需手动清理。
