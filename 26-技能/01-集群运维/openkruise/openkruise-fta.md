...
### 案例 1: OpenKruise CloneSet 原地升级失败

| 时间 | 事件 |
|------|------|
| 16:00 | CloneSet 原地升级(in-place update)卡住 |
| 16:05 | Pod 状态 ImagePullBackOff，新镜像不存在 |
| 16:08 | 镜像 tag 拼写错误 |
| 16:10 | 🟡 修正镜像，`kubectl patch cloneset` 触发重新升级 |

**根因**: 原地升级直接替换容器镜像，不重建 Pod，镜像错误时无法回退到旧容器。

### 案例 2: OpenKruise DaemonSet 分批发布卡住

**现象**: 分批发布第 2 批等待中，第 1 批 Pod 未 Ready。

**诊断**: `kubectl get daemonset -o yaml` 查看 partition 和 updatedNumberReady

**修复**: 🟢 修复第 1 批 Pod 问题，或调整 partition 跳过

## 升级决策点

| 级别 | 条件 | 动作 |
|------|------|------|
| P0 | 原地升级导致服务中断 | 回滚镜像版本 |
| P1 | 分批发布卡住 | 检查当前批次 Pod 状态 |
| P2 | OpenKruise 控制器异常 | 检查 kruise-manager Pod |

## 面试要点

1. **Q: OpenKruise CloneSet 与原生 Deployment 的区别？**
   A: CloneSet 支持: ① 原地升级(in-place update，不重建 Pod) ② 指定 Pod 缩容 ③ 分批发布(partition) ④ 镜像预热 ⑤ 更灵活的更新策略(MaxSurge/MaxUnavailable/Partition)。

2. **Q: OpenKruise 原地升级的原理？**
   A: 只替换 Pod 中的容器镜像并重启容器，保持 Pod IP、挂载卷、所在节点不变。通过 CRI 接口更新容器，减少调度开销，适合大规模发布。

3. **Q: OpenKruise 的核心组件有哪些？**
   A: kruise-manager(控制器: CloneSet/Advanced StatefulSet/SidecarSet/BroadcastJob 等) + kruise-daemon(节点代理: 镜像预热/容器重启)。
...
- [[26-技能/03-节点/node/skill-notready/skill-k8s-node-notready-SKILL.md|SKILL]].md|skill-k8s-node-notready-SKILL]] — Skill
- [[kudig-prompts-catalog]] — KUDIG Prompt 模板集：故障排查、架构评审、配置生成与学习路径
- [[26-技能/04-工作负载/pod/培训/learn-decision-tree-mermaid.md|learn-decision-tree-mermaid]] — 故障排查决策树 - Mermaid 可视化版
- [[openkruise]] — OpenKruise

- [[nginx-ingress-fta]]
- [[19-故障诊断/06-FTA故障树/list/openkruise-fta.md|OpenKruise 工作负载异常故障树分析]]
