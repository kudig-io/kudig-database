...
- [[26-技能/04-工作负载/pod/运维操作/configure-health-probes.md|配置健康探针]]
- [[deployment|Deployment]]

## 生产案例

### 案例 1: 集群证书批量过期导致全集群不可用

| 时间 | 事件 |
|------|------|
| 00:00 | 所有节点 kubelet 报错: certificate has expired |
| 00:05 | kubectl 无法连接 API Server |
| 00:10 | SSH 登录控制平面，`kubeadm certs check-expiration` 确认全部过期 |
| 00:15 | 🔴 `kubeadm certs renew all` + 重启控制平面组件 |
| 00:25 | 集群恢复 |

**根因**: 未配置证书自动轮换，未设置过期监控告警。

### 案例 2: 集群扩容时新节点 kubelet 版本不兼容

**现象**: 新节点加入后 NotReady，kubelet 日志显示 API 版本不支持。

**诊断**: 新节点 kubelet 1.32，控制平面 1.28，偏差超过 2 minor

**修复**: 🟡 安装与控制平面兼容的 kubelet 版本

## 升级决策点

| 级别 | 条件 | 动作 |
|------|------|------|
| P0 | 集群完全不可用 | 检查证书 + etcd + 控制平面 |
| P1 | 部分节点异常 | 检查 kubelet 和证书 |
| P2 | 生命周期管理 | 规划升级和证书续期 |

## 面试要点

1. **Q: 集群生命周期管理包含哪些内容？**
   A: ① 创建(kubeadm init) ② 扩容(join) ③ 升级(upgrade) ④ 证书管理(renew) ⑤ 缩容/移除节点 ⑥ 备份恢复 ⑦ 集群销毁(reset)。

2. **Q: 如何自动化集群生命周期管理？**
   A: ① Cluster API(CAPI): 声明式集群管理 ② Ansible/Terraform: 基础设施自动化 ③ 云托管服务(ACK/EKS/GKE): 云厂商管理控制平面 ④ GitOps: 集群配置版本化。

3. **Q: 集群升级的零停机策略？**
   A: ① 多控制平面 + LB ② 逐节点 cordon+drain+升级+uncordon ③ PDB 保护关键服务 ④ 先升级控制平面再升级节点 ⑤ 每次只升级一个 minor。
