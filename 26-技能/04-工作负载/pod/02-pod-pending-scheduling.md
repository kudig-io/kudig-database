...
|---|---------------------------|---------|:---:|---------|
| S1 | Pod STATUS 为 `Pending` 且无 nodeName | `kubectl get pod -o wide` NODE 列为 `<none>` | 0.90 | 有 nodeName 则属启动阶段问题 → S6/转 03 |
| S2 | Events 出现 `FailedScheduling: Insufficient cpu/memory` | `kubectl describe pod` Events 段 | 0.95 | 集群容量不足需扩容 → 转 [[26-技能/01-集群运维/cluster/04-cluster-autoscaling-sop.md|集群扩缩容]] |
| S3 | Events 出现 `node(s) didn't match Pod's node affinity/selector` | `kubectl describe pod` Events 段 | 0.95 | 标签刚变更需确认节点标签现状 |
| S4 | Events 出现 `node(s) had untolerated taint {...}` | `kubectl describe pod` Events 段 | 0.95 | 节点处于计划维护（cordon）属预期行为 |
| S5 | Events 出现 `exceeded quota: ... requested: ... limited: ...` | `kubectl describe pod` / `kubectl describe quota -n <ns>` | 0.95 | 非 ResourceQuota 的 `Insufficient` 属 S2 分支 |
| S6 | Events 出现 `pod has unbound immediate PersistentVolumeClaims` | `kubectl describe pod` + `kubectl get pvc -n <ns>` | 0.90 | PVC `WaitForFirstConsumer` 下短暂 Pending 属正常 |
| S7 | Events 出现 `no nodes available to schedule pods` | `kubectl describe pod` Events 段 | 0.90 | 节点批量 NotReady → 转 [[26-技能/03-节点/node/01-node-notready-diagnosis.md|Node NotReady 诊断]] |
| S8 | Pod 状态为 `SchedulingGated` | `kubectl get pod` STATUS 列（1.27+） | 0.90 | 属 schedulingGates 机制，需确认门控控制器行为 |

**工单关键词映射**：`Pending`、`FailedScheduling`、`无法调度`、`Insufficient`、`taint`、`扩容失败`、`unbound PVC` → 触发本技能。
...
### Step 1.3 常见错误消息与事件日志速查

> 以下错误消息和事件日志是 Pod Pending/调度失败场景的高频诊断线索。Agent 在采集 Events 后可直接匹配本表快速路由。

#### FailedScheduling 事件完整消息模式

| 事件 Message 模式 | 含义 | 检测命令 | 路由 |
|------------------|------|---------|------|
| `0/N nodes are available: N Insufficient cpu.` | 所有节点 CPU 分配已满 | `kubectl describe pod <pod> \| grep -A5 FailedScheduling` | → 路径 A: 资源不足 |
| `0/N nodes are available: N Insufficient memory.` | 所有节点内存分配已满 | 同上 | → 路径 A: 资源不足 |
| `0/N nodes are available: N node(s) had taint {key: value}, that the pod didn't tolerate.` | 污点未被容忍 | 同上 | → 路径 B: 污点/容忍 |
| `0/N nodes are available: N node(s) didn't match Pod's node affinity/selector.` | 节点选择器/亲和性不匹配 | 同上 | → 路径 C: 选择器 |
| `0/N nodes are available: N node(s) had untolerated taint {node.kubernetes.io/not-ready: }.` | 节点 NotReady 污点 | 同上 | → 转 [[26-技能/03-节点/node/01-node-notready-diagnosis.md\|Node NotReady 诊断]] |
| `0/N nodes are available: N node(s) had untolerated taint {node.kubernetes.io/unreachable: }.` | 节点不可达污点 | 同上 | → 转 Node NotReady 诊断 |
| `0/N nodes are available: N node(s) were unschedulable.` | 节点被 cordon（SchedulingDisabled） | `kubectl get nodes \| grep SchedulingDisabled` | → 检查是否人工 cordon |
| `0/N nodes are available: N node(s) didn't match pod anti-affinity rules.` | Pod 反亲和性冲突 | `kubectl get pod -o jsonpath='{.spec.affinity.podAntiAffinity}'` | → 路径 C: 亲和性 |
| `0/N nodes are available: N node(s) didn't satisfy pod topology spread constraints.` | 拓扑分布约束无法满足 | `kubectl get pod -o jsonpath='{.spec.topologySpreadConstraints}'` | → 检查拓扑域 |
| `pod has unbound immediate PersistentVolumeClaims` | PVC 未绑定（Immediate 模式） | `kubectl get pvc -n <ns>` | → 路径 E: PVC |
| `persistentvolumeclaim "<name>" not found` | PVC 不存在 | `kubectl get pvc <name> -n <ns>` | → 路径 E: PVC |
| `exceeded quota: <quota-name>, requested: <resource>` | 命名空间 ResourceQuota 超限 | `kubectl describe resourcequota -n <ns>` | → 路径 D: 配额 |
| `must request at least <resource> (LimitRange)` | LimitRange 强制最小资源 | `kubectl get limitrange -n <ns> -o yaml` | → 调整 requests |
| `failed to get pvc <name> datasource` | PVC 数据源（快照/克隆）不可用 | `kubectl describe pvc <name> -n <ns>` | → 检查 VolumeSnapshot |

#### 调度器日志关键错误（kube-scheduler）

```bash
# 🟢 低风险：只读/信息收集
kubectl logs -n kube-system -l component=kube-scheduler --tail=100 | grep -iE "error|failed|unable|timeout"
```

| 日志模式 | 含义 | 处置 |
|---------|------|------|
| `Error scheduling pod; ... no nodes available` | 无可用节点（全部 NotReady/Unschedulable） | 检查节点池健康状态 |
| `Attempting to schedule pod ... but all nodes are filtered` | 所有节点被过滤器排除 | 检查 taint/affinity/resource 过滤 |
| `Preemption is not helpful for scheduling` | 抢占也无法解决（资源绝对不足） | 扩容节点 |
| `Error while reading pod ... from cache` | 调度器缓存异常 | 重启 kube-scheduler（🔴 高风险） |
| `binding rejected: ... already bound` | Pod 已被绑定（竞态） | 通常自愈，观察即可 |

#### Pod Conditions 关键状态（`kubectl describe pod` → Conditions）

| Condition | Status | Message 模式 | 含义 |
|-----------|--------|-------------|------|
| `PodScheduled` | `False` | `0/N nodes are available: ...` | 调度失败（核心诊断入口） |
| `PodScheduled` | `False` | `waiting for a volume to be created` | 等待 PVC 绑定/供给 |
| `PodScheduled` | `True` | — | 已调度成功（Pending 原因在节点侧） |
| `Initialized` | `False` | `containers with incomplete status: <init-container>` | Init 容器未完成 |
| `ContainersReady` | `False` | — | 容器未就绪（可能仍在拉取镜像） |

#### SchedulingGated 状态（v1.28+）

```bash
# 🟢 低风险：只读/信息收集
# 检查 Pod 是否被调度门控阻塞
kubectl get pod <pod> -n <ns> -o jsonpath='{.spec.schedulingGates}'
kubectl get pod <pod> -n <ns> -o jsonpath='{.status.conditions[?(@.type=="PodScheduled")].message}'
```

| 状态 | 含义 | 处置 |
|------|------|------|
| `STATUS=SchedulingGated` | Pod 被 schedulingGates 阻塞，未进入调度队列 | 检查谁设置了 gate（Operator/Webhook） |
| Events 含 `SchedulingGated` | 调度器跳过该 Pod | 移除 schedulingGates 字段 |
