...
- Feeds execution results back to the [[26-技能/04-工作负载/pod/方法论/FTA Diagnostic Execution Engine.md|FTA Diagnostic Execution Engine]] learning loop
...
| 组件域 | FTA 数量 | 核心技能 | 难度 |
|----------|----------|----------|------|
| 工作负载 | 5 | Pod/Deployment/StatefulSet/DaemonSet/Job | L1-L2 |
| 网络 | 8 | Service/DNS/Ingress/Gateway/NetworkPolicy/CNI | L2-L3 |
| 存储 | 2 | CSI/PV/PVC | L2 |
| 调度 | 4 | HPA/VPA/ClusterAutoscaler/ResourceQuota | L2-L3 |
| 控制平面 | 4 | kubeadm/证书/CRD/CloudProvider | L3 |
| 安全 | 3 | RBAC/PSA/PDB | L2-L3 |
| 可观测性 | 2 | 监控/日志 | L2 |
| 发布 | 3 | GitOps/ServiceMesh/OpenKruise | L3 |

### 诊断工具链

```
症状 → FTA 故障树 → 诊断命令 → 根因 → 修复方案
  │         │            │          │         │
  └── FEBM 证据法    └── kubectl   └── 置信度  └── 风险等级
```

### 技能认证路径

1. **CKA 基础**: Pod/Deployment/Service/NetworkPolicy
2. **CKS 安全**: RBAC/PSA/NetworkPolicy/审计
3. **SRE 进阶**: FTA/FEBM/性能调优/容灾
4. **架构师**: 多集群/Service Mesh/AI Agent
...
- [[26-技能/04-工作负载/pod/方法论/skill-reference-remediation-playbook.md|skill-reference-remediation-playbook]] — Remediation Playbook
