...
用户/告警
    │
    ├── Router Agent (意图识别 + FTA 路由)
    │
    ├── Diagnostic Agent (执行 FTA 诊断)
    │       ├── 命令执行器
    │       ├── 输出解析器
    │       └── 分支决策器
    │
    ├── Remediation Agent (执行修复)
    │       ├── 风险评估
    │       ├── 修复执行
    │       └── 验证确认
    │
...
| 模式 | 描述 | 适用场景 |
|------|------|----------|
| 串行 | 一个 Agent 完成后交给下一个 | 简单诊断流程 |
| 并行 | 多个 Agent 同时执行 | 多 FTA 并行诊断 |
| 分层 | 主 Agent 调度子 Agent | 复杂多步骤任务 |
| 辩论 | 多 Agent 独立诊断后投票 | 不确定根因 |

### 安全护栏

1. **权限最小化**: Agent 只有只读权限，修复需人工确认
2. **操作审计**: 所有 Agent 操作记录到审计日志
3. **超时控制**: 每个步骤设置超时，防止无限循环
4. **人工升级**: 置信度 <0.8 时自动升级给人工
5. **回滚机制**: 每个修复操作都有对应的回滚方案
