...
summary: FTA-Driven Runbook Automation — Kubernetes 生产运维知识库
category: skill
tags:
- k8s
- fta
...
| 等级 | 描述 | 适用场景 |
|------|------|----------|
| L1 手动 | 人工执行 Runbook 步骤 | 高风险操作 |
| L2 半自动 | 自动诊断 + 人工确认修复 | 中风险操作 |
| L3 自动 | 全自动诊断+修复+验证 | 低风险/已知问题 |
| L4 自愈 | 预测性维护 + 自动修复 | 成熟场景 |

### 生产环境自动化建议

1. **先诊断后修复**: 诊断全自动，修复需确认
2. **灰度执行**: 先在一个节点/Pod 验证
3. **回滚预案**: 每个修复步骤都有回滚方案
4. **审计日志**: 记录所有自动化操作
5. **人工升级**: 失败时自动升级给 On-call
...
- [[26-技能/04-工作负载/pod/方法论/FTA Methodology and Core Principles.md|[[FTA Methodology and Core Principles|FTA Methodology and Core Principles]]]]
