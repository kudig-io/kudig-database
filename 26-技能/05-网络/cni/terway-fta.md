...
> 以下错误消息和事件日志是 Terway CNI 故障场景的高频诊断线索。Agent 在采集 Events 和 Terway 日志后可直接匹配本表快速路由。

#### 关键 Events（`kubectl get events` / `kubectl describe pod`）

| 事件 Reason | 事件 Message 模式 | 含义 | 检测命令 | 路由 |
|-------------|------------------|------|---------|------|
| `FailedCreatePodSandBox` | `Failed to create pod sandbox: rpc error: ... failed to setup network for sandbox: plugin type="terway" failed (add): error alloc ip: ...` | Terway 分配 IP 失败 | `kubectl get events -n <ns> --field-selector reason=FailedCreatePodSandBox` | → RC-002/RC-005 |
| `FailedCreatePodSandBox` | `... plugin type="terway" failed (add): bindquota exceeded` | ENI 配额耗尽 | 同上 | → RC-002 |
| `FailedCreatePodSandBox` | `... plugin type="terway" failed (add): timeout waiting for IP` | IP 分配超时（池耗尽/API 慢） | 同上 | → RC-005/RC-009 |
| `FailedCreatePodSandBox` | `... plugin type="terway" failed (add): no available vSwitch` | vSwitch 无可用 IP | 同上 | → RC-005 |
| `FailedCreatePodSandBox` | `... NetworkPluginNotReady: network plugin is not ready: CNI plugin not initialized` | Terway DaemonSet 未就绪 | 同上 | → RC-001 |
| `FailedCreatePodSandBox` | `... plugin type="terway" failed (add): AttachNetworkInterface: ...` | ENI 绑定 API 调用失败 | 同上 | → RC-003 |
| `FailedCreatePodSandBox` | `... context deadline exceeded` | 操作超时（API Server/云平台） | 同上 | → RC-009 |
| `SandboxChanged` | `Pod sandbox changed, it will be killed and re-created` | 沙箱变更（网络配置更新） | `kubectl get events -n <ns> --field-selector reason=SandboxChanged` | 通常自愈，观察即可 |

#### Terway DaemonSet 日志关键错误（`kubectl logs -n kube-system -l app=terway`）

```bash
# 🟢 低风险：只读/信息收集
kubectl logs -n kube-system -l app=terway --tail=100 | grep -iE "error|failed|exceeded|timeout|quota|bindquota"
```

| 日志模式 | 含义 | 对应根因 | 修复方向 |
|---------|------|---------|----------|
| `bindquota exceeded` / `QuotaExceeded.Ip` | ENI/IP 配额耗尽 | RC-002 | 提升配额或切换 ENIIP 模式 |
| `AttachNetworkInterface failed` / `InvalidOperation.Bindable` | ENI 绑定失败（云平台 API 错误） | RC-003 | 等待恢复/联系阿里云 |
| `DetachNetworkInterface failed` / `IncorrectEniStatus` | ENI 解绑失败/状态异常 | RC-004 | 重启 Terway Pod 触发同步 |
| `no available vSwitch` / `InsufficientIpAddressCount` | vSwitch IP 耗尽 | RC-005 | 扩展 vSwitch/添加新 vSwitch |
| `pool is empty` / `no available IP in pool` | 本地 IP 池耗尽 | RC-006 | 重启 Terway Pod 刷新缓存 |
| `context deadline exceeded` / `connection refused` | API Server 连接超时/拒绝 | RC-009 | 检查 API Server 健康状态 |
| `security group ... not found` / `InvalidSecurityGroupId` | 安全组配置错误 | RC-007 | 检查安全组 ID 配置 |
| `route table ... not found` / `InvalidRouteEntry` | VPC 路由表异常 | RC-008 | 检查路由表配置 |
| `eni mode mismatch` / `config error` | Terway 配置与实际模式不匹配 | RC-010 | 修正 ConfigMap |
| `panic: runtime error` | Terway 崩溃 | RC-001 | 重启 Terway DaemonSet |
...
# 检查 Terway 网络接口状态
ssh <node-ip> "ip link show | grep -E 'eth|eni|veth'"
ssh <node-ip> "ip addr show | grep 'inet ' | grep -v '127.0.0.1'"

# 检查 Terway 本地状态
kubectl exec -n kube-system <terway-pod> -- terway-cli show
kubectl exec -n kube-system <terway-pod> -- terway-cli mapping

# 检查安全组规则（需 aliyun CLI）
aliyun ecs DescribeSecurityGroupAttribute --SecurityGroupId ${SG_ID} --Direction ingress
```

| 节点级现象 | 含义 | 对应根因 |
|-----------|------|----------|
| `terway-cli show` 显示 ENI 数 = 实例上限 | ENI 配额耗尽 | RC-002 |
| `terway-cli mapping` 显示已释放 IP 仍被占用 | IP 池缓存不一致 | RC-006 |
| 节点无 `eth1`/`eni` 接口 | ENI 未绑定 | RC-003/RC-004 |
| 安全组无 Pod CIDR 入方向规则 | 跨节点流量被拦截 | RC-007 |
| VPC 路由表无 Pod CIDR 条目 | 跨节点路由缺失 | RC-008 |
...
- [[26-技能/05-网络/cni/flannel-fta.md|Flannel 网络异常诊断]] — 同域技能
- [[26-技能/03-节点/node/01-node-notready-diagnosis.md|Node NotReady 诊断]] — 跨域关联
- [[21-生态参考/03-领域索引/terway-index.md|Terway 知识图谱索引]] — 知识索引
