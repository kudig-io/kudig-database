...
class ComplianceReporter:
    ...
    def create_report_content(self, compliance_data: Dict, security_events: List[Dict], 
        ...
        **Period**: {{ start_date.strftime('%Y-%m-%d') }} to {{ end_date.strftime('%Y-%m-%d') }}
        ...
        <!-- chunk: Compliance Scores -->## Compliance Scores
        ...
        <!-- chunk: Security Events Summary -->## Security Events Summary
        {% for event in events %}
        - **{{ event.timestamp.strftime('%Y-%m-%d %H:%M') }}**: {{ event.type }} ({{ event.severity }}) - {{ event.description }}
        {% endfor %}
        
        <!-- chunk: Vulnerability Statistics -->## Vulnerability Statistics
        ...
        <!-- chunk: Recommendations -->## Recommendations
...
- [[安全/README.md|[[Domain 25: 云原生安全 (Cloud Native Security)|Domain 25: 云原生安全 (Cloud Native Security)]]]]
