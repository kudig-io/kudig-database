...
- **本领域（domain-98）覆盖**: 从集群外部到 Service 的南北向流量入口管理
- **domain-26 覆盖**: Service 到 Service 的东西向流量治理（Istio、Linkerd 等服务网格）
- **domain-5 覆盖**: Kubernetes 网络基础（CNI、Service、DNS、NetworkPolicy）
...
> 详细的 Gateway API 技术解析请参考 [02-Kubernetes Gateway API 标准深度解析](./02-kubernetes-gateway-api-deep-dive.md) 以及 [Domain-5: 网络 - Gateway API 概览](../domain-5-networking/35-gateway-api-overview.md)。
...
- [Domain-5: 网络 - Ingress 与 API Gateway 对比表](../domain-5-networking/36-api-gateway-patterns.md)
- [Domain-26: 服务网格与微服务治理](../domain-26-service-mesh-microservices)
