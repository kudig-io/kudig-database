---
title: K8s 存储高级快问快答（15 题）
description: 面试向 K8s 存储高级快问快答：CSI 架构、Attach/Mount 两阶段、拓扑调度、PVC 扩容、快照体系、回收策略、StatefulSet 存储模式、性能调优、分布式存储选型、备份容灾、加密、卷模式与挂载权限、删除保护、CSI 迁移
summary: 15 道 K8s 存储高难度口述自测题（CSI/Attach/拓扑/扩容/快照/StatefulSet/性能/选型/容灾/加密/卷模式/删除保护/CSI 迁移），含话题覆盖清单与语料深挖链接，配套 22 号入门速查与 21 号联动机制面经使用。
category: 存储
tags:
- k8s
- storage
...
入门版 61 题速查见 [[06-存储/01-K8s存储/22-k8s-storage-quick-qa.md|K8s 存储快问快答（61 题）]]，联动机制详解见 [[06-存储/01-K8s存储/21-storageclass-pvc-pv-interview-notes.md|StorageClass、PVC、PV 联动机制面经]]。本文 15 道高难度题与网络域快问快答同体例：先遮住答案口述，再对照关键词补全，最后沿"深挖"链接回语料源文件精读。
...
| 13 | 卷模式与挂载权限 | volumeMode Filesystem vs Block、volumeDevices、fsGroup 权限坑 |
| 14 | 删除保护体系 | pvc-protection/pv-protection finalizer、与 VA finalizer 多层防护 |
| 15 | CSI 迁移 | In-Tree 插件移除、CSIMigration 翻译层、存量 StorageClass 零修改 |

**知识地图**：15 题归并为六大板块——答题先定位板块，再展开考点；每片叶子对应正文某题的拿分点。
...
**CSIStorageCapacity**：它补的是"调度成功但容量不足"——调度器把卷调度到节点后，CreateVolume 才发现后端池容量不够，Pod 反复重建。有了 CSIStorageCapacity 对象（KEP-2317），调度器在**调度时**就能校验目标拓扑下的剩余容量，提前把 Pod 排除。
...
```mermaid
flowchart LR
    A["kubectl patch PVC<br/>100Gi 改 200Gi"] --> B["external-resizer"]
    B --> C["ControllerExpandVolume<br/>云盘侧扩容"]
    C --> D["NodeExpand / 手动<br/>resize2fs · xfs_growfs"]
    D --> E["PVC status.capacity 更新"]
    C -->|"驱动不支持在线"| F["离线路径<br/>scale 0 → 扩容 → 恢复副本"]
```
...
```mermaid
flowchart LR
    subgraph S1["快照 · 同后端 · 分钟级 · 管 RPO"]
        V["源卷"] -->|"COW 写时复制"| S["时间点副本"]
    end
    subgraph S2["备份 · 独立存储 · 抗灾难"]
        S -->|"导出拉取"| O[("对象存储<br/>可跨区域")]
        O --> R["跨集群 / 跨区恢复"]
    end
```
...
**第一性问题**：先问"要块还是文件、要 RWX 吗、能接受多高的运维复杂度"——需要 RWX 再谈文件系统（NFS/CephFS/JuiceFS）。只要 RWO 块存储，运维复杂度从低到高是云盘 CSI < Longhorn < Ceph；阿里云 ACK 生产首选云盘 CSI（更高 IOPS 与 SLA），Longhorn 作补充，Ceph 用于大规模统一存储。JuiceFS 的缓存策略（节点缓存命中率）直接决定 AI 场景实际吞吐。
...
```mermaid
flowchart TD
    subgraph L1["第 1 层 · CSI 快照 —— 管 RPO"]
        S["高频定时快照<br/>同后端 · 分钟级<br/>RPO ≈ 快照间隔"]
    end
    subgraph L2["第 2 层 · Velero —— 管可重建"]
        V["资源 YAML + PV 数据<br/>对象存储 · 跨集群恢复<br/>RTO ≈ 恢复速度"]
    end
    subgraph L3["第 3 层 · 后端复制 —— 管基础设施冗余"]
        R["云盘跨 AZ 复制 / Ceph 池副本"]
    end
```
...
### 13. `volumeMode` 的 Filesystem 与 Block 有什么本质区别？Pod 里怎么用？挂载报 Permission Denied 和 fsGroup 有什么关系？

> **一句话**：Filesystem 先格式化再挂 `volumeMounts`，Block 把裸设备经 `volumeDevices` 直接交给应用（数据库裸设备/自定义 FS）；Permission Denied 常因卷属主与容器进程 uid/gid 不匹配，用 `securityContext.fsGroup` 补齐。
...
**Block 模式用法**：PVC 声明 `volumeMode: Block`，Pod 在 `volumeDevices` 里声明 `devicePath`，应用拿到的是 `/dev/xvdb` 这样的裸设备——格式化与文件系统扩展全部由应用自己管理（第 5 题的 `resize2fs` 自动扩展不再适用）。
...
**挂载 Permission Denied**：卷挂上来了但容器进程读写报错，第一嫌疑是卷内文件属主与容器进程 uid/gid 不匹配——`securityContext.fsGroup` 让 kubelet 把卷属组 chown 给指定组并附加进容器的 supplemental groups；NFS/SAN 共享存储还要核对导出侧权限与 SELinux 标签。

深挖：[[06-存储/01-K8s存储/04-pvc-patterns-practices.md|PVC 模式与实践]]、[[06-存储/05-存储网络/01-san-nas-architecture.md|SAN/NAS 架构]]

### 14. PV/PVC 自带哪两个保护 finalizer？它们与 VolumeAttachment 的 finalizer 如何构成多层防误删体系？

> **一句话**：`pvc-protection` 挡住「Pod 还在用就删 PVC」，`pv-protection` 挡住「绑定中直接删 PV」，再叠 VA 的 `external-attacher` finalizer——三层 finalizer 让删除必须按 Pod→PVC→PV→后端逐层解锁。

| finalizer | 挂在谁身上 | 挡住什么 |
|-----------|-----------|---------|
| `kubernetes.io/pvc-protection` | PVC | PVC 被 Pod 引用期间不可删——所有引用 Pod 终止后控制器才摘除 finalizer 放行删除 |
| `kubernetes.io/pv-protection` | PV | PV 绑定到 PVC 期间不可直接删——必须先删 PVC（进 Released 再走回收） |
| `external-attacher/<driver>` | VolumeAttachment | ControllerUnpublishVolume 完成前 VA 不会消失（第 2 题：强摘有双挂载风险） |

```mermaid
flowchart TD
    A["kubectl delete pvc"] --> B{"pvc-protection:<br/>还有 Pod 在用？"}
    B -->|"是"| C["PVC 卡在 Terminating<br/>所有引用 Pod 终止后才放行"]
    B -->|"否"| D["PVC 删除<br/>PV 进 Released"]
    E["kubectl delete pv"] --> F{"pv-protection:<br/>PVC 已删？"}
    F -->|"否"| G["PV 删除被阻止"]
    F -->|"是"| H["PV 可回收：<br/>Delete 删后端真卷 / Retain 留数据"]
```

**与第 7 题的分工**：`reclaimPolicy` 决定「删除动作发生时后端数据怎么办」，protection finalizer 决定「删除动作现在能不能发生」——两者组合出完整的防误删语义。PVC 长期卡 Terminating，先看还有没有 Pod 引用它（Pod 卡 Terminating、节点假死最常见）；确认无引用后仍卡住，再查 protection controller 日志，**强摘 finalizer 是最后手段**。

深挖：[[06-存储/01-K8s存储/02-pv-architecture-fundamentals.md|PV/PVC 架构基础]]、[[06-存储/01-K8s存储/20-volume-attachment-controller.md|VolumeAttachment 与 Attach/Detach 控制器]]

### 15. In-Tree 存储插件为什么被移除？CSI Migration 的翻译机制是什么？存量 StorageClass 需要改吗？

> **一句话**：In-Tree 插件跟 K8s 发版耦合、修个 Bug 都要升级集群，v1.26 起主流云厂商插件代码已从 K8s 删除；CSI Migration 在 kube-controller-manager/kubelet 内置翻译层，把 In-Tree 操作重定向到等价 CSI 驱动——存量 StorageClass 的 provisioner 写法**无需修改**。

**为什么要迁移**：In-Tree 插件的更新节奏被绑死在 K8s 发版周期上，新功能、Bug 修复、安全补丁都要等集群升级；CSI 驱动独立发版、热更新 CSI Pod 即生效、插件化开发。时间线：v1.14 Alpha → v1.23 GA（AWS/GCE/Azure/OpenStack）→ v1.26 删除 AWS/GCE/Azure/OpenStack/vSphere 等 In-Tree 插件代码 → v1.29 移除 Portworx/StorageOS。

```mermaid
flowchart LR
    PVC["PVC / PV<br/>provisioner: kubernetes.io/aws-ebs"]
    TG["CSI Migration 翻译层<br/>kube-controller-manager / kubelet 内置"]
    DRV["CSI Driver<br/>ebs.csi.aws.com"]
    BK[("真实云盘")]
    PVC --> TG -->|"CSIMigration* 门控（GA v1.23+）"| DRV --> BK
```

**四个关键点**：① `CSIMigration` 总开关 + 每插件门控（`CSIMigrationAWS` 等，v1.23 GA），把 In-Tree 操作重定向到 CSI；② `InTreePluginAWSUnregister`（v1.26+）彻底注销 In-Tree 插件；③ 翻译层自动把 `provisioner: kubernetes.io/aws-ebs` 映射到 `ebs.csi.aws.com`、参数转成 CSI 等价项——**存量 StorageClass 零修改**；④ 前提是节点已安装对应 CSI 驱动，出问题可关闭 feature gate 回滚。

深挖：[[06-存储/01-K8s存储/17-csi-migration-in-tree-to-csi.md|CSI 迁移：从 In-Tree 到 CSI]]、[[06-存储/05-存储网络/02-csi-driver-architecture.md|Kubernetes CSI Driver 架构与实现]]
...
| 题号 | 一句话答案 | 记忆钩子 |
|------|-----------|---------|
| 1 | CSI = Identity/Controller/Node 三个 gRPC 服务；sidecar 监听 API 对象，kubelet 走 Unix socket 连 Node 服务 | 三服务 + 四 sidecar + 一 socket |
| 2 | Attach 节点级可见（控制面），Mount Pod 级挂载（kubelet）；VA 是 attach 事实存档，摘 finalizer 有双挂载风险 | 先可见，再挂载 |
| 3 | 排查链路八层：Events→PVC→PV→SC→VA→CSI Controller→CSI Node→后端；Multi-Attach = 假死未 detach + 强制漂移 | 八层逐级验 |
| 4 | WFFC 延迟建卷到调度选节点后，卷随 Pod 同拓扑；CSIStorageCapacity 让调度时校验容量 | 卷跟 Pod 走 |
| 5 | 扩容三步：patch PVC → 云盘扩容 → 文件系统扩展；只能扩不能缩，仅动态供给 | 扩不缩，动才扩 |
| 6 | 快照 = 同后端 COW 时间点（crash-consistent）；备份 = 独立存储完整副本；应用一致靠冻结 | 快照不等于备份 |
| 7 | Delete 删真数据；Retain 进 Released，清 claimRef 才能重绑 | Released 清引用 |
| 8 | volumeClaimTemplates 每副本一盘且不可变；缩容 PVC 残留防误删；数据库网络盘、吞吐型才 Local PV | 模板不可变 |
| 9 | 性能三维 IOPS/吞吐/延迟；调参三处：SC 性能等级、mountOptions、fsType | 三维三处 |
| 10 | 先问块还是文件、要不要 RWX、运维复杂度；RWX 才谈 NFS/CephFS/JuiceFS | 先问三个问题 |
| 11 | 快照管 RPO、Velero 管可重建、后端复制管冗余；RPO≈间隔，RTO≈恢复速度 | 三层三管 |
| 12 | 卷级加密一行参数个位数损耗、密钥交 KMS；AI 存储 = 对象底座 + POSIX 层 + 节点缓存 | 分层救吞吐 |
| 13 | Filesystem 格式化后挂 volumeMounts，Block 裸设备走 volumeDevices；Permission Denied 用 securityContext.fsGroup 补 gid | 裸设备自己管 |
| 14 | pvc-protection 挡「使用中删 PVC」，pv-protection 挡「绑定中删 PV」，叠 VA finalizer 成三层；PVC Terminating 先看引用 Pod | 删除逐层解锁 |
| 15 | In-Tree 随发版弃用，CSIMigration 翻译层把旧 provisioner 重定向到 CSI；存量 StorageClass 零修改 | 翻译层接管 |
...
- [[06-存储/01-K8s存储/04-pvc-patterns-practices.md|PVC 模式与实践]]
- [[06-存储/01-K8s存储/17-csi-migration-in-tree-to-csi.md|CSI 迁移：从 In-Tree 到 CSI]]
