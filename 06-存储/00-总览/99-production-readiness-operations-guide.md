...
本指南聚焦将 `存储` 的存储能力从“可用”推进到“可长期稳定运行”，重点补齐当前知识库在**存储多租户治理、临时存储管理、CSI 供应链安全、容量规划、存储混沌演练**五个方向的缺口。阅读前建议先熟悉 [[06-存储/01-K8s存储/01-storage-architecture-overview.md|存储架构概览]] 与 [[06-存储/01-K8s存储/07-storage-daily-operations.md|存储日常运维操作手册]]。
...
通用诊断脚本可直接使用 [[06-存储/01-K8s存储/09-pv-pvc-troubleshooting.md|PV/PVC 故障排查与解决方案]] 中的 `pv-pvc-diagnostics.sh`。
...
- [[06-存储/01-K8s存储/07-storage-daily-operations.md|存储日常运维操作手册]]
- [[06-存储/01-K8s存储/09-pv-pvc-troubleshooting.md|PV/PVC 故障排查与解决方案]]
- [[06-存储/01-K8s存储/13-storage-security-compliance.md|存储安全与合规管理]]
