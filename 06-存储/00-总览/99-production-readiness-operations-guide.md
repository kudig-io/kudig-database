...
本指南聚焦将 `存储` 的存储能力从“可用”推进到“可长期稳定运行”，重点补齐当前知识库在**存储多租户治理、临时存储管理、CSI 供应链安全、容量规划、存储混沌演练**五个方向的缺口。阅读前建议先熟悉 [[K8s存储/01-storage-architecture-overview.md|存储架构概览]] 与 [[K8s存储/07-storage-daily-operations.md|存储日常运维操作手册]]。
