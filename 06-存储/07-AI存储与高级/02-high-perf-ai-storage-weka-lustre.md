...
1. **分层存储架构**：热数据（当前训练集）放 NVMe 并行文件系统，温数据（近期模型）放 [[06-存储/07-AI存储与高级/01-minio-object-storage-ai.md|MinIO 对象存储]]，冷数据归档
2. **数据本地性**：Alluxio Worker 与 GPU 节点共置，利用内存/NVMe 缓存减少网络传输
3. **网络隔离**：存储网络（RDMA/IB）与训练通信网络（NCCL）分离，避免带宽争抢
4. **Checkpoint 策略**：使用异步 Checkpoint（先写本地 NVMe，后台同步到并行文件系统），参考 [[15-AI基础设施/01-基础设施/06-ai-data-pipeline.md|AI 数据管线]]
5. **容量监控**：并行文件系统容量使用超过 80% 时性能显著下降，设置 70% 告警阈值
6. **客户端调优**：根据 I/O 模式调整 readahead、stripe count、stripe size 参数
7. **故障演练**：定期进行存储节点故障注入测试，参考 [[06-存储/07-AI存储与高级/10-storage-chaos-engineering.md|存储混沌工程]]
8. **基准测试**：部署前使用 [[06-存储/07-AI存储与高级/07-storage-benchmarking-methodology.md|存储基准测试方法论]] 验证性能达标
