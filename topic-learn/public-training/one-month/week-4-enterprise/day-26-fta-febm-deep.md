...
related_topics:
  ...
  - topic-learn/public-training/one-month/week-4-enterprise/day-25-production-best-practices
...
FTA（故障树分析，Fault Tree Analysis）和 FEBM（取证循证方法，Forensic Evidence-Based Method）是两种互补的故障诊断方法论。FTA 提供系统化的故障原因分析框架，帮助你从顶层事件出发，逐层分解所有可能的故障路径；FEBM 提供严谨的证据收集和假设验证方法，确保故障排查过程有据可依，避免凭直觉猜测导致的误判。

本课程将深入探讨这两种方法论在生产环境中的实际应用。你将学习如何为 Kubernetes 集群构建完整的故障树，如何使用 FEBM 方法系统化地排查复杂故障，以及如何将 AI Agent 技术与传统的故障诊断方法相结合，提升故障定位的效率和准确性。

**学习目标**：
- 深入学习 FTA 生产落地方法和实践技巧
- 掌握 Kubernetes 完整故障树的构建和分析
- 了解 AI Agent 运维模式与 FTA/FEBM 的结合方式
- 能够运用 FTA + FEBM 方法独立排查复杂生产故障
...
1. **系统性**: 确保所有可能的故障路径都被考虑到，不会遗漏
2. **可视化**: 通过图形化的故障树直观展示故障传播路径
3. **定量化**: 可以为每个基本事件赋予权率，计算顶事件发生概率
4. **预防性**: 可以提前构建故障树，在故障发生时快速定位
...
2. **智能假设生成**: 基于历史故障数据和知识库，自动生成可能的故障原因
...
│  │  └─ Controller 故障: Ingress Controller Pod 异常
│  │
│  └─ [OR] 存储问题
...
选择一个你遇到过的（或想象的）生产故障，按照以下模板完整记录：
...
| 节点故障 | 3-5% | 8760h | 5-30min |
| 控制平面故障 | 1-3% | 43800h | 15-60min |
...
**A**: 两种方法互补使用：
- **FTA 适合**: 故障发生前的预防性分析、构建故障知识库、新人培训
- **FEBM 适合**: 故障发生时的实时排查、需要证据支撑的根因分析
- **最佳实践**: 先用 FTA 框架缩小范围，再用 FEBM 方法验证确认

### Q2: 故障树构建到什么粒度合适？

**A**: 粒度原则：
1. **第一层**: 故障大类别（应用/平台/控制面/基础设施）
2. **第二层**: 具体组件（Pod/Service/Ingress/存储等）
3. **第三层**: 具体故障现象（CrashLoopBackOff/Pending/OOMKilled 等）
...
**A**: 建立故障知识库：
1. 每次故障修复后，更新对应的故障树分支
2. 定期（每月）回顾故障报告，补充新的故障模式
...
- **FTA** 从顶事件出发，通过逻辑门逐层分解故障原因，确保系统性覆盖
- **FEBM** 要求先收集证据，再生成假设，然后逐一验证，避免直觉猜测
- **故障全景树** 是团队共同维护的知识资产，应持续更新
...
- [文件: `../../topic-fta/23-fta-production-quick-start.md`](../../topic-fta/23-fta-production-quick-start.md)
- [文件: `../../topic-fta/kubernetes-fta-full-analysis.md`](../../topic-fta/kubernetes-fta-full-analysis.md)
- [文件: `../../topic-febm/08-febm-production-quick-start.md`](../../topic-febm/08-febm-production-quick-start.md)
- [文件: `../../topic-fta/10-agent-orchestration-patterns.md`](../../topic-fta/10-agent-orchestration-patterns.md)
