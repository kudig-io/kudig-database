...
related_topics:
  - topic-learn/public-training/one-month/week-2-core-technologies/README
  - topic-learn/public-training/one-month/week-3-operations/day-15-security-1
  - topic-learn/public-training/one-month/week-4-enterprise/README
---
...
**FTA（故障树分析，Fault Tree Analysis）** 是一种自顶向下的分析方法。它从一个故障现象（顶事件）出发，通过布尔逻辑（AND/OR）将可能的故障原因层层分解，最终构建一棵故障树。当故障发生时，你沿着故障树从上到下逐层排查，可以避免遗漏关键排查路径。FTA 特别适合复杂系统的故障分析，因为它强制你系统化地思考所有可能的故障路径。

FTA 核心元素:

```
顶事件 (Top Event): 要分析的故障现象
├── 中间事件 (Intermediate): 进一步分解的子故障
│   ├── 基本事件 (Basic): 不可再分的根因
│   └── 基本事件
└── 中间事件
    ├── 基本事件
...
- [认证授权系统](../../domain-7-security/01-authentication-authorization-system.md)
- [RBAC 矩阵配置](../../domain-7-security/07-rbac-matrix-configuration.md)
- [Pod 安全标准](../../domain-7-security/06-pod-security-standards.md)
- [可观测性架构总览](../../domain-8-observability/01-observability-architecture-overview.md)
- [Prometheus 监控](../../domain-8-observability/10-monitoring-metrics-prometheus.md)
- [FTA 故障树分析](../../../topic-fta/04-fta-core-principles.md)
- [FEBM 取证循证方法](../../../topic-febm/01-febm-theory-foundations.md)
- [Pod 综合排障](../../domain-12-troubleshooting/08-pod-comprehensive-troubleshooting.md)
- [Node NotReady 诊断](../../domain-12-troubleshooting/06-node-notready-diagnosis.md)
