...
证书过期是 K8s 集群中最严重的故障之一，影响范围取决于过期的证书类型：
...
推荐使用 `cert-manager` 或自定义脚本定期检查证书过期时间。可以在 Prometheus 中配置告警规则，当证书剩余有效期少于 30 天时触发告警。ACK 托管版的管控面证书由阿里云自动监控和轮换，用户无需关注。但用户管理的 Webhook 证书、自定义证书需要自行监控。
...
- [证书管理详解](../../domain-7-security/10-certificate-management.md)
- [安全架构总览](../../domain-1-architecture-fundamentals/14-security-architecture.md)
- [证书排障指南](../../domain-12-troubleshooting/13-certificate-troubleshooting.md)
- [ACK 集群管理](../../domain-17-cloud-provider/04-alicloud-ack/210-ack-cluster-management.md)
- [kubelet TLS Bootstrap](../../domain-7-security/02-kubelet-tls-bootstrap.md)
