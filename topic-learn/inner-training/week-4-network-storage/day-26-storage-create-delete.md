...
related_topics:
  ...
  - domain-6-storage/01-storage-architecture-overview
id: WEEK4-DAY26
topic: training
type: hands-on
...
云盘的关键限制：只支持 RWO（同一时间只能被一个节点挂载）。这意味着使用云盘的 Pod 不能在多个节点上运行（适合 StatefulSet），且节点故障时需要等待 Volume Detach 后才能在另一个节点上 Attach。
...
- [ACK 存储管理](../../domain-17-cloud-provider/04-alicloud-ack/245-ack-ebs-storage.md)
