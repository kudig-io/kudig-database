...
related_topics:
  ...
  - topic-learn/inner-training/week-4-network-storage/day-27-storage-mount
  - topic-learn/inner-training/week-4-network-storage/day-28-comprehensive-review
id: WEEK4-INDEX
topic: training
type: week-index
...
**Nginx Ingress Controller** 是社区最流行的 Ingress Controller。它在集群内部以 DaemonSet 或 Deployment 方式运行 Nginx 反向代理，通过 Watch Ingress 资源动态生成 Nginx 配置。优势是功能丰富、社区支持好；缺点是流量需要经过 Nginx 中转，增加了一跳延迟。
...
**Flannel** 是社区最简单的 CNI 插件之一。它使用 VxLAN 覆盖网络（Overlay）在节点间建立隧道。Flannel 的优势是配置简单、不依赖特定的网络基础设施；缺点是 VxLAN 封装带来一定的性能开销、不支持 Kubernetes NetworkPolicy。
...
- [网络架构总览](../../domain-5-networking/01-network-architecture-overview.md)
- [CNI 架构基础](../../domain-5-networking/02-cni-architecture-fundamentals.md)
- [Service 概念与类型](../../domain-5-networking/06-service-concepts-types.md)
- [Ingress 基础](../../domain-5-networking/19-ingress-fundamentals.md)
- [Nginx Ingress 完整指南](../../domain-5-networking/21-nginx-ingress-complete-guide.md)
- [存储架构总览](../../domain-6-storage/01-storage-architecture-overview.md)
- [PV 架构基础](../../domain-6-storage/02-pv-architecture-fundamentals.md)
- [StorageClass 动态供给](../../domain-6-storage/04-storageclass-dynamic-provisioning.md)
- [ACK 服务总览](../../domain-17-cloud-provider/04-alicloud-ack/alicloud-ack-overview.md)
