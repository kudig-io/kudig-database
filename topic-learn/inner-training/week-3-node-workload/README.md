...
节点和工作负载是 Kubernetes 最基础也最重要的两个概念。节点是集群的计算资源单元，工作负载（尤其是 Pod）是应用的运行载体。一个优秀的 K8s 运维工程师需要深刻理解这两个层面的工作原理，才能在故障发生时快速定位问题、在架构设计时做出合理决策。
...
**托管节点池** 是 ACK 的特色功能。启用托管模式后，ACK 平台会自动处理节点的运维工作：当节点出现 NotReady 状态超过一定时间后，系统会自动创建新节点并替换故障节点；当有新的安全补丁或 K8s 版本更新时，系统可以自动执行节点轮换。这极大减轻了运维人员的日常工作负担。
...
托管节点池会定期检查节点健康状态。当检测到节点连续 NotReady 超过配置的时间阈值后，系统会自动创建一个新的节点来替换故障节点。在新节点 Ready 后，系统会安全地排空（drain）故障节点上的 Pod，然后释放故障节点的 ECS 实例。
...
- [K8s 架构与组件深入](../../domain-1-architecture-fundamentals/02-core-components-deep-dive.md)
- [ACK 服务总览](../../domain-17-cloud-provider/04-alicloud-ack/alicloud-ack-overview.md)
- [ECS 计算资源](../../domain-17-cloud-provider/04-alicloud-ack/240-ack-ecs-compute.md)
- [Pod 生命周期事件](../../domain-4-workloads/11-pod-lifecycle-events.md)
- [HPA/VPA 自动伸缩](../../domain-4-workloads/21-hpa-vpa-autoscaling.md)
...
- [集群自动伸缩排障](../../domain-12-troubleshooting/28-cluster-autoscaler-troubleshooting.md)
