...
- 中的代理
- dictionary
title_en: Proxies In Kubernetes
---
...
| Service 无法访问（kube-proxy） | kube-proxy 未运行或 iptables 规则异常 | `kubectl -n kube-system get pods -l k8s-app=kube-proxy`；`iptables-save \| grep <svc-name>` |
