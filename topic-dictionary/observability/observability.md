...
Kubernetes 组件以 Prometheus 格式从 `/metrics` 端点暴露指标，包括：
...
常用工具：Prometheus、Thanos、Cortex、Grafana Mimir。
...
- **集群级日志架构**：通常在每个节点上运行日志代理（如 Fluent Bit、Fluentd），将日志转发到集中式日志存储（如 Elasticsearch、Grafana Loki、OpenSearch）。
