...
与 [[09-可观测性/01-总览/01-observability-architecture-overview.md|可观测性架构总览]] 中描述的传统 Agent 采集模式不同，eBPF 方案直接在内核层捕获事件，避免了应用层 SDK 集成的侵入性和维护成本。
...
传统可观测性方案（如 [[09-可观测性/04-链路追踪/04-opentelemetry-distributed-tracing.md|OpenTelemetry SDK]]）需要修改应用代码或注入 Agent，存在以下痛点：多语言 SDK 维护成本高、遗留系统无法接入、SDK 版本升级需要重新部署。eBPF 方案完全绕过这些限制，在内核层统一采集所有语言、所有框架的遥测数据。
...
3. **与现有监控集成**：Hubble 指标通过 ServiceMonitor 接入 [[09-可观测性/02-指标/01-prometheus-enterprise-monitoring.md|Prometheus]]，安全事件通过 Fluent Bit 接入 [[09-可观测性/01-总览/04-elastic-stack-enterprise-observability.md|ELK/Loki]]。
...
- [[09-可观测性/04-链路追踪/04-opentelemetry-distributed-tracing.md|OpenTelemetry 分布式追踪]]
- [[09-可观测性/02-指标/01-prometheus-enterprise-monitoring.md|Prometheus 企业级监控]]
- [[09-可观测性/01-总览/04-elastic-stack-enterprise-observability.md|Elastic Stack 企业可观测性]]
- [[09-可观测性/05-告警/01-alertmanager-deep-configuration.md|Alertmanager 深度配置]]
- [[09-可观测性/01-总览/07-security-compliance-governance.md|安全合规治理]]
- [[09-可观测性/04-链路追踪/06-otel-collector-deep-configuration.md|OTel Collector 深度配置]]
