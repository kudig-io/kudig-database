...
StatefulSet 是 [[23-实体/02-K8s核心组件/kubernetes.md|kubernetes]] 中管理有状态应用的核心工作负载控制器。与 Deployment 不同，StatefulSet 为每个 Pod 提供稳定的网络标识（hostname）、稳定的存储（PVC）和有序的部署/扩展/更新保证。这些特性使其故障模式更为复杂：Pod 启动顺序依赖、PVC 与 Pod 的生命周期绑定、Headless Service 依赖、以及分布式一致性要求（如数据库集群的脑裂问题）。
