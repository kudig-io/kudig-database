...
- target: '[[26-技能/03-节点/node/skill-notready/skill-k8s-node-notready-SKILL.md]]'
  type: uses
- target: '[[23-实体/02-K8s核心组件/coredns.md]]'
  type: uses
...
**工程师**：「Pod 里域名解析失败了」/「nslookup 不通」/「[[23-实体/02-K8s核心组件/coredns.md|CoreDNS]] 好像挂了」
...
>
> 1. 确认 CoreDNS [[17-系统基础/06-知识字典/networking/service.md|Service]] ClusterIP：`kubectl get svc kube-dns -n kube-system`
...
>
> 1. 确认当前配置：`kubectl get [[23-实体/02-K8s核心组件/deployment.md|deployment]] <deploy-name> -n <namespace> -o yaml | grep -A 3 dnsPolicy`
...
**顾问指令**：
> resolv.conf 缺失或为空非常异常，可能由 [[23-实体/02-K8s核心组件/kubelet.md|kubelet]] 或容器运行时 Bug 引起。
>
...
**分支决策**：
- **I1**：CNI Pod 异常 → 升级决策点（网络深度诊断 [[26-技能/03-节点/node/skill-notready/skill-k8s-node-notready-SKILL.md|SKILL]]-NET-003）
