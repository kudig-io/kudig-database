...
- target: '[[23-实体/02-K8s核心组件/kubelet.md]]'
  type: uses
- target: '[[26-技能/07-安全/certificate/kubelet-certificate-rotation.md]]'
  type: uses
---
...
### 入口 D：工程师提到 [[23-实体/02-K8s核心组件/kubelet.md|kubelet]] 报错
...
**顾问**：当前信息有限。请尽可能收集：
> 1. **监控截图**：是否有 apiserver、[[23-实体/02-K8s核心组件/etcd.md|etcd]]、kubelet 告警？
> 2. **告警内容**：告警通知的完整内容
...
>
> 这可能需要引导至 [[26-技能/03-节点/node/skill-notready/skill-k8s-node-notready-SKILL.md|SKILL]]-NODE-001（节点问题诊断）。
...
**顾问**：确认是 kubelet 证书问题。请先确认集群的 [[26-技能/07-安全/certificate/kubelet-certificate-rotation.md|kubelet 证书轮换机制]]（自动/手动）。
> **自动轮换场景**，请执行：
> ```bash
> kubectl get csr
> ```
