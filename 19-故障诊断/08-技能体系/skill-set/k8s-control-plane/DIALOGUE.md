...
### 分支 3：etcd异常
- `kubectl exec -it [[23-实体/02-K8s核心组件/etcd.md|etcd]]-<node> -n kube-system -- etcdctl endpoint health`
...
- `journalctl -u kube-apiserver -n 100 --no-pager`
  - 如无法执行：请提供当前可执行的环境信息
- `检查证书有效期: openssl x509 -in /[[23-实体/02-K8s核心组件/kubernetes.md|kubernetes]]/pki/apiserver.crt -noout -dates`
  - 如无法执行：请提供当前可执行的环境信息
...
| 原命令 | 替代方案 A | 替代方案 B | 替代方案 C |
|:---|:---|:---|:---|
| `etcdctl endpoint health` | 通过 apiserver 间接检查：`kubectl get --raw /healthz/etcd` | 查看 etcd Pod 日志 | 检查 etcd [[17-系统基础/06-知识字典/networking/service.md|Service]] Endpoint |
...
| 原命令 | 替代方案 A | 替代方案 B | 替代方案 C |
|:---|:---|:---|:---|
| 任何 kubectl 命令 | 提供截图和描述 | 审查现有配置文件（[[23-实体/08-交付与制品/helm.md|Helm]]） | 转为文档指导和流程确认 |
