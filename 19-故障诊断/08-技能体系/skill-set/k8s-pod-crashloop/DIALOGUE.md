...
```
> **如果无法执行**：请手动统计一下：有多少个 Pod 受影响？它们是否属于同一个 [[23-实体/02-K8s核心组件/deployment.md|Deployment]]/StatefulSet/DaemonSet？是否分布在不同的节点上？
...
```
> **如果无法执行**：请确认你们是否使用了 [[23-实体/04-网络/cilium.md|Cilium]] 等网络策略。如果使用了，请检查是否有规则阻止了 Pod 访问依赖服务。如果无法确认，可以暂时将网络策略全部删除测试（仅限测试环境）。
...
> 2. **升级至存储专家** → 如果涉及数据库/消息队列等有状态服务，我将转接至 [[26-技能/03-节点/node/skill-notready/skill-k8s-node-notready-SKILL.md|SKILL]]-STORE-001
