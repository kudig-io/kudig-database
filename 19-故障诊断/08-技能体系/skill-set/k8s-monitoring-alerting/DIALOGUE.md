...
- target: '[[23-实体/02-K8s核心组件/kubernetes.md]]'
  type: uses
- target: '[[17-系统基础/06-知识字典/fundamentals/namespaces.md]]'
  type: uses
---
...
> ```
> **如果无法执行**（不知道 monitoring namespace）→ `kubectl get pods --all-[[17-系统基础/06-知识字典/fundamentals/namespaces.md|namespaces]] | grep -i alertmanager`
...
>    **如果无 prometheus CRD** → `kubectl get [[23-实体/02-K8s核心组件/deployment.md|deployment]] -n monitoring | grep prometheus`
...
**修复方案**：
> 修正规则中的 PromQL：`kubectl patch prometheusrules <rule-name> -n monitoring --type='json' -p='[{"op": "replace", "path": "/spec/groups/0/rules/0/expr", "value": "up{job=\"[[23-实体/02-K8s核心组件/kubernetes.md|kubernetes]]-pods\"} == 0"}]'`
