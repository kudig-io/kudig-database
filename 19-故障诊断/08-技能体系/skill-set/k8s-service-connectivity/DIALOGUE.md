...
**工程师**：「NodePort 不通」/「LoadBalancer 外部 IP 无法访问」/「[[17-系统基础/06-知识字典/networking/ingress.md|Ingress]] 后端超时」
...
**顾问指令**：
> 多个 Service 同时问题通常指向集群级组件异常（kube-proxy、[[23-实体/02-K8s核心组件/coredns.md|CoreDNS]]、CNI）。先确认范围。
...
- **B2**：CoreDNS Pod 异常 → 升级至 [[26-技能/03-节点/node/skill-notready/skill-k8s-node-notready-SKILL.md|SKILL]]-NET-001（DNS 问题诊断）
...
> **如果使用 [[23-实体/04-网络/cilium.md|Cilium]]** → `kubectl get ciliumnetworkpolicy -n <namespace>`
...
> 3. 如需修改，在 [[23-实体/02-K8s核心组件/deployment.md|Deployment]] 中增加 dnsConfig：
> ```yaml
