...
>    **如果无 ds** → `kubectl get [[23-实体/02-K8s核心组件/deployment.md|deployment]] -n logging | grep -E "fluent|filebeat"`
...
>    **如果无法 debug node** → `kubectl run node-test --image=busybox --rm -it --restart=Never --overrides='{"spec":{"nodeSelector":{"[[23-实体/02-K8s核心组件/kubernetes.md|kubernetes]].io/hostname":"<node-name>"},"hostNetwork":true}}' -- ls -la /var/log/containers/ | head -10`
...
> 2. 检查 [[23-实体/02-K8s核心组件/kubelet.md|kubelet]] 日志配置：`kubectl run node-debug ... -- cat /var/lib/kubelet/config.yaml | grep -A 5 containerLogMaxSize`
...
| 安全策略限制 | **[[26-技能/03-节点/node/skill-notready/skill-k8s-node-notready-SKILL.md|SKILL]]-SEC-003** | RBAC/PSP/OPA 相关 |
...
- [[23-实体/04-网络/cilium.md|Cilium (entities)]]
