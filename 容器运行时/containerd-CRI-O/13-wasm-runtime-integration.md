...
1. **镜像仓库策略**：WASM 镜像极小（通常 < 5MB），建议使用 OCI Registry 统一存储，配合 [[容器运行时/containerd-CRI-O/10-snapshotter-strategies|snapshotter 策略]] 优化拉取
...
6. **版本管理**：WASM 运行时迭代快，建议使用 DaemonSet 管理节点上的 shim 二进制版本，配合 [[生产运维/升级策略|滚动升级策略]] 逐节点更新
