...
5. **与 [[容器运行时/containerd-CRI-O/02-cri-o-production-guide|CRI-O]] 配合**：K8s 节点使用 CRI-O，开发环境使用 Podman，共享 OCI 镜像格式
6. **CI/CD 中使用 Buildah**：无需特权容器即可构建镜像（`--isolation chroot`），适合 [[平台工程/构建/01-platform-engineering-overview|平台工程]] 流水线
...
- [[容器运行时/containerd-CRI-O/02-cri-o-production-guide|CRI-O 生产指南]]
- [[容器运行时/containerd-CRI-O/06-rootless-containers-guide|无根容器指南]]
