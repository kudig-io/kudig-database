				...
				struct sxe2_drv_cmd_params *cmd_buf)
{
	...
	if (is_add)
		ret = sxe2vf_ucmd_unicast_mac_add(adapter, vsi_id, addr);
	else
		ret = sxe2vf_ucmd_unicast_mac_del(adapter, vsi_id, addr);
				...
				struct sxe2_drv_cmd_params *cmd_buf)
{
	...
	if (is_add)
		ret = sxe2vf_ucmd_multi_broad_mac_add(adapter, vsi_id, addr);
	else
		ret = sxe2vf_ucmd_multi_broad_mac_del(adapter, vsi_id, addr);
					  ...
					  struct sxe2_drv_cmd_params *cmd_buf)
{
	...
	ret = sxe2vf_ucmd_vlan_filter_cfg(adapter, vsi_id, is_oper_enable);
	if (ret)
				       ...
				       struct sxe2_drv_cmd_params *cmd_buf)
{
	...
	ret = sxe2vf_ucmd_vlan_rule_process(adapter, vsi_id, &vlan, is_add);
	if (ret) {
		LOG_ERROR_BDF("user driver(vsi=%u) %s vlan tpid:%u vid:%u prio:%u fail, ret=%d\n",
			      vsi_id, is_add ? "add" : "del", vlan.tpid, vlan.vid,
			      vlan.prio, ret);
	} else {
				     ...
				     struct sxe2_drv_cmd_params *cmd_buf)
{
	...
	if (is_add)
		ret = sxe2vf_ucmd_promisc_rule_add(adapter, vsi_id);
	else
		ret = sxe2vf_ucmd_promisc_rule_del(adapter, vsi_id);
				      ...
				      struct sxe2_drv_cmd_params *cmd_buf)
{
	...
	is_add = promisc_filter_cfg_req.is_add;
	if (is_add)
		ret = sxe2vf_ucmd_allmulti_rule_add(adapter, vsi_id);
	else
		ret = sxe2vf_ucmd_allmulti_rule_del(adapter, vsi_id);
