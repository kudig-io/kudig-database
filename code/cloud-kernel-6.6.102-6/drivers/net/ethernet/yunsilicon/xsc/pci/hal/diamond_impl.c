...
static void diamond_ring_rx_doorbell(void *hal, void __iomem *bar, u32 rqn, u32 next_pid)
{
	struct xsc_hw_abstract_layer *_hal = hal;
...
static void diamond_update_cq_db(void *hal, void __iomem *bar, u32 cqn, u32 next_cid, u8 solicited)
{
	struct xsc_hw_abstract_layer *_hal = hal;
	union diamond_cq_doorbell db;
...
static void diamond_set_cq_ci(void *hal, void __iomem *bar, u32 cqn, u32 next_cid)
{
	struct xsc_hw_abstract_layer *_hal = hal;
	union diamond_cq_doorbell db;
...
static void diamond_set_eq_ci(void *hal, void __iomem *bar, u32 eqn, u32 next_cid, u8 arm)
{
	struct xsc_hw_abstract_layer *_hal = hal;
...
static u8 diamond_get_mr_page_mode(u8 page_shift)
{
	return page_shift;
}

static inline u32 diamond_mkey_to_idx(u32 mkey)
{
	return mkey >> 8;
}

static inline u32 diamond_idx_to_mkey(u32 mkey_idx)
{
	return mkey_idx << 8;
}

static void diamond_set_mpt_tbl(void *hal, void __iomem *bar, int iae_idx,
				u32 mtt_base, void *mr_request)
{
	...
	u32 idx = diamond_mkey_to_idx(mkey);
...
static void diamond_clear_mpt_tbl(void *hal, void __iomem *bar, int iae_idx,
				  u32 mtt_base, void *mr_request)
{
	struct xsc_hw_abstract_layer *_hal = hal;
...
static void diamond_set_mtt_tbl(void *hal, void __iomem *bar, int iae_idx,
				u32 mtt_base, void *mr_request)
{
	struct xsc_hw_abstract_layer *_hal = hal;
	struct xsc_register_mr_request *req = mr_request;
...
static void diamond_set_read_done_msix_vector(void *hal, void __iomem *bar, u32 vector)
{
}

static int diamond_dma_write_tbl_once(void *hal, void __iomem *bar,
				      u32 data_len, u64 dma_wr_addr,
				      u32 host_id, u32 func_id,
				      u64 success[2], u32 size)
{
	return -1;
}

static void diamond_dma_read_tbl(void *hal, void __iomem *bar,
...
{
	struct diamond_cqe *_cqe = cqe;

	return !!_cqe->error_code;
}

static u8 diamond_get_cqe_error_code(void *cqe)
{
	struct diamond_cqe *_cqe = cqe;
...
static u8 diamond_get_cqe_opcode(void *cqe)
{
	struct diamond_cqe *_cqe = cqe;
...
struct diamond_data_seg {
	u32	length;
	u32	key;
	u64	addr;
};

static void diamond_set_data_seg(void *data_seg, u32 length, u32 key, u64 addr)
{
	struct diamond_data_seg *seg = data_seg;
...
static struct xsc_hw_ops diamond_arch_ops = {
	...
	.ring_tx_doorbell = diamond_ring_tx_doorbell,
	.ring_rx_doorbell = diamond_ring_rx_doorbell,
	.update_cq_db = diamond_update_cq_db,
	.set_cq_ci = diamond_set_cq_ci,
	.set_eq_ci = diamond_set_eq_ci,
	.get_mr_page_mode = diamond_get_mr_page_mode,
	.mkey_to_idx = diamond_mkey_to_idx,
	.idx_to_mkey = diamond_idx_to_mkey,
	.set_mpt = diamond_set_mpt_tbl,
	.clear_mpt = diamond_clear_mpt_tbl,
	.set_mtt = diamond_set_mtt_tbl,
	.set_read_done_msix_vector = diamond_set_read_done_msix_vector,
	.dma_write_tbl_once = diamond_dma_write_tbl_once,
	.dma_read_tbl = diamond_dma_read_tbl,
...
struct xsc_hw_abstract_layer diamond_pf_hal = {
	.ops = &diamond_arch_ops,
	.regs = &diamond_pf_regs,
};

struct xsc_hw_abstract_layer diamond_vf_hal = {
	.ops = &diamond_arch_ops,
	.regs = &diamond_vf_regs,
};

struct xsc_hw_abstract_layer *get_diamond_pf_hal(void)
{
	return &diamond_pf_hal;
}

struct xsc_hw_abstract_layer *get_diamond_vf_hal(void)
{
	return &diamond_vf_hal;
}
