...
在一个完整的故障周期中，系统经历「正常运行 → 故障 → 恢复 → 正常运行」的循环。
...
串联系统中，所有组件必须同时正常运行，系统才能提供服务。任何一个组件故障，整个系统就故障。
...
并联系统中，只要至少一个组件正常运行，系统就能提供服务。常用于消除单点故障。
...
N+1 冗余是最常见的生产配置：N 个组件承载正常负载，1 个备用组件在故障时接管。

```
假设: N=3 个活跃节点 + 1 个备用节点，单节点可用性 99.9%

系统可用条件: 至少 N 个节点可用 (即最多 1 个节点故障)
...
```
故障频率 λ = 1 / MTBF  (单位: 次/小时)

对于指数分布（无记忆性假设）:
P(在时间 t 内无故障) = e^(-λt) = e^(-t/MTBF)
```
...
A_dr = P(Region A 正常)
     + P(Region A 故障) × P(Region B 正常) × P(切换成功)
...
对于 5 个 9 目标:
  ...
  如果每年故障 1 次，RTO 必须 < 5 分钟
  如果每年故障 2 次，RTO 必须 < 2.5 分钟
```
