...
Kubernetes（简称 [[23-实体/02-K8s核心组件/kubernetes.md|k8s]]）是一个开源的容器编排平台，最初由 Google 设计并捐赠给 Cloud Native Computing Foundation（CNCF）。它的核心定位是**分布式系统的操作系统**（Cloud OS）——正如操作系统管理 CPU、内存、磁盘等硬件资源，Kubernetes 管理的是节点、网络、存储等分布式资源。
