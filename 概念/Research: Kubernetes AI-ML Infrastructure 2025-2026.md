...
相关核心概念详见 [[概念/k8s-ai-ml-infrastructure.md|k8s ai ml infrastructure]]，涵盖：
...
- [[概念/container-runtime-evolution.md|container runtime evolution]] — 容器运行时演进（Wasm、gVisor）为 AI/ML 推理工作负载提供更高效的隔离与执行环境
- [[概念/k8s-networking-evolution.md|k8s networking evolution]] — 高性能网络（RDMA、GPUDirect RDMA）是分布式 AI 训练集群的关键基础设施
- [[概念/storage-performance-optimization.md|storage performance optimization]] — 存储性能优化（并行文件系统、本地 NVMe 缓存）直接影响模型训练 I/O 吞吐
- [[概念/finops-greenops-practices.md|finops greenops practices]] — GPU 资源成本管理与绿色计算是 AI/ML 基础设施可持续运营的核心议题
