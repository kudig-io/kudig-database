---
sources:
- "平台工程/运维/14-cluster-api-deep-dive.md"
...
典型叠加：CAPI 供给一个"空"集群 → Karpenter 接管该集群内的节点弹性扩缩。即 **CAPI 决定"集群存在"，Karpenter 决定"集群内节点数"**。详见 [[平台工程/99-karpenter-node-autoscaling-guide|Karpenter 节点弹性]]。
...
- [[平台工程/运维/13-multi-cluster-management|多集群管理]] — 多集群架构模式与工具对比，CAPI 与 Karmada/Rancher 的定位
- [[平台工程/运维/25-virtual-clusters|虚拟集群 vcluster]] — Namespace 内的轻量控制平面，与 CAPI 真实集群的对比与叠加
- [[平台工程/99-karpenter-node-autoscaling-guide|Karpenter 节点弹性]] — 节点级弹性供给，与 CAPI 集群级供给的边界划分
- [[平台工程/运维/02-cluster-lifecycle-management|集群生命周期管理]] — 集群从创建到退役的通用流程

### GitOps 与应用分发

- [[发布变更/GitOps/01-argo-cd-enterprise-gitops|Argo CD GitOps]] — 用 ApplicationSet 同步 CAPI Cluster CR 的实践
- [[发布变更/GitOps/08-fleet-gitops-operations-guide|Fleet GitOps 运维]] — Rancher Fleet 的多集群应用分发

### 知识字典

- [[系统基础/知识字典/platform-engineering/cluster-api-and-fleet-management|Cluster API 与 Fleet]] — CAPI 与集群舰队管理的概述性入门
