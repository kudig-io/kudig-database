...
description: '[[概念/controller-pattern.md|controller pattern]] 描述协调循环的通用框架，[[实体/deployment.md|deployment]]
...
summary: '[[概念/controller-pattern.md|controller pattern]] 描述协调循环的通用框架，[[实体/deployment.md|deployment]]
...
[[概念/controller-pattern.md|controller pattern]] 描述协调循环的通用框架，[[实体/deployment.md|deployment]] 是最常用的工作负载控制器。wiki 将前者视为抽象模式、后者视为具体资源，但 Deployment 不仅是控制器模式的一个实例——它是控制器模式的**最小完备实现**。Deployment 的每一特性（声明式 replica 管理、滚动更新策略、版本回滚、进度追踪）都直接映射到控制器模式的四个阶段（Observe→Compare→Act→Update）。
...
| **状态爆炸** | 每个 Deployment 保留 10 个（默认）历史 ReplicaSet 用于回滚。大规模集群中，这些历史版本显著增加 [[实体/etcd.md|etcd]] 存储压力 |
| **控制器递归** | Deployment 管理 ReplicaSet，ReplicaSet 管理 Pod——两级控制器增加了调试复杂度。`kubectl describe deployment` 需要穿透两层才能定位问题 |

## 开放问题

- **Deployment 是否过度通用？** 金丝雀发布、蓝绿部署等高级场景需要 [[实体/argo.md|Argo]] Rollouts 等专门控制器，因为 Deployment 的滚动更新策略过于简单。这是否意味着 Deployment 的设计过于保守？
...
- [[实体/prometheus.md|Prometheus]]-Grafana.md|可观测性支柱 × Prometheus-Grafana]]
