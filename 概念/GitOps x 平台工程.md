...
GitOps 和平台工程在 wiki 中被当作两个独立学科来处理——[[概念/gitops-principles.md|gitops-principles]] 覆盖 ArgoCD/Flux 和四个 OpenGitOps 原则，[[概念/platform-engineering-idp.md|platform-engineering-idp]] 覆盖 Backstage、黄金路径和开发者体验。但它们是**相互增强**的关系：GitOps 提供了技术基础（声明式状态、自动化协调、漂移检测），使得平台工程的自助服务承诺变得可靠；平台工程则提供了面向开发者的界面，使得 GitOps 的采用是自然的而非强加的。
