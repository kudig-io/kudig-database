...
结论：**Metrics Server 服务于 HPA/VPA 的实时决策，Prometheus + cAdvisor 服务于监控与告警**。两者互补，不可互相替代。详见 [[可观测性/指标/19-kube-state-metrics-deep-dive.md|kube-state-metrics 深度解析]] 了解 KSM 如何补充对象状态维度。
...
- [[可观测性/指标/01-prometheus-enterprise-monitoring.md|Prometheus 企业级监控]] — Prometheus 部署、联邦与长期存储
- [[可观测性/指标/02-monitoring-metrics-system.md|监控指标体系]] — 指标体系总览与本篇的位置
- [[可观测性/指标/19-kube-state-metrics-deep-dive.md|kube-state-metrics 深度解析]] — 对象状态指标，与 cAdvisor 互补
- [[集群基础/控制平面/15-kubelet-deep-dive.md|kubelet 深度剖析]] — kubelet 架构、PLEG、CRI 交互细节
- [[可观测性/SLO-SLI/04-sli-definition-selection.md|SLI 定义与选择方法论]] — kubelet 运行时错误率 SLI 的上游定义
- [[可观测性/指标/10-monitoring-metrics-prometheus.md|监控和指标表]] — 各组件关键指标速查表
