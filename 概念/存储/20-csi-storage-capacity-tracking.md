...
短期内（1.28–1.33）CSIStorageCapacity 仍是 CSI 存储容量感知的事实标准；DRA 的 CSI 集成在 1.32+ 才逐步成熟。详见 [[集群基础/控制平面/22-container-storage-deep-dive.md|容器存储深度剖析]]。
...
- [[集群基础/控制平面/22-container-storage-deep-dive.md|容器存储深度剖析]] — CSI 三段式 attach/mount 全链路，理解 capacity 在其中的位置
- [[存储/K8s存储/04-storageclass-dynamic-provisioning.md|StorageClass 动态制备]] — StorageClass 字段与 volumeBindingMode 详解
- [[存储/AI存储与高级/05-csi-topology-awareness.md|CSI 拓扑感知]] — topologyKey、AllowedTopologies、跨 AZ 调度排障
- [[存储/K8s存储/05-csi-drivers-integration.md|CSI 驱动集成与运维管理]] — 主流 CSI 驱动部署与 sidecar 配置矩阵
