...
相关核心概念详见 [[概念/k8s-security-compliance.md|k8s security compliance]]，涵盖：

- [[概念/supply-chain-security.md|supply chain security]] — Sigstore、SLSA与镜像签名验证
- eBPF 运行时安全 — eBPF运行时监控与Tetragon
- CEL 原生策略 — CEL策略引擎与Admission Policy GA
- 零信任 mTLS — mTLS、SPIFFE与零信任微分段
- [[概念/secrets-management.md|secrets management]] — External Secrets Operator与CSI Driver
...
- [[概念/k8s-networking-evolution.md|k8s networking evolution]] — 网络安全（Cilium NetworkPolicy、eBPF 可观测）是零信任架构的网络层核心
- [[概念/container-runtime-evolution.md|container runtime evolution]] — 安全容器运行时（gVisor、Kata Containers、Confidential Containers）提供工作负载级隔离
- [[概念/gitops-production-operations.md|gitops production operations]] — GitOps 审计追踪与声明式配置管理是安全合规（SOC2、PCI-DSS）的技术支撑
- [[概念/k8s-observability-stack.md|k8s observability stack]] — 安全可观测性（审计日志、运行时威胁检测）是安全运营中心（SOC）的数据基础
