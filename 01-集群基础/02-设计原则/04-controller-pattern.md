...
intent_queries:
- 控制器模式与调谐循环 (Controller Pattern) 是什么
- 如何 控制器模式与调谐循环 (Controller Pattern)
...
- path: 02-declarative-api-pattern.md
  type: depth
  desc: 声明式 API
- path: ../集群基础/12-operator-development-guide.md
  type: depth
  desc: Operator 开发指南
cross_refs:
- type: domain
...
Kubernetes 控制器采用的是**水平触发 (Level-triggered)** 逻辑，而非边缘触发 (Edge-triggered)。

### 为什么选择水平触发？
* **鲁棒性**: 即使控制器重启、网络中断或事件丢失，下一次调谐循环依然会全量比较 `Spec` 和 `Status`，确保系统最终一致。
* **自愈**: 系统不依赖于“发生了什么”，而只关注“现状是什么”。

> **专家建议**: 在实现 `Reconcile` 函数时，务必保持其**幂等性**。不要假设上一次操作成功，每一次循环都应该是一个完整且独立的检查过程。
...
- [[10-平台工程/06-代码分析/kubernetes-core/06-declarative-api-informer-mechanism.md|声明式 API 与 Informer 机制源码剖析（kubernetes-core）]]
- [[10-平台工程/06-代码分析/kubernetes-core/03-kube-controller-manager-deep-dive.md|KCM 源码深度剖析（控制器模式的官方实现参照）]]
