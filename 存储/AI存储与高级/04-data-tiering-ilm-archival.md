...
3. **AI 数据版本化**：训练数据集使用语义化版本（v1.0.0），模型 artifact 关联训练实验 ID，参考 [[AI基础设施/基础设施/06-ai-data-pipeline.md|AI 数据管线]]
4. **成本可视化**：定期生成各 Namespace/团队的存储成本报告，参考 [[概念/finops-greenops-practices.md|FinOps 实践]]
...
8. **监控覆盖**：为每层存储设置容量水位告警（70%/85%/95%），参考 [[存储/K8s存储/12-storage-monitoring-alerting.md|存储监控告警]]
9. **灾备考虑**：分层策略需与 [[可靠性/灾难恢复/01-multi-region-dr-architecture.md|灾备架构]] 协调，确保各层数据均有备份
