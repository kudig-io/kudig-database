...
2. **定期恢复演练**：每月至少执行一次恢复演练，验证备份有效性，参考 [[可靠性/灾难恢复/17-disaster-recovery-drills.md|灾备演练]]
3. **CSI 快照优先**：块存储 PV 使用 CSI 快照备份（速度快），NFS/共享存储使用 Kopia 文件级备份
4. **TTL 管理**：按数据重要性设置不同 TTL，避免对象存储成本失控
5. **监控告警**：监控备份成功率、持续时间、BSL 容量，参考 [[存储/K8s存储/12-storage-monitoring-alerting.md|存储监控告警]]
6. **AI Checkpoint 保护**：为训练 Checkpoint PVC 添加 `backup=ai-platform` 标签，纳入高频备份策略
7. **跨集群 DR**：配置 `snapshotMoveData=true` 实现跨集群/跨云恢复能力，参考 [[可靠性/灾难恢复/01-multi-region-dr-architecture.md|多区域灾备架构]]
8. **版本升级**：Velero 升级前备份 BSL 配置，遵循 N-1 兼容原则
9. **RBAC 最小化**：Velero ServiceAccount 仅授予备份所需的最小权限，参考 [[存储/AI存储与高级/08-storage-multitenant-isolation.md|存储多租户隔离]]

## Related

- [[可靠性/备份恢复/03-pv-backup-snapshot.md|PV 备份与快照]]
- [[可靠性/灾难恢复/99-velero-backup-recovery-guide.md|Velero 备份恢复指南]]
- [[存储/分布式存储/01-velero-backup-recovery.md|Velero 备份恢复]]
- [[存储/K8s存储/10-storage-backup-disaster-recovery.md|存储备份与灾备]]
- [[可靠性/灾难恢复/01-multi-region-dr-architecture.md|多区域灾备架构]]
