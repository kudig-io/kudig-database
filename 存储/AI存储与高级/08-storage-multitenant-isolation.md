...
2. **最小权限**：通过 RBAC 限制团队只能使用授权的 StorageClass，参考 [[概念/rbac-authorization.md|RBAC 授权]]
3. **存储网络分离**：存储流量（iSCSI/NFS/MinIO）使用独立 VLAN 或 NetworkPolicy 隔离
4. **加密隔离**：敏感数据使用 per-tenant KMS Key 加密，参考 [[存储/K8s存储/18-storage-encryption-at-rest.md|存储加密]]
5. **定期审计**：每月生成各团队存储使用报告，识别异常增长和闲置资源
6. **标签规范**：所有 PVC 必须携带 `team`、`project`、`data-classification` 标签
7. **自动清理**：配置 CronJob 清理超过保留期的 PVC 和快照，参考 [[存储/AI存储与高级/04-data-tiering-ilm-archival.md|数据分层与生命周期管理]]
8. **AI 数据隔离**：训练数据按项目/团队隔离桶或目录，MinIO IAM Policy 限制跨团队访问，参考 [[存储/AI存储与高级/01-minio-object-storage-ai.md|MinIO 对象存储]]
9. **合规报告**：为监管要求生成数据驻留和访问审计报告，参考 [[概念/multi-tenancy-isolation.md|多租户隔离概念]]

## Related

- [[概念/multi-tenancy-isolation.md|多租户隔离概念]]
- [[概念/rbac-authorization.md|RBAC 授权]]
- [[存储/K8s存储/18-storage-encryption-at-rest.md|存储加密]]
- [[存储/AI存储与高级/04-data-tiering-ilm-archival.md|数据分层与生命周期管理]]
- [[存储/K8s存储/13-storage-security-compliance.md|存储安全合规]]
