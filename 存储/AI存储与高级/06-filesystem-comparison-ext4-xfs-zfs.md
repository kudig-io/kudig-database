...
1. **XFS 为首选**：K8s 节点默认选择 XFS（`ftype=1`），大文件性能和并发能力优于 ext4，参考 [[存储/存储基础/01-storage-technologies-overview.md|存储技术概览]]
...
6. **镜像层数控制**：容器镜像层数不超过 20 层，过多层显著影响 OverlayFS 性能，参考 [[概念/overlayfs-storage.md|OverlayFS 存储]]
7. **inode 监控**：将 inode 使用率纳入节点监控告警（阈值 80%），参考 [[存储/K8s存储/12-storage-monitoring-alerting.md|存储监控告警]]
8. **NVMe 优化**：NVMe 磁盘使用 `none` I/O 调度器，增大 `nr_requests` 和 `read_ahead_kb`
9. **Checkpoint 写入**：大模型 Checkpoint 写入使用 `O_DIRECT` 或 `sync_file_range` 避免 page cache 压力，参考 [[AI基础设施/基础设施/06-ai-data-pipeline.md|AI 数据管线]]

## Related

- [[存储/存储基础/01-storage-technologies-overview.md|存储技术概览]]
- [[概念/overlayfs-storage.md|OverlayFS 存储]]
