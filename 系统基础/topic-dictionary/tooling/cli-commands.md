...
| 属性 | 说明 |
|------|------|
| **文件角色** | CLI 命令速查手册 — 运维人员的日常工具箱 |
| **适合读者** | 初学者（学基础命令）→ 中级（调试排错）→ 专家（高级诊断与自动化） |
| **前置知识** | 基本 Linux 命令行操作、../fundamentals/kubernetes-concepts-reference.md 中的核心概念 |
| **关联文件** | 01(运维最佳实践)、02(故障分析)、08(故障排查Playbook)、07(工具生态) |
...
| 等级 | 标记 | 说明 | 示例 |
|------|------|------|------|
| 🟢 安全 | 只读操作 | 不修改任何资源，可放心执行 | `kubectl get`, `kubectl describe` |
| 🟡 注意 | 修改操作 | 会修改资源配置，需确认目标正确 | `kubectl apply`, `kubectl scale` |
| 🔴 危险 | 删除/强制操作 | 可能导致服务中断或数据丢失 | `kubectl delete`, `kubectl drain` |
| ⚫ 高危 | 集群级操作 | 影响整个集群，需要审批流程 | `etcdctl snapshot restore`, 强制删除PV |
...
> **🔰 初学者导读**: kubectl 是与 Kubernetes 集群交互的核心命令行工具，就像操作系统的"终端"一样。所有对集群的操作（查看、创建、修改、删除）都通过 kubectl 完成。初学者只需掌握 `get`、`describe`、`logs`、`exec` 四个子命令，就能应对 80% 的日常场景。
>
> **📋 新手速查 — 最常用的 5 个命令**:
> | 场景 | 命令 | 危险等级 |
> |------|------|----------|
...
> | 应用配置变更 | `kubectl apply -f <文件>.yaml` | 🟡 注意 |
...
> **🔰 初学者导读**: 集群管理命令用于查看和维护集群整体健康状态，就像"体检报告"一样。日常运维中最常用的是检查节点状态和集群信息。注意：节点维护操作（cordon/drain）会影响上面运行的Pod，需要谨慎执行。
>
> **📋 新手速查**:
> | 场景 | 命令 | 危险等级 |
> |------|------|----------|
> | 查看集群信息 | `kubectl cluster-info` | 🟢 安全 |
> | 查看节点状态 | `kubectl get nodes -o wide` | 🟢 安全 |
> | 查看节点详情 | `kubectl describe node <节点名>` | 🟢 安全 |
> | 标记节点不可调度 | `kubectl cordon <节点名>` | 🔴 危险 |
> | 驱逐节点上的Pod | `kubectl drain <节点名>` | 🔴 危险 |
...
> **🔰 初学者导读**: 当 Pod 出问题时，调试命令就是你的"听诊器"。最常用的排查流程是：先 `describe` 看事件 → 再 `logs` 看日志 → 最后 `exec` 进入容器检查。掌握这三步就能解决大多数 Pod 问题。
>
> **📋 故障排查三步法**:
> ```bash
> # 第1步: 看 Pod 状态和事件（找到问题线索）
> kubectl describe pod <问题pod名>
> # 第2步: 看容器日志（找到错误信息）
> kubectl logs <pod名> --previous  # --previous 查看上次崩溃的日志
> # 第3步: 进入容器检查（确认环境和配置）
...
> | 性能问题 | `kubectl top pod/node` | 查看资源使用率 |
...
> **🔰 初学者导读**: 安全命令用于管理"谁能做什么"——包括身份认证（证书/Token）和授权（RBAC权限）。就像管理办公室的门禁系统一样。初学者最常用的是 `kubectl auth can-i` 命令来检查权限，以及查看 ServiceAccount 和 Secret。
>
> **📋 权限检查速查**:
> ```bash
> # 检查当前用户能否执行某操作
> kubectl auth can-i create deployments
> # 检查某 ServiceAccount 的权限
...
> **🔰 说明**: 以下是日常运维中最常见的操作场景，将多个命令组合成完整工作流。
...
# 2. 检查部署状态（等待所有Pod就绪）
kubectl rollout status deployment/<名称> --timeout=300s

# 3. 验证 Pod 运行正常
kubectl get pods -l app=<名称> -o wide

# 4. 检查 Service 端点
kubectl get endpoints <service名称>

# 5. 测试服务连通性
kubectl run test-curl --rm -it --image=curlimages/curl -- curl http://<service名称>:<端口>/health
```
### 场景2: Pod 故障排查完整流程
...
# 3. 查看当前日志
kubectl logs <pod名> -n <命名空间> --tail=100

# 4. 查看上次崩溃日志（如果 Pod 在重启）
...
# 6. 检查容器内 DNS 解析
kubectl exec <pod名> -- nslookup kubernetes.default

# 7. 检查容器内网络连通
kubectl exec <pod名> -- curl -s http://<目标service>:<端口>/health
```
### 场景3: 节点维护（安全驱逐）
...
# 2. 标记节点不可调度（阻止新 Pod 调度）
kubectl cordon <节点名>

# 3. 安全驱逐所有 Pod（等待优雅终止）
kubectl drain <节点名> --ignore-daemonsets --delete-emptydir-data --timeout=300s
...
# 🟡 中风险：会修改集群/资源状态，执行前请确认目标、影响范围与授权
# 1. 查看当前副本数
kubectl get deployment <名称> -o wide

# 2. 手动扩容
kubectl scale deployment <名称> --replicas=<目标数>

# 3. 监控扩容进度
kubectl rollout status deployment/<名称>

# 4. 确认所有新 Pod 就绪
kubectl get pods -l app=<名称> -w
```
### 场景5: 回滚发布
