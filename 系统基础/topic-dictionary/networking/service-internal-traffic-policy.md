...
- **与 [[系统基础/topic-dictionary/networking/topology-aware-routing.md|Topology Aware Routing]] 的互斥**：同一 Service 上不能同时启用 `internalTrafficPolicy: Local` 和 Topology Aware Routing（`service.[[entities/kubernetes.md|[[Kubernetes|kubernetes]]]].io/topology-mode: Auto`），但可以在集群中为不同的 Service 分别使用这两种特性。
