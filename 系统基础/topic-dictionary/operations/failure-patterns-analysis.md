...
> **生产环境问题实战经验**: 基于千次真实故障处理经验总结，涵盖从初级到高级的完整故障分析体系

---

## 知识地图

**本文定位**: 这是一份 Kubernetes 故障分析的完整指南，涵盖问题分类、根因分析方法论、FMEA 分析、MTTR 优化、复盘流程和预防体系，帮助团队系统性地处理和预防问题。

**面向读者**:
- **初学者**: 了解常见问题类型，学会基本的排查思路和工具
- **中级工程师**: 掌握根因分析方法论（5Why、鱼骨图），能独立处理 P1/P2 问题
- **资深专家**: 建立问题预防体系、混沌工程实践、智能化故障预测
...
**关联文件**:
- [./operations-best-practices.md](./operations-best-practices.md) - 运维最佳实践（预防问题的配置标准）
- [12-incident-management-runbooks.md](12-incident-management-runbooks.md) - 事故管理 Runbook（问题发生时的操作手册）
- [16-production-troubleshooting-playbook.md](16-production-troubleshooting-playbook.md) - 故障排查 Playbook（具体排查步骤）
- [15-sli-slo-sla-engineering.md](15-sli-slo-sla-engineering.md) - SLI/SLO（衡量问题影响的指标体系）
...
**一句话定义**: 故障模式分类是将 Kubernetes 中可能发生的各种问题按照发生位置（控制平面/节点/应用/网络/存储）系统化整理，帮助运维人员快速定位问题所在层次。

**类比**: 就像看病时医生先判断「是骨科的问题还是内科的问题」一样，问题分类就是在集群出问题时，先确定问题在哪一层——是「大脑」（控制平面）的问题、「骨骼」（节点）的问题、「器官」（应用）的问题、「血管」（网络）的问题，还是「存储器官」（存储）的问题。

**核心要点**:
- **控制平面问题**: API Server/etcd/调度器/控制器管理器——影响最大，优先级最高
- **节点问题**: [[kubelet|kubelet]]/容器运行时/kube-proxy——影响该节点上所有 Pod
- **应用问题**: CrashLoopBackOff/OOM/配置错误——影响单个应用
- **网络问题**: DNS/CNI/NetworkPolicy/Service——影响通信链路
- **存储问题**: PVC/PV/CSI——影响数据持久化
...
**问题影响传播**:
- **上游问题影响下游**: [[系统基础/topic-dictionary/fundamentals/etcd.md|etcd]] 问题 → API Server 无法读写 → 调度器无法工作 → 新 Pod 无法创建
...
| **etcd数据不一致** | 数据读取异常、集群状态混乱 | 全集群 | P0 | 网络分区、磁盘问题、配置错误 |
...
| **容器运行时异常** | Pod启动失败、容器无法创建 | 节点容器操作 | P1 | Docker daemon问题、镜像损坏、存储问题 |
...
| 故障模式 | 症状表现 | 影响范围 | 紧急程度 | 常见根因 |
|---------|---------|---------|---------|---------|
| **DNS解析失败** | 服务名无法解析、连接超时 | 全部服务间通信 | P0 | CoreDNS问题、网络策略阻止、配置错误 |
| **网络策略阻断** | Pod间通信异常、访问被拒绝 | 受策略影响的通信 | P1 | NetworkPolicy配置错误、标签匹配问题 |
| **CNI插件问题** | Pod无法获取IP、网络不通 | 新Pod网络 | P0 | CNI配置错误、IP地址耗尽、插件版本不兼容 |
...
| 故障模式 | 症状表现 | 影响范围 | 紧急程度 | 常见根因 |
|---------|---------|---------|---------|---------|
| **PVC绑定失败** | Pod卡在ContainerCreating | 使用持久化的应用 | P1 | StorageClass配置错误、存储后端问题、配额限制 |
| **PV挂载超时** | Volume挂载失败、Pod启动缓慢 | 存储依赖的应用 | P1 | CSI驱动问题、网络存储延迟、权限配置错误 |
| **存储IO性能差** | 应用响应慢、数据库超时 | 存储密集型应用 | P2 | 存储类型选择不当、IO争用、存储后端性能瓶颈 |
| **数据丢失风险** | PV删除、快照失败 | 重要数据 | P0 | 误操作、备份策略缺失、存储问题 |

### 常见误区与最佳实践

**常见误区**:
1. **问题发生时直接看应用日志**: 应从全局到局部，先判断问题层级再深入
2. **把所有 CrashLoopBackOff 都当应用 Bug**: 可能是资源不足、配置错误、依赖服务不可用等外部原因
3. **忽略存储问题**: Pod 卡在 ContainerCreating 往往是 PVC 绑定问题，而非调度问题
4. **DNS 问题被误判为应用问题**: 服务间调用超时时，应首先检查 CoreDNS 是否正常

**最佳实践**:
- **建立分层检查清单**: 从基础设施层逐层向上排查，不要跳层
- **标记问题影响范围**: 第一时间判断是全局问题还是局部问题，决定响应级别
- **保留现场**: 诊断前先收集 describe/events/logs 快照，不要急于重启
- **关联变更记录**: 90% 的问题与近期变更相关，首先查看 24h 内的变更历史
...
**一句话定义**: 根因分析（RCA）是通过系统化的方法从问题表象追溯到最本质原因的过程，目标是找到「治本」而非「治标」的解决方案。
...
- **时间线分析**: 按时间顺序还原问题全过程，找到关键转折点
- **影响面分析**: 评估问题对业务、用户、数据、声誉的综合影响
...
| 方法 | 适用场景 | 复杂度 | 所需时间 |
|-----|---------|--------|---------|
| 5Why | 单一因果链的简单问题 | 低 | 15-30 分钟 |
| 鱼骨图 | 多因素复杂问题 | 中 | 1-2 小时 |
| 时间线分析 | 需要还原问题全过程 | 中 | 30-60 分钟 |
...
10:05:00 | 发现API Server CPU使用率95% | 运维人员 | 确认问题
...
2. **只找一个根因**: 复杂问题往往有多个贡献因素，需要同时修复才能真正预防
...
**一句话定义**: 故障树分析（FTA）和失效模式与效应分析（FMEA）是从系统设计角度，**在问题发生前**识别潜在风险点和失效路径的主动分析方法。

**类比**: 如果根因分析是「事后查案」，那 FMEA 就是「事前安检」——就像飞机起飞前的安全检查清单，系统性地检查每个组件可能出什么问题、后果有多严重、被发现的难度有多大。

**核心要点**:
- **故障树（FTA）**: 从顶层问题事件出发，逐层分解可能的原因，形成树状结构
...
```markdown
| 组件 | 可能问题 | 严重度(1-10) | 频率(1-10) | 可检测性(1-10) | RPN | 优先级 |
|------|---------|-------------|-----------|---------------|-----|--------|
| etcd | 磁盘满 | 9 | 3 | 2 | 54 | 中 |
...
└─────────────────────────┬───────────────────────────────────┘
        ...
        ┌─────────────────┼─────────────────┐
        ▼                 ▼                 ▼
   硬件问题(30%)      软件问题(50%)      配置问题(20%)
        │                 │                 │
  ┌─────┴─────┐    ┌─────┴─────┐    ┌─────┴─────┐
  ▼           ▼    ▼           ▼    ▼           ▼
磁盘问题   网卡问题  内存泄漏   死锁   证书过期   参数错误
(15%)     (15%)   (25%)     (25%)   (12%)     (8%)
...
**一句话定义**: MTTR（Mean Time To Recovery，平均恢复时间）是衡量团队处理问题效率的核心指标，优化 MTTR 的目标是让问题被更快发现、更快定位、更快修复。

**类比**: MTTR 就像急救中的「黄金 4 分钟」——心脏骤停后 4 分钟内开始 CPR 存活率最高。同理，生产问题的每一秒都意味着业务损失，MTTR 衡量的就是从「发现问题」到「恢复服务」的平均时间。

**核心要点**:
- **MTTD（发现时间）**: 从问题发生到被检测到的时间——靠监控告警
- **MTTI（识别时间）**: 从检测到问题到确认根因的时间——靠诊断工具和经验
...
- **MTTR 仪表板**: 持续跟踪每次问题的 MTTD/MTTI/MTTR，识别改进方向
...
**一句话定义**: 问题复盘（Postmortem）是在故障恢复后，团队系统性回顾问题全过程，提取教训并制定改进措施的标准化流程。
...
```markdown
# SOR (Summary of Restoration) 问题复盘报告

## 基本信息
- **问题编号**: INC-20260205-001
- **问题时间**: 2026-02-05 14:30 - 14:50 (20分钟)
- **问题等级**: P0 - 核心服务中断
- **影响范围**: 用户注册服务100%不可用
- **业务损失**: 约¥200,000
...
**一句话定义**: 预防措施体系是一套从「被动救火」转向「主动预防」的分层防御机制，通过监控、检查、演练、自动化等手段在问题发生前消除风险。
...
- **日常检查**: 每日执行集群健康检查清单
- **混沌工程**: 主动注入问题，验证系统容错能力
- **持续改进**: 将每次问题的教训转化为预防措施
...
**一句话定义**: 问题案例库是团队将真实问题经验系统化整理的知识仓库，记录问题现象、诊断过程、解决方案和预防措施，供所有成员学习参考。

**类比**: 就像法律界的「判例法」一样——律师通过研究过往案例来应对新案件。运维工程师研究历史问题案例，能更快识别和解决类似问题。

**核心要点**:
- **案例标准化**: 每个案例包含背景、时间线、诊断、根因、解决方案、预防措施
- **可搜索**: 按问题类型、组件、严重度分类，便于快速查找
- **持续积累**: 每次新问题都补充到案例库
- **定期培训**: 用案例库作为团队培训材料
...
| **忽视监控告警** | 建立告警分级响应机制 | 小问题可能演变成大问题 |
| **单独处理问题** | 团队协作共同诊断 | 复杂问题需要多方 expertise |
| **忽略变更历史** | 建立完整的变更日志 | 90%的问题源于变更 |
| **缺乏回滚预案** | 制定详细的回滚计划 | 快速回滚能大幅缩短MTTR |

#### 故障处理黄金法则

1. **保持冷静**: 故障处理的第一要务是保持头脑清醒
2. **快速确认**: 5分钟内确认问题影响范围和严重程度
3. **及时沟通**: 立即通知相关方，避免信息不对称
4. **记录完整**: 详细记录故障处理全过程
5. **彻底复盘**: 问题解决后必须进行深度复盘
...
**一句话定义**: 高级故障诊断技术是超越基础 kubectl 命令的进阶方法，包括分布式追踪、异常检测算法、混沌工程和自愈系统，用于处理复杂的分布式系统问题。
...
- **混沌工程**: Chaos Mesh/Litmus 主动注入问题，验证系统韧性
- **自愈系统**: 基于规则或 AI 自动检测和修复常见问题
...
| 传播类型 | 特征表现 | 诊断要点 | 阻断策略 |
|----------|----------|----------|----------|
| **垂直传播** | 从底层向上层逐级影响 | 从基础设施开始排查 | 隔离问题节点 |
| **水平传播** | 同层组件间相互影响 | 分析依赖关系图 | 断开环形依赖 |
| **跨层传播** | 跨越多个抽象层影响 | 建立端到端追踪 | 实施熔断机制 |
| **连锁反应** | 一个小问题引发雪崩效应 | 监控关键指标阈值 | 设置降级预案 |
...
| **节点问题** | 测试自动恢复机制 | 中 | 月度 | 恢复时间、数据丢失 |
| **存储问题** | 验证数据保护策略 | 高 | 半年 | RTO、RPO达成率 |
...
2. **自愈系统替代人工**: 自愈处理已知故障模式，未知问题仍需人工介入
3. **异常检测算法过于灵敏**: 产生大量误报，需要根据历史数据调整阈值

**最佳实践**:
- **混沌工程三步曲**: 建立基线 → 注入问题 → 验证恢复 → 对比基线
...
| 主题 | 文件 | 说明 |
|-----|------|------|
| 运维最佳实践 | [./operations-best-practices.md](./operations-best-practices.md) | 预防问题的配置标准 |
| 核心概念 | [../fundamentals/kubernetes-concepts-reference.md](../fundamentals/kubernetes-concepts-reference.md) | 不熟悉术语时查阅 |
| CLI 命令 | [../tooling/cli-commands.md](../tooling/cli-commands.md) | 故障排查命令速查 |
...
| SLI/SLO | [15-sli-slo-sla-engineering.md](15-sli-slo-sla-engineering.md) | 衡量问题影响 |
| 容量规划 | [13-capacity-planning-forecasting.md](13-capacity-planning-forecasting.md) | 预防资源不足问题 |

## 故障排查

> 本文件即为完整的故障模式与根因分析字典。详见上方各章节：1.常见故障模式分类、2.根因分析方法论、3.故障树分析(FMEA)、4.MTTR优化策略、7.真实问题案例库、8.高级故障诊断技术。
