...
本清单旨在从应用开发者的视角，提供在 [[实体/kubernetes.md|[[Kubernetes|kubernetes]]]] 上运行应用的安全基线指南。该列表并非详尽无遗，而是随着时间不断发展。清单中的项目顺序不反映优先级，某些项目在各小节下的段落中有更详细的说明。本文档假设“开发者”是指与命名空间范围对象交互的 Kubernetes 集群用户。
...
- 除非 Pod 明确需要访问 [[系统基础/知识字典/fundamentals/the-kubernetes-api.md|Kubernetes API]] 才能运行，否则应将 `automountServiceAccountToken` 设置为 `false`。
...
- [[系统基础/知识字典/security/bank-vaults.md|Bank Vaults Vault 集成]]
