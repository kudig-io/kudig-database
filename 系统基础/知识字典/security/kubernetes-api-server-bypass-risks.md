...
攻击者如果能够直接访问某些组件或配置，就可以在不经过 API server 的情况下创建、修改或删除集群中的资源，从而绕过 API server 的审计和准入控制。主要风险点包括：Static [[Pods|Pods]]、[[kubelet|Kubelet]] API、[[系统基础/知识字典/fundamentals/etcd.md|etcd]] API 和容器运行时套接字。
