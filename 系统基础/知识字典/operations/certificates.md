...
- **[[系统基础/topic-dictionary/fundamentals/etcd.md|etcd]] Server**：Kubernetes 的后端数据库，需要 TLS 保护键值存储访问。
- **[[kubelet|Kubelet]]**：每个节点上的代理，提供 Pod/容器管理接口。
- **Front-Proxy（可选）**：用于 API Server 的聚合层（Aggregation Layer），[[系统基础/topic-dictionary/fundamentals/the-kubernetes-api.md|扩展 Kubernetes API]]PI|Kubernetes API]]。
