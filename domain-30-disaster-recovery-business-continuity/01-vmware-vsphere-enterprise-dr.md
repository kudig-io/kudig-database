...
## VMware Site Recovery Manager 深度实践
...
vSAN 拉伸集群（Stretched Cluster）是 vSphere 实现同城双活的核心技术。通过将 vSAN 集群横跨两个数据中心，配合见证主机（Witness Host）在第三个位置，实现数据同步复制和自动故障切换。当任一数据中心发生故障时，另一个数据中心的虚拟机无需手动干预即可继续运行。
